...
import { dkd_palette_value, dkd_privacy_url_value } from './dkd_courier_theme';
import { dkd_haptic_value, dkd_modal_head_value, dkd_profile_avatar_value } from './dkd_courier_ui';
import {
...
function dkd_profile_modal_value({
  ...
  useEffect(() => {
    ...
    dkd_fetch_my_account_deletion_request_value({ dkd_user_id_value })
      .then((dkd_result_value) => {
        dkd_set_delete_request_value(dkd_result_value?.dkd_data_value || null);
      });
  }, [dkd_profile_value, dkd_user_id_value, dkd_visible_value]);

  const dkd_save_value = async () => {
    ...
    if (dkd_response_value?.error) {
      return Alert.alert('Profil', String(dkd_response_value.error.message || dkd_response_value.error));
    }
  ...
  const dkd_request_delete_value = async () => {
    dkd_set_busy_value(true);
    ...
    const dkd_refresh_value = await dkd_fetch_my_account_deletion_request_value({
      dkd_user_id_value,
    });
    dkd_set_delete_request_value(dkd_refresh_value?.dkd_data_value || null);
  };

  const dkd_cancel_delete_value = async () => {
    dkd_set_busy_value(true);

    const dkd_result_value = await dkd_cancel_account_deletion_request_value({
      dkd_user_id_value,
    });
  ...
  const dkd_input_rows_value = [
    ['GÖRÜNEN AD', dkd_nickname_value, dkd_set_nickname_value, 'Ad'],
    ['AVATAR YEDEĞİ', dkd_avatar_value, dkd_set_avatar_value, '🦅'],
    ['ŞEHİR', dkd_city_value, dkd_set_city_value, 'Antalya'],
    ['İLÇE / BÖLGE', dkd_region_value, dkd_set_region_value, 'Muratpaşa'],
  ];

  const dkd_profile_hero_value = dkd_e_value(
    LinearGradient,
    {
      colors: ['#101C34', '#0A1728', '#07101B'],
      style: dkd_styles_value.dkd_profile_hero_value,
    },
    dkd_e_value(dkd_profile_avatar_value, {
      ...
      dkd_size_value: 92,
      dkd_on_error_value: dkd_on_avatar_error_value,
    }),
    dkd_e_value(
      Text,
      { style: dkd_styles_value.dkd_profile_hero_name_value },
      String(dkd_profile_value?.nickname || 'DraBornGo'),
    ),
    dkd_e_value(
      Text,
      { style: dkd_styles_value.dkd_profile_hero_meta_value },
      [dkd_profile_value?.dkd_city, dkd_profile_value?.dkd_region]
        .filter(Boolean)
        .join(' • ') || 'Bölge bilgisi eklenmedi',
    ),
  );

  const dkd_profile_inputs_value = dkd_input_rows_value.map((dkd_input_value) => (
    dkd_e_value(
      React.Fragment,
      { key: dkd_input_value[0] },
      dkd_e_value(
        Text,
        { style: dkd_styles_value.dkd_form_label_value },
        dkd_input_value[0],
      ),
      dkd_e_value(TextInput, {
        value: dkd_input_value[1],
        onChangeText: dkd_input_value[2],
        placeholder: dkd_input_value[3],
  ...
  const dkd_save_button_value = dkd_e_value(
    Pressable,
    {
      disabled: dkd_busy_value,
      onPress: dkd_save_value,
      style: [
        dkd_styles_value.dkd_primary_value,
        { backgroundColor: dkd_palette_value.dkd_purple_value },
      ],
    },
    dkd_e_value(
      Text,
      { style: dkd_styles_value.dkd_primary_text_value },
      dkd_busy_value ? 'İŞLENİYOR…' : 'PROFİLİ KAYDET',
    ),
  );

  const dkd_privacy_button_value = dkd_e_value(
    Pressable,
    {
      onPress: () => Linking.openURL(dkd_privacy_url_value),
      style: dkd_styles_value.dkd_outline_value,
    },
    dkd_e_value(
      Text,
      { style: dkd_styles_value.dkd_outline_text_value },
      'GİZLİLİK POLİTİKASINI AÇ',
    ),
  );

  const dkd_delete_control_value = dkd_delete_request_value
    ? dkd_e_value(
      View,
      { style: dkd_styles_value.dkd_notice_value },
      dkd_e_value(
        Text,
        { style: dkd_styles_value.dkd_notice_title_value },
        'Silme talebi: ' + String(dkd_delete_request_value.dkd_status_value || 'pending'),
      ),
      dkd_e_value(
        Pressable,
        {
          disabled: dkd_busy_value,
          onPress: dkd_cancel_delete_value,
          style: dkd_styles_value.dkd_outline_value,
        },
        dkd_e_value(
          Text,
          { style: dkd_styles_value.dkd_outline_text_value },
          'BEKLEYEN TALEBİ İPTAL ET',
        ),
      ),
    )
    : dkd_e_value(
      ...
      {
        disabled: dkd_busy_value,
        onPress: dkd_request_delete_value,
        style: dkd_styles_value.dkd_danger_value,
      },
      dkd_e_value(
        Text,
        { style: dkd_styles_value.dkd_danger_text_value },
        'HESABIMI VE İLİŞKİLİ VERİLERİMİ SİLME TALEBİ OLUŞTUR',
      ),
    );
  ...
  return dkd_e_value(
    ...
    dkd_e_value(
      ...
      dkd_modal_head_value(
        'Profil Merkezi',
        'Kimlik, fotoğraf, bölge ve hesap',
        dkd_on_close_value,
        dkd_palette_value.dkd_purple_value,
      ...
      dkd_e_value(
        ...
        dkd_profile_hero_value,
        ...dkd_profile_inputs_value,
        dkd_save_button_value,
        dkd_privacy_button_value,
        dkd_delete_control_value,
      ),
    ),
  );
}

export { dkd_profile_modal_value };
