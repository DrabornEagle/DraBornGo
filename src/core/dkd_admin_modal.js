...
function dkd_admin_modal_value({ dkd_visible_value, dkd_on_close_value }) {
  ...
  const dkd_load_value = useCallback(async () => {
    ...
    dkd_set_requests_value(
      Array.isArray(dkd_delete_value?.dkd_data_value)
        ? dkd_delete_value.dkd_data_value
        : [],
    );
  }, []);
  ...
  const dkd_decide_value = async (dkd_row_value, dkd_approve_value) => {
    const dkd_request_id_value = String(dkd_row_value?.dkd_id_value || '');
    const dkd_result_value = dkd_approve_value
      ? await dkd_approve_account_deletion_request_value({
          dkd_request_id_value,
          dkd_admin_note_value: 'v0.0.6 yönetim ekranından onaylandı.',
        })
      : await dkd_reject_account_deletion_request_value({
          dkd_request_id_value,
          dkd_admin_note_value: 'v0.0.6 yönetim ekranından reddedildi.',
        });
  ...
  const dkd_metrics_value = dkd_e_value(
    View,
    { style: dkd_styles_value.dkd_admin_metrics_value },
    dkd_metric_value(
      'people-outline',
      'Kullanıcı',
      String(dkd_counts_value.users || 0),
      dkd_palette_value.dkd_cyan_value,
    ),
    dkd_metric_value(
      'cube-outline',
      'Teslimat',
      String(dkd_counts_value.jobs || 0),
      dkd_palette_value.dkd_green_value,
    ),
    dkd_metric_value(
      'document-text-outline',
      'Başvuru',
      String(dkd_counts_value.applications || 0),
      dkd_palette_value.dkd_orange_value,
    ),
  );

  const dkd_request_cards_value = dkd_requests_value.map((dkd_row_value) => {
    const dkd_status_value = String(dkd_row_value.dkd_status_value || '');
    const dkd_pending_value = dkd_status_value === 'pending';

    return dkd_e_value(
      View,
      {
        key: String(dkd_row_value.dkd_id_value),
        style: dkd_styles_value.dkd_simple_card_value,
      },
      dkd_panel_title_value(
        'person-outline',
        String(
          dkd_row_value.dkd_display_name_value ||
            dkd_row_value.dkd_user_email_value ||
            'Kullanıcı',
        ),
        dkd_palette_value.dkd_gold_value,
      ),
      dkd_badge_value(
        String(dkd_row_value.dkd_status_value || 'pending').toUpperCase(),
        dkd_palette_value.dkd_gold_value,
      ),
      dkd_pending_value
        ? dkd_e_value(
            ...
            dkd_e_value(
              Pressable,
              {
                onPress: () => dkd_decide_value(dkd_row_value, false),
                style: dkd_styles_value.dkd_danger_compact_value,
              },
              dkd_e_value(
                Text,
            ...
            dkd_e_value(
              Pressable,
              {
                onPress: () => dkd_decide_value(dkd_row_value, true),
                style: [
                  dkd_styles_value.dkd_primary_compact_value,
                  { backgroundColor: dkd_palette_value.dkd_green_value },
                ],
              },
              dkd_e_value(
                Text,
  ...
  const dkd_requests_content_value = dkd_requests_value.length
    ? dkd_request_cards_value
    : [
        dkd_e_value(
          Text,
          { key: 'empty', style: dkd_styles_value.dkd_empty_text_value },
          'Bekleyen talep yok.',
        ),
      ];
  ...
  return dkd_e_value(
    ...
    dkd_e_value(
      ...
      dkd_e_value(
        ...
        ...dkd_requests_content_value,
      ),
    ),
  );
}

export { dkd_admin_modal_value };
