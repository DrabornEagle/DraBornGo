export const COURIER_STATUS_META = {
  none:{key:'none',label:'Kurye Erişimi Yok',shortLabel:'Erişim Yok',toneBg:'rgba(255,255,255,0.08)',toneText:'#FFFFFF'},
  pending:{key:'pending',label:'Kurye Onayı Bekliyor',shortLabel:'Onay Bekliyor',toneBg:'rgba(255,193,7,0.18)',toneText:'#FFE9A8'},
  approved:{key:'approved',label:'Kurye Onayı Aktif',shortLabel:'Kurye Onaylı',toneBg:'rgba(33,212,253,0.18)',toneText:'#DFF8FF'},
  rejected:{key:'rejected',label:'Kurye Erişimi Reddedildi',shortLabel:'Red',toneBg:'rgba(255,99,132,0.16)',toneText:'#FFD7E0'},
...
export function getCourierMeta(profile={}){
  ...
  let description='Kurye erişimi hesabında aktif değil.';
  if(status==='pending')description='Kurye erişimin yönetim incelemesinde.';
  if(status==='approved')description='Kurye onayın aktif.';
  if(status==='rejected')description='Kurye erişimin onaylanmadı.';
