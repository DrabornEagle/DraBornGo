...
export function humanizeAdminError(dkd_error_value, contextTable = '') {
  ...
  if ((low.includes('permission denied') || low.includes('row-level security') || low.includes('rls')) && (low.includes('dkd_drops') || ctx === 'dkd_drops')) {
    return 'Ödül noktası verisine erişim engellendi. Supabase admin yetkisi ve dkd_drops RLS politikasını kontrol et.';
  }

  if (low.includes('expo-file-system') || low.includes('copyasync') || low.includes('readasstringasync')) {
    if (ctx === 'dkd_card_defs') {
      return 'Kart görsel yükleme katmanı Expo SDK 54 ile çakıştı. Bu düzeltme dosya işlemlerini expo-file-system/legacy üzerinden çalıştırır. Paketi kurup expo-file-system paketini tekrar eşitle.';
    }
    if (ctx === 'dkd_boss_defs') {
      return 'Özel hedef görsel yükleme tarafında dosya sistemi hatası oluştu. Görsel seçimini tekrar dene veya görsel yükleme akışını yeniden kontrol et.';
    }
  }
