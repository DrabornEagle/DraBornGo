...
export function collectionPointValueFromRarity(dkd_result_value) {
...
export function buildCollectionPointSummary(cards = []) {
  const byCard = {};
  ...
  const entries = Object.values(byCard).sort((dkd_left_item, dkd_right_item) => {
    const dkd_weight_delta = collectionPointValueFromRarity(dkd_right_item.rarity) - collectionPointValueFromRarity(dkd_left_item.rarity);
  ...
  const potentialCollectionPoints = duplicates.reduce((sum, dkd_error_value) => sum + Math.max(0, dkd_error_value.count - 1) * collectionPointValueFromRarity(dkd_error_value.rarity), 0);
  ...
  return {
    ...
    duplicates,
    potentialCollectionPoints,
    duplicateCards,
    countsByRarity,
  };
}

export const buildShardSummary = buildCollectionPointSummary;
export const shardValueFromRarity = collectionPointValueFromRarity;
