...
const dkd_raw_mapbox_access_token_value = dkd_public_env_value('EXPO_PUBLIC_MAPBOX_ACCESS_TOKEN');
...
export function dkd_mapbox_access_token_problem_text_value() {
  if (!dkd_raw_mapbox_access_token_value) return 'Mapbox erişim anahtarı eksik. .env içine EXPO_PUBLIC_MAPBOX_ACCESS_TOKEN eklenmeli.';
  if (dkd_mapbox_placeholder_pattern_value.test(dkd_raw_mapbox_access_token_value)) return 'Mapbox erişim anahtarı hâlâ örnek/placeholder görünüyor. Mapbox hesabındaki gerçek pk. anahtar yazılmalı.';
  if (!dkd_mapbox_access_token_ready_value) return 'Mapbox erişim anahtarı formatı hatalı. Public anahtar pk. ile başlamalı ve eksiksiz olmalı.';
  return '';
}
