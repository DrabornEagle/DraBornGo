...
const STORAGE_KEY = 'dkd_draborngo:achievements:v3';
...
const CATEGORY_UI = {
  adventure: {
    ...
    label: 'Av & Özel Hedef',
    emoji: '🗺️',
    desc: 'Sandık ve özel hedef odaklı rozetler.',
  },
  market: {
    id: 'market',
    label: 'Pazar',
...
export const ACHIEVEMENT_DEFS = [
  ...
  {
    id: 'boss-hunt',
    title: 'Özel Hedef Avı',
    desc: '3 özel hedef noktasını başarıyla tamamla.',
    badgeEmoji: '👑',
    rarity: 'epic',
    category: 'adventure',
    target: 3,
...
function rewardLabel(def) {
  const parts = [];
  if (Number(def?.rewardPuan || 0) > 0) parts.push(`+${Number(def.rewardPuan)} puan`);
    ...
    if (schemaMissing) {
      ...
      return {
        data: {
          ...
          puan: Number(fallback?.data?.dkd_puan ?? optimistic.dkd_puan),
          xp: optimistic.xp,
...
export async function claimAchievementReward(userId, profile, achievementId) {
  ...
  const reward = await applyRewardToProfile(userId, profile, {
    dkd_puan_delta_value: Number(achievement.rewardPuan || achievement.reward_puan || 0),
