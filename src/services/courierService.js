...
export function dkd_peek_cached_courier_jobs_value() {
  return dkd_read_cached_job_rows_value();
}
  ...
  return {
    dkd_unsubscribe: () => {
      try {
        ...
        console.log('[DraBornGo][courier-realtime]', dkd_error_value?.message || String(dkd_error_value));
      }
    },
  };
}
...
async function dkd_select_active_job_rows_value() {
  let dkd_result_value = await supabase
    ...
    .select(dkd_courier_select_value)
    .eq('is_active', true)
  ...
  if (dkd_result_value?.error && dkd_is_missing_optional_column_error(dkd_result_value.error)) {
    dkd_result_value = await supabase
      .from('dkd_courier_jobs')
      .select('id, title, pickup, dropoff, distance_km, eta_min, status, job_type, assigned_user_id, created_at, updated_at, accepted_at, completed_at, pickup_status, picked_up_at, merchant_name, product_title, delivery_note, delivery_address_text, pickup_lat, pickup_lng, dropoff_lat, dropoff_lng, fee_tl, is_active')
      .eq('is_active', true)
      .order('created_at', { ascending: false })
      .limit(60);
  }
...
async function dkd_enrich_jobs_with_cargo_value(dkd_rows_value) {
  ...
  const dkd_shipment_id_values = dkd_rows_safe_value
    .map((dkd_row_value) => Number(dkd_row_value?.cargo_shipment_id))
    .filter((dkd_value) => Number.isFinite(dkd_value) && dkd_value > 0);
...
async function dkd_fetch_courier_jobs_remote_value() {
  ...
  if (!dkd_rpc_result_value?.error) {
    return { data: await dkd_enrich_jobs_with_cargo_value(dkd_rpc_result_value.data), error: null };
  }
...
export async function dkd_reject_courier_job(dkd_job_id_value) {
  ...
  const dkd_result_value = await dkd_run_rpc_with_fallback('dkd_courier_job_reject_dkd', [
...
export async function dkd_lock_courier_delivery_state_value(dkd_job_id_value) {
  ...
  const dkd_result_value = await dkd_run_rpc_with_fallback('dkd_courier_delivery_lock_dkd', [
    { dkd_param_job_id: dkd_numeric_job_id_value },
    { dkd_job_id: dkd_numeric_job_id_value },
  ]);
...
export async function dkd_unlock_courier_delivery_state_value(dkd_job_id_value) {
  const dkd_numeric_job_id_value = Number(dkd_job_id_value);
  if (!Number.isFinite(dkd_numeric_job_id_value) || dkd_numeric_job_id_value <= 0) return { data: null, error: null, dkd_skipped_value: true };
  const dkd_result_value = await dkd_run_rpc_with_fallback('dkd_courier_delivery_unlock_dkd', [
    { dkd_param_job_id: dkd_numeric_job_id_value },
    { dkd_job_id: dkd_numeric_job_id_value },
  ]);
  if (dkd_result_value?.error && dkd_is_missing_function_error(dkd_result_value.error)) return { data: null, error: null, dkd_skipped_value: true };
...
export async function dkd_emit_courier_customer_status_push_value(dkd_job_id_value, dkd_event_key_value = 'courier_job_status_changed') {
  ...
  try {
    const dkd_edge_result_value = await supabase.functions.invoke('send-push-event-bridge', {
      body: {
  ...
  const dkd_rpc_result_value = await dkd_run_rpc_with_fallback('dkd_courier_job_emit_customer_status_push_dkd', [
    { dkd_param_job_id: dkd_numeric_job_id_value, dkd_param_event_key: dkd_event_key_text_value },
    { dkd_job_id: dkd_numeric_job_id_value, dkd_event_key: dkd_event_key_text_value },
  ]);
...
export async function acceptCourierJob(dkd_job_id_value, dkd_current_location_value = null) {
  ...
  const dkd_result_value = await dkd_run_rpc_with_fallback('dkd_courier_job_accept', [
    {
      dkd_param_job_id: dkd_numeric_job_id_value,
      dkd_param_live_lat: Number.isFinite(dkd_live_lat_value) ? dkd_live_lat_value : null,
      dkd_param_live_lng: Number.isFinite(dkd_live_lng_value) ? dkd_live_lng_value : null,
  ...
  if (!dkd_result_value?.error) {
    dkd_clear_courier_jobs_cache_value();
    dkd_queue_customer_push_value(dkd_numeric_job_id_value, 'courier_job_accepted');
  }
  return dkd_result_value;
}
...
export async function markCourierJobPickedUp(dkd_job_id_value, dkd_input_value = {}) {
  ...
  const dkd_result_value = await dkd_run_rpc_with_fallback('dkd_courier_job_mark_picked_up', [
    dkd_proof_url_value ? { dkd_param_job_id: dkd_numeric_job_id_value, dkd_param_pickup_proof_image_url: dkd_proof_url_value } : null,
    { dkd_job_id: dkd_numeric_job_id_value },
    { dkd_param_job_id: dkd_numeric_job_id_value },
  ].filter(Boolean));
  if (!dkd_result_value?.error) {
    dkd_clear_courier_jobs_cache_value();
    dkd_queue_customer_push_value(dkd_numeric_job_id_value, 'courier_job_picked_up');
  }
  return dkd_result_value;
}
...
export async function completeCourierJob(dkd_job_id_value) {
  ...
  if (!dkd_result_value?.error) {
    dkd_clear_courier_jobs_cache_value();
    dkd_queue_customer_push_value(dkd_numeric_job_id_value, 'courier_job_delivered');
  }
  return dkd_result_value;
}
