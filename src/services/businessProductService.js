...
const BUSINESS_PRODUCT_BUCKET = 'dkd_draborngo_business_product_art';
...
async function readBase64FromUri(uri) {
  ...
  try {
    ...
    const tempUri = `${FileSystem.cacheDirectory || ''}dkd_draborngo_product_upload_${Date.now()}.${ext}`;
...
export async function upsertMerchantBusinessProduct(input) {
  ...
  const payload = {
    ...
    dkd_price_puan: Math.max(0, Number(input?.pricePuan ?? input?.dkd_price_puan ?? 0) || 0),
  ...
  ({ data, error } = await supabase.rpc('dkd_business_product_upsert', {
    ...
    dkd_param_price_puan: payload.dkd_price_puan,
...
function dkd_normalize_catalog_rows(dkd_rows) {
  return (Array.isArray(dkd_rows) ? dkd_rows : []).map((row) => ({
    ...
    dkd_reward_puan: Number(row?.dkd_reward_puan ?? row?.price_puan ?? row?.point_price ?? 0),
    ...
    delivery_fee_tl: Number(row?.dkd_delivery_fee_tl ?? row?.delivery_fee_tl ?? row?.dkd_fee_tl ?? row?.fee_tl ?? 0),
...
export async function dkd_create_business_product_point_order(dkd_input_value) {
  const dkd_product_key = (typeof dkd_input_value === 'string' ? String(dkd_input_value || '').trim() : String(dkd_input_value?.dkd_point_order_key || dkd_input_value?.id || '').trim()).replace(/^merchant_product:/, '');
  const dkd_address_text = String(dkd_input_value?.deliveryAddressText || '').trim();
  const dkd_delivery_note = String(dkd_input_value?.deliveryNote || '').trim();
  const dkd_delivery_lat = dkd_input_value?.deliveryLat == null ? null : Number(dkd_input_value.deliveryLat);
  const dkd_delivery_lng = dkd_input_value?.deliveryLng == null ? null : Number(dkd_input_value.deliveryLng);

  return supabase.rpc('dkd_business_product_create_point_order_dkd', {
