...
const STORAGE_KEY = 'dkd_draborngo:liveops:v3';
const MAX_ACTIVITY_ENTRIES = 8;

const DAILY_REWARD_CYCLE = [
  { dkd_puan: 25, xp: 10, label: 'Başlangıç Kasası' },
  { dkd_puan: 35, xp: 12, label: 'Rota Isıtma' },
  { dkd_puan: 45, xp: 14, label: 'Ankara Turu' },
  { dkd_puan: 60, xp: 18, label: 'Radar Hızı' },
  { dkd_puan: 80, xp: 24, label: 'Gece Avı' },
  { dkd_puan: 110, xp: 30, label: 'Özel Hedef Hazırlığı' },
  { dkd_puan: 150, xp: 40, label: 'Haftalık Zirve' },
];


const STREAK_MILESTONES = [
  { days: 3, dkd_puan: 70, xp: 20, tokens: 1, label: '3 Gün Serisi' },
  { days: 5, dkd_puan: 120, xp: 32, tokens: 2, label: '5 Gün Serisi' },
  { days: 7, dkd_puan: 180, xp: 50, tokens: 3, label: '7 Gün Serisi' },
];

const EVENT_CHEST_COST = 3;
const EVENT_CHEST_POOL = [
  { id: 'ops-cache-lite', dkd_puan: 55, xp: 16, tokens: 0, label: 'Ops Cache Lite' },
  { id: 'ops-cache-plus', dkd_puan: 80, xp: 22, tokens: 1, label: 'Ops Cache Plus' },
  { id: 'ops-cache-xp', dkd_puan: 40, xp: 34, tokens: 0, label: 'XP Pulse Cache' },
];

export const LIVEOPS_STORE_ITEMS = [
  { id: 'dkd_point_reward_pack', title: 'Kazanılmış Puan Ödülü', cost: 2, dkd_puan: 60, xp: 0, label: 'Kazanılmış puan ödülü' },
  { id: 'dkd_xp_reward_boost', title: 'XP Etkinlik Ödülü', cost: 2, dkd_puan: 0, xp: 32, label: 'Seviye ödülünü güçlendirir' },
  { id: 'dkd_ops_reward_cache', title: 'Operasyon Etkinlik Ödülü', cost: 4, dkd_puan: 95, xp: 38, label: 'Dengeli puan ve XP ödülü' },
];
...
export const LIVEOPS_EVENTS = [
  {
    ...
    subtitle: 'Günlük sandık ve özel hedef akışını hızlandır, seri bonuslarını topla.',
    ...
    tasks: [
      { key: 'evt_daily_chest_1', title: 'Isınma Sandığı', desc: 'Bugün 1 sandık aç.', refType: 'dailyChest', target: 1, reward_puan: 25, reward_xp: 6 },
      { key: 'evt_daily_chest_3', title: 'Hızlı Tur', desc: 'Bugün toplam 3 sandık aç.', refType: 'dailyChest', target: 3, reward_puan: 45, reward_xp: 12 },
      { key: 'evt_daily_boss_1', title: 'Özel Hedef Turu', desc: 'Bugün bir özel hedef oturumunu tamamla.', refType: 'dailyBoss', target: 1, reward_puan: 80, reward_xp: 20 },
    ],
  },
  {
    id: 'night-route',
    ...
    tasks: [
      { key: 'evt_weekly_chest_5', title: 'Uzun Mesai', desc: 'Bu hafta 5 sandık aç.', refType: 'weeklyChest', target: 5, reward_puan: 55, reward_xp: 14 },
      { key: 'evt_weekly_unique_3', title: 'Yeni Noktalar', desc: 'Bu hafta 3 farklı drop keşfet.', refType: 'weeklyUnique', target: 3, reward_puan: 70, reward_xp: 18 },
      { key: 'evt_shard_upgrade_1', title: 'Gece Forge', desc: 'Bir kart yükseltmesi yap.', refType: 'shardUpgrade', target: 1, reward_puan: 90, reward_xp: 22 },
    ],
  },
  {
    id: 'market-rush',
    title: 'Market Rush',
    subtitle: 'Ekonomi haftası açık. İlan ver, satın al ve koleksiyon puanı akışını hızlandır.',
    ...
    tasks: [
      { key: 'evt_market_list_1', title: 'Pazar Açılışı', desc: 'Market’e 1 ilan ver.', refType: 'marketList', target: 1, reward_puan: 40, reward_xp: 10 },
      { key: 'evt_market_buy_1', title: 'Hızlı Alım', desc: 'Koleksiyona 1 kart ekle.', refType: 'marketBuy', target: 1, reward_puan: 55, reward_xp: 14 },
      { key: 'evt_shard_craft_1', title: 'Forge Çıkışı', desc: 'Koleksiyon puanı ile 1 kart üret.', refType: 'shardCraft', target: 1, reward_puan: 75, reward_xp: 18 },
    ],
  },
  {
    id: 'forge-district',
    title: 'Forge District',
    subtitle: 'Koleksiyon fazlalıklarını erit, yükselt ve özel kart hazırla.',
    ...
    tasks: [
      { key: 'evt_recycle_3', title: 'Koleksiyon Puanı Dönüştürücü', desc: 'Toplam 3 kopya kartı recycle et.', refType: 'recycleDuplicate', target: 3, reward_puan: 50, reward_xp: 12 },
      { key: 'evt_shard_upgrade_2', title: 'Yükseltme Hattı', desc: '1 kart yükseltmesi yap.', refType: 'shardUpgrade', target: 1, reward_puan: 85, reward_xp: 20 },
      { key: 'evt_boss_ticket_1', title: 'Özel Kart', desc: '1 özel kart üret.', refType: 'bossTicketCraft', target: 1, reward_puan: 110, reward_xp: 26 },
    ],
  },
];
...
export const LIVEOPS_WEEKLY_PLANS = [
  ...
  },
  {
    ...
    title: 'Özel Hedef + Market Salınımı',
    subtitle: 'Hafta ortasında özel hedef odaklı, hafta sonunda ekonomi ve koleksiyon hızında kalan daha net plan.',
    days: [
...
function buildStreakMilestones(raw = {}, streak = 0, runId = dayKeyFromDate()) {
  ...
  return STREAK_MILESTONES.map((entry) => {
    ...
    return {
      ...
      rewardLabel: `+${Number(entry.dkd_puan || 0)} puan${Number(entry.xp || 0) ? ` • +${Number(entry.xp)} XP` : ''}${Number(entry.tokens || 0) ? ` • +${Number(entry.tokens)} Ops` : ''}`,
    };
  });
}
...
function buildEventChestState(raw = {}) {
  ...
  return {
    ...
    subtitle: 'Ops puanlarını ödül kasasına çevir ve ekstra puan/XP topla.',
  };
}
...
function buildStoreItems(raw = {}) {
  ...
  return LIVEOPS_STORE_ITEMS.map((item) => ({
    ...
    rewardLabel: `+${Number(item.dkd_puan || 0)} puan${Number(item.xp || 0) ? ` • +${Number(item.xp)} XP` : ''}`,
  }));
}

function rewardLabelParts(entry = {}) {
  const parts = [];
  if (Number(entry?.dkd_puan || 0) > 0) parts.push(`+${Number(entry.dkd_puan)} puan`);
...
function actionCategoryLabel(refType) {
  const map = {
    dailyChest: 'Sandık',
    dailyBoss: 'Özel Hedef',
    weeklyChest: 'Haftalık Sandık',
    weeklyBoss: 'Haftalık Özel Hedef',
    weeklyUnique: 'Farklı Drop',
    marketList: 'Market İlanı',
    marketBuy: 'Market Alımı',
    shardCraft: 'Koleksiyon Craft',
    shardUpgrade: 'Koleksiyon Upgrade',
    recycleDuplicate: 'Recycle',
    bossTicketCraft: 'Özel Kart',
  };
...
function buildEventTasks(activeEvent, raw = {}, progress = {}) {
  ...
  return (activeEvent.tasks || []).map((task) => {
    ...
    return {
      ...
      rewardText: `Etkinlik ödülü: +${Number(task?.reward_puan || 0)} 🪙${Number(task?.reward_xp || 0) ? `  +${Number(task.reward_xp)} XP` : ''} • +${Math.max(1, Number(task?.reward_tokens || 1))} Ops`,
    };
  });
}
...
function bannerForState(state) {
  ...
  return {
    ...
    subtitle: `${state.rewardPreview.dkd_puan} puan • ${state.rewardPreview.xp} XP • ${event?.title || 'Canlı etkinlik'}`,
...
async function loadRawLiveOpsState(userId) {
  ...
  try {
    ...
    console.log('[DraBornGo][liveops][loadRaw]', error?.message || String(error));
    return {};
  }
}
...
async function applyRewardToProfile(userId, profile, { dkd_puan_delta_value = 0, xpDelta = 0 } = {}) {
  const currentPuan = Math.max(0, Number(profile?.dkd_puan || 0));
  const currentXp = Math.max(0, Number(profile?.xp || 0));
  const nextPuan = currentPuan + Math.max(0, Number(dkd_puan_delta_value || 0));
  ...
  const { data, error } = await supabase
    .from('dkd_profiles')
    .update({ dkd_puan: optimistic.dkd_puan, xp: optimistic.xp, level: optimistic.level, rank_key: optimistic.rank_key })
    .eq('user_id', userId)
    .select('user_id, dkd_puan, xp, level, rank_key')
    .single();
  ...
  if (error) {
    ...
    if (schemaMissing) {
      const fallback = await supabase
        .from('dkd_profiles')
        .update({ dkd_puan: nextPuan })
        .eq('user_id', userId)
        .select('user_id, dkd_puan')
        .single();
      ...
      return {
        data: {
          user_id: userId,
          dkd_puan: Number(fallback?.data?.dkd_puan ?? optimistic.dkd_puan),
          xp: optimistic.xp,
...
export async function claimLiveOpsDailyReward(userId, profile, progress = {}) {
  ...
  const profileResult = await applyRewardToProfile(userId, profile, {
    dkd_puan_delta_value: reward.dkd_puan,
    xpDelta: reward.xp,
  });
...
export async function claimLiveOpsEventTask(userId, profile, taskKey, progress = {}) {
  ...
  const profileResult = await applyRewardToProfile(userId, profile, {
    dkd_puan_delta_value: task.reward_puan,
  ...
  const nextRaw = await updateRawLiveOpsState(userId, (prev) => ({
    ...
    activityLog: appendActivity(prev, {
      ...
      dkd_puan: Number(task.reward_puan || 0),
...
export async function claimLiveOpsStreakMilestone(userId, profile, days, progress = {}) {
  ...
  const profileResult = await applyRewardToProfile(userId, profile, {
    dkd_puan_delta_value: milestone.dkd_puan,
  ...
  const nextRaw = await updateRawLiveOpsState(userId, (prev) => ({
    ...
    activityLog: appendActivity(prev, {
      ...
      dkd_puan: Number(milestone.dkd_puan || 0),
...
export async function redeemLiveOpsStoreItem(userId, profile, itemId, progress = {}) {
  ...
  const profileResult = await applyRewardToProfile(userId, profile, {
    dkd_puan_delta_value: item.dkd_puan,
    xpDelta: item.xp,
  });
  if (profileResult?.error) {
  ...
  const nextRaw = await updateRawLiveOpsState(userId, (prev) => ({
    ...
    activityLog: appendActivity(prev, {
      ...
      dkd_puan: Number(item.dkd_puan || 0),
      xp: Number(item.xp || 0),
      at: nowIso,
    }),
  }));
