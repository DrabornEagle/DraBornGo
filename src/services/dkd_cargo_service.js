...
const dkd_cargo_package_bucket_value = 'dkd_draborngo_cargo_package_art';

function dkd_slugify_value(dkd_input_value) {
  return String(dkd_input_value || 'cargo').normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-zA-Z0-9]+/g, '-').replace(/^-+|-+$/g, '').toLowerCase() || 'cargo';
}
...
function dkd_base64_to_array_buffer(dkd_base64_value) {
  ...
  let dkd_buffer_length_value = dkd_clean_value.length * 0.75;
  if (dkd_clean_value.endsWith('==')) dkd_buffer_length_value -= 2;
  else if (dkd_clean_value.endsWith('=')) dkd_buffer_length_value -= 1;
  const dkd_array_buffer_value = new ArrayBuffer(Math.max(0, dkd_buffer_length_value));
  ...
  for (let dkd_loop_index_value = 0; dkd_loop_index_value < dkd_clean_value.length; dkd_loop_index_value += 4) {
    ...
    dkd_bytes_value[dkd_write_index_value++] = (dkd_enc1_value << 2) | (dkd_enc2_value >> 4);
    if (dkd_enc3_value !== 64) dkd_bytes_value[dkd_write_index_value++] = ((dkd_enc2_value & 15) << 4) | (dkd_enc3_value >> 2);
    if (dkd_enc4_value !== 64) dkd_bytes_value[dkd_write_index_value++] = ((dkd_enc3_value & 3) << 6) | dkd_enc4_value;
  }
...
async function dkd_read_base64_from_uri(dkd_uri_value) {
  ...
  try {
    ...
    const dkd_temp_uri_value = `${FileSystem.cacheDirectory || ''}dkd_draborngo_cargo_upload_${Date.now()}.${dkd_ext_value}`;
    if (!dkd_temp_uri_value) throw dkd_direct_error_value;
    await FileSystem.copyAsync({ from: dkd_input_value, to: dkd_temp_uri_value });
    try { return await FileSystem.readAsStringAsync(dkd_temp_uri_value, { encoding: FileSystem.EncodingType.Base64 }); }
    finally { try { await FileSystem.deleteAsync(dkd_temp_uri_value, { idempotent: true }); } catch {} }
  }
}
async function dkd_normalize_upload_uri(dkd_uri_value) {
  ...
  try {
    const dkd_output_value = await ImageManipulator.manipulateAsync(dkd_input_value, [{ resize: { width: 1280 } }], { compress: 0.82, format: ImageManipulator.SaveFormat.JPEG });
    return dkd_output_value?.uri || dkd_input_value;
  } catch { return dkd_input_value; }
}
...
export async function dkd_upload_cargo_package_art({ dkd_image_uri, dkd_sender_slug, dkd_content_label }) {
  ...
  const dkd_path_value = `cargo-packages/${dkd_folder_value}/${Date.now()}-${dkd_stem_value}.${dkd_ext_value}`;
  const { error: dkd_upload_error_value } = await supabase.storage.from(dkd_cargo_package_bucket_value).upload(dkd_path_value, dkd_array_buffer_value, { contentType: dkd_mime_value, upsert: true });
  ...
  return { data: { path: dkd_path_value, publicUrl: dkd_public_url_value?.publicUrl || '' }, error: null };
}

export async function dkd_create_cargo_shipment(dkd_input_value) {
  return supabase.rpc('dkd_cargo_shipment_create', {
...
export async function dkd_emit_cargo_shipment_push_event(dkd_create_data_value = {}, dkd_input_value = {}) {
  const dkd_courier_job_id_value = dkd_create_data_value?.dkd_courier_job_id || dkd_create_data_value?.courier_job_id || null;
  const dkd_cargo_shipment_id_value = dkd_create_data_value?.dkd_cargo_shipment_id || dkd_create_data_value?.cargo_shipment_id || dkd_create_data_value?.id || null;
  const dkd_package_title_value = String(dkd_input_value?.dkd_package_content_text || 'Paket').trim() || 'Paket';
  const dkd_order_title_value = dkd_package_title_value.toLocaleLowerCase('tr-TR').includes('sipariş') ? dkd_package_title_value : `${dkd_package_title_value} Siparişi`;
  if (!dkd_courier_job_id_value && !dkd_cargo_shipment_id_value) return { data: { ok: false, dkd_reason_value: 'dkd_missing_cargo_push_ids' }, error: null };
  const dkd_push_payload_value = {
    event_key: 'dkd_cargo_order_created',
    record: {
      id: dkd_courier_job_id_value || dkd_cargo_shipment_id_value,
      job_id: dkd_courier_job_id_value || null,
      dkd_job_id: dkd_courier_job_id_value || null,
      cargo_shipment_id: dkd_cargo_shipment_id_value || null,
      dkd_cargo_shipment_id: dkd_cargo_shipment_id_value || null,
      job_type: 'cargo', dkd_job_type: 'cargo', type: 'cargo', status: 'open',
      title: dkd_order_title_value,
      product_title: dkd_package_title_value,
      pickup: String(dkd_input_value?.dkd_pickup_address_text || '').trim() || 'Gönderici adresi',
      dropoff: String(dkd_input_value?.dkd_delivery_address_text || '').trim() || 'Teslimat adresi',
      delivery_address_text: String(dkd_input_value?.dkd_delivery_address_text || '').trim(),
      delivery_note: String(dkd_input_value?.dkd_delivery_note_text || '').trim(),
      fee_tl: dkd_to_number_or_null(dkd_input_value?.dkd_courier_fee_tl) ?? 0,
      customer_charge_tl: dkd_to_number_or_null(dkd_input_value?.dkd_customer_charge_tl) ?? 0,
      dkd_event_key: 'dkd_cargo_order_created',
    },
  };
  try {
    const dkd_bridge_result_value = await supabase.functions.invoke('send-push-event-bridge', { body: dkd_push_payload_value });
    if (!dkd_bridge_result_value?.error) return dkd_bridge_result_value;
    return supabase.functions.invoke('send-courier-order-alert', { body: dkd_push_payload_value });
  } catch (dkd_error_value) { return { data: null, error: dkd_error_value }; }
}

async function dkd_fetch_direct_my_cargo_shipments_by_user_id_value(dkd_user_id_value) {
  if (!dkd_user_id_value) return { data: [], error: null };
  const dkd_shipment_result_value = await supabase.from('dkd_cargo_shipments')
    ...
    .eq('customer_user_id', dkd_user_id_value).order('created_at', { ascending: false }).limit(30);
  if (dkd_shipment_result_value?.error) return { data: [], error: dkd_shipment_result_value.error };
  ...
  const dkd_profile_result_value = dkd_courier_user_id_list.length ? await supabase.from('dkd_profiles').select('user_id, nickname, courier_vehicle_type, courier_profile_meta').in('user_id', dkd_courier_user_id_list) : { data: [] };
  const dkd_location_result_value = dkd_courier_user_id_list.length ? await supabase.from('dkd_courier_live_locations').select('courier_user_id, lat, lng, eta_min, plate_no, vehicle_type, heading_deg, updated_at').in('courier_user_id', dkd_courier_user_id_list) : { data: [] };
  const dkd_profile_map_value = new Map((Array.isArray(dkd_profile_result_value?.data) ? dkd_profile_result_value.data : []).map((dkd_row_value) => [String(dkd_row_value.user_id), dkd_row_value]));
  const dkd_location_map_value = new Map((Array.isArray(dkd_location_result_value?.data) ? dkd_location_result_value.data : []).map((dkd_row_value) => [String(dkd_row_value.courier_user_id), dkd_row_value]));
  return { data: dkd_normalize_shipment_rows(dkd_shipment_rows_value.map((dkd_row_value) => {
    const dkd_key_value = String(dkd_row_value?.assigned_courier_user_id || '');
    const dkd_profile_value = dkd_profile_map_value.get(dkd_key_value);
    const dkd_location_value = dkd_location_map_value.get(dkd_key_value);
    return { ...dkd_row_value, courier_display_name: dkd_profile_value?.nickname || '', courier_plate_no: dkd_location_value?.plate_no || dkd_row_value?.assigned_courier_plate_no || dkd_profile_value?.courier_profile_meta?.plate_no || '', courier_vehicle_type: dkd_location_value?.vehicle_type || dkd_row_value?.assigned_courier_vehicle_type || dkd_profile_value?.courier_vehicle_type || '', courier_lat: dkd_location_value?.lat, courier_lng: dkd_location_value?.lng, courier_eta_min: dkd_location_value?.eta_min ?? dkd_row_value?.courier_eta_min, courier_heading_deg: dkd_location_value?.heading_deg, courier_location_updated_at: dkd_location_value?.updated_at || null };
  })), error: null };
}
...
export async function dkd_fetch_my_cargo_shipments() {
  ...
  if (!dkd_rpc_result_value?.error) return { data: dkd_normalize_shipment_rows(dkd_rpc_result_value.data), error: null };
  if (!dkd_user_id_value) return { data: [], error: null };
...
export async function dkd_upsert_courier_live_location(dkd_input_value) {
  ...
  const dkd_upsert_result_value = await supabase.from('dkd_courier_live_locations').upsert({ courier_user_id: dkd_user_id_value, lat: dkd_payload_value.dkd_param_lat, lng: dkd_payload_value.dkd_param_lng, eta_min: dkd_payload_value.dkd_param_eta_min, heading_deg: dkd_payload_value.dkd_param_heading_deg, plate_no: dkd_payload_value.dkd_param_plate_no, vehicle_type: dkd_payload_value.dkd_param_vehicle_type, updated_at: new Date().toISOString() }, { onConflict: 'courier_user_id' });
