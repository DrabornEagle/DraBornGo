...
export async function dkd_upload_cargo_package_art({ dkd_image_uri, dkd_sender_slug, dkd_content_label }) {
  ...
  if (dkd_upload_error_value) {
    const dkd_storage_error_text_value = String(
      dkd_upload_error_value?.message ||
      dkd_upload_error_value?.details ||
      dkd_upload_error_value?.hint ||
      ''
    ).toLowerCase();
    if (
      dkd_storage_error_text_value.includes('row-level security') ||
      dkd_storage_error_text_value.includes('rls') ||
      dkd_storage_error_text_value.includes('policy') ||
      dkd_storage_error_text_value.includes('storage.objects') ||
      dkd_storage_error_text_value.includes('bucket')
    ) {
      throw new Error('dkd_cargo_package_storage_rls');
    }
    throw dkd_upload_error_value;
  }
...
export async function dkd_emit_cargo_shipment_push_event(dkd_create_data_value = {}, dkd_input_value = {}) {
  ...
  const dkd_order_title_value = dkd_package_title_value.toLocaleLowerCase('tr-TR').includes('sipariş')
    ? dkd_package_title_value
    : `${dkd_package_title_value} Siparişi`;

  if (!dkd_courier_job_id_value && !dkd_cargo_shipment_id_value) {
    return { data: { ok: false, dkd_reason_value: 'dkd_missing_cargo_push_ids' }, error: null };
  }
  ...
  try {
    ...
    const dkd_legacy_alert_result_value = await supabase.functions.invoke('send-courier-order-alert', { body: dkd_push_payload_value });
    if (!dkd_legacy_alert_result_value?.error) return dkd_legacy_alert_result_value;

    return {
      data: dkd_legacy_alert_result_value?.data || dkd_bridge_result_value?.data || null,
      error: dkd_legacy_alert_result_value?.error || dkd_bridge_result_value?.error || null,
    };
