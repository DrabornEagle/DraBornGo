...
function dkd_urgent_push_record_value(dkd_event_key_value, dkd_safe_order_id_value) {
  const dkd_safe_event_key_value = String(dkd_event_key_value || '').trim();
  const dkd_created_event_value = dkd_safe_event_key_value === 'dkd_urgent_order_created';
  return {
    id: dkd_safe_order_id_value,
    dkd_order_id: dkd_safe_order_id_value,
    order_id: dkd_safe_order_id_value,
    job_type: 'urgent_courier',
    dkd_job_type: 'urgent_courier',
    type: 'urgent_courier',
    dkd_type: 'urgent_courier',
    status: dkd_created_event_value ? 'open' : dkd_safe_event_key_value,
    dkd_status: dkd_created_event_value ? 'open' : dkd_safe_event_key_value,
    title: 'Acil Kurye Siparişi',
    dkd_title: 'Acil Kurye Siparişi',
    pickup: 'Acil alış noktası',
    dkd_pickup: 'Acil alış noktası',
  };
}

async function dkd_invoke_urgent_push_function_value(dkd_function_name_value, dkd_payload_value) {
  try {
    const { data: dkd_data_value, error: dkd_error_value } = await supabase.functions.invoke(dkd_function_name_value, {
...
async function dkd_emit_urgent_courier_push_event(dkd_event_key_value, dkd_order_id_value, dkd_message_id_value = null) {
  ...
  const dkd_payload_value = {
    dkd_event_key: dkd_safe_event_key_value,
    event_key: dkd_safe_event_key_value,
    dkd_order_id: dkd_safe_order_id_value,
    order_id: dkd_safe_order_id_value,
    record: dkd_urgent_push_record_value(dkd_safe_event_key_value, dkd_safe_order_id_value),
  };
  if (dkd_message_id_value) dkd_payload_value.dkd_message_id = String(dkd_message_id_value).trim();

  const dkd_created_event_value = dkd_safe_event_key_value === 'dkd_urgent_order_created';
  if (dkd_created_event_value) {
    const dkd_pool_first_result_value = await dkd_invoke_urgent_push_function_value('send-courier-order-alert', dkd_payload_value);
    const dkd_pool_first_sent_count_value = Number(dkd_pool_first_result_value?.data?.dkd_sent_count_value ?? dkd_pool_first_result_value?.data?.sent_count ?? 0);
    if (dkd_pool_first_result_value?.ok && dkd_pool_first_sent_count_value > 0) {
      return { ...dkd_pool_first_result_value, dkd_primary_channel_value: 'send-courier-order-alert' };
    }
    const dkd_urgent_fallback_result_value = await dkd_invoke_urgent_push_function_value('send-urgent-courier-alert', dkd_payload_value);
    return dkd_urgent_fallback_result_value?.ok
      ? { ...dkd_urgent_fallback_result_value, dkd_fallback_channel_value: 'send-urgent-courier-alert' }
      : dkd_pool_first_result_value;
  }

  const dkd_urgent_result_value = await dkd_invoke_urgent_push_function_value('send-urgent-courier-alert', dkd_payload_value);
  return dkd_urgent_result_value;
}
...
export async function dkd_fetch_urgent_courier_snapshot() {
  return supabase.rpc('dkd_urgent_courier_snapshot_fast_dkd', {
    dkd_param_order_limit: 32,
    dkd_param_message_limit: 24,
  });
}
export async function dkd_fetch_urgent_courier_map_order(dkd_order_id_value) {
  return supabase.rpc('dkd_urgent_courier_live_map_order_dkd', {
    dkd_param_order_id: dkd_order_id_value,
  });
}

export async function dkd_ping_urgent_courier_live_location(dkd_order_id_value, dkd_input_value = {}) {
  return dkd_normalize_rpc_response_value(await supabase.rpc('dkd_urgent_courier_location_ping_dkd', {
    dkd_param_order_id: dkd_order_id_value,
    dkd_param_live_lat: Number.isFinite(Number(dkd_input_value?.dkd_live_lat)) ? Number(dkd_input_value.dkd_live_lat) : null,
    dkd_param_live_lng: Number.isFinite(Number(dkd_input_value?.dkd_live_lng)) ? Number(dkd_input_value.dkd_live_lng) : null,
    dkd_param_heading_deg: Number.isFinite(Number(dkd_input_value?.dkd_heading_deg)) ? Number(dkd_input_value.dkd_heading_deg) : null,
    dkd_param_eta_min: Number.isFinite(Number(dkd_input_value?.dkd_eta_min)) ? Math.max(0, Math.round(Number(dkd_input_value.dkd_eta_min))) : null,
  }));
}


export async function dkd_create_urgent_courier_order(dkd_input_value = {}) {
  const dkd_customer_lat_value = Number(dkd_input_value?.dkd_customer_lat);
  const dkd_customer_lng_value = Number(dkd_input_value?.dkd_customer_lng);
  const dkd_result_value = await supabase.rpc('dkd_urgent_courier_create_order_dkd', {
    ...
    dkd_param_customer_lat: Number.isFinite(dkd_customer_lat_value) ? dkd_customer_lat_value : null,
    dkd_param_customer_lng: Number.isFinite(dkd_customer_lng_value) ? dkd_customer_lng_value : null,
  });
...
export async function dkd_reject_urgent_courier_fee(dkd_order_id_value) {
  const dkd_result_value = await supabase.rpc('dkd_urgent_courier_customer_reject_fee_dkd', {
    dkd_param_order_id: dkd_order_id_value,
  });
  const dkd_normalized_result_value = dkd_normalize_rpc_response_value(dkd_result_value);
  if (!dkd_normalized_result_value?.error) {
    await dkd_emit_urgent_courier_push_event('dkd_urgent_fee_rejected', dkd_rpc_result_order_id_value(dkd_normalized_result_value, dkd_order_id_value));
  }
  return dkd_normalized_result_value;
}
...
export async function dkd_admin_delete_urgent_courier_order(dkd_order_id_value) {
  const dkd_result_value = await supabase.rpc('dkd_admin_urgent_courier_order_delete_dkd', {
    dkd_param_order_id: dkd_order_id_value,
  });
  return dkd_normalize_rpc_response_value(dkd_result_value);
}
