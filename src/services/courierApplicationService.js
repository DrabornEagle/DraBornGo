...
async function readBase64FromUri(uri) {
  ...
  try {
    ...
    const tempUri = `${FileSystem.cacheDirectory || ''}dkd_draborngo_courier_doc_${Date.now()}.${ext}`;
...
export async function uploadCourierDocument({ imageUri, userId, kind }) {
  ...
  const { error } = await supabase.storage
    .from('dkd_draborngo_courier_docs')
    .upload(path, arrayBuffer, { contentType: mime, upsert: true });

  if (error) throw error;

  const { data } = supabase.storage.from('dkd_draborngo_courier_docs').getPublicUrl(path);
