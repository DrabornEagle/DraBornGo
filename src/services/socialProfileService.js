...
const TEMPLATE_PREFIX = 'dkd_draborngo:social:template:';
const HISTORY_PREFIX = 'dkd_draborngo:social:compare-history:';
...
export function buildSocialCollectionSummary(cards = []) {
  ...
  const shard = buildCollectionPointSummary(rows);
...
export function buildPlayerStyleTag(profile, summary) {
  ...
  const puan = Number(profile?.puan || 0);
  ...
  if (puan >= 5000) return 'Pazar Efendisi';
...
export function deriveProfileId(sessionUserId, profile) {
  const raw = String(sessionUserId || profile?.id || profile?.user_id || profile?.nickname || 'DraBornGo')
    .replace(/[^a-zA-Z0-9]/g, '')
    .toUpperCase();
...
export function buildPlayerCardPayload({ sessionUserId, profile, summary, taskState, weeklyTaskState }) {
...
export function parseSharedPlayerCardText(text) {
  ...
  return {
    ...
    styleTag: String(map.tag || 'DraBornGo Oyuncusu'),
...
export function comparePlayerCards(localCard, remoteCard) {
  const pairs = [
    ...
    { key: 'puan', label: 'Puan', local: Number(localCard?.puan || 0), remote: Number(remoteCard?.puan || 0) },
