...
function dkd_missing_restaurant_payment_rpc_value(dkd_error_value) {
  const dkd_error_text_value = String(dkd_error_value?.message || dkd_error_value?.details || dkd_error_value?.hint || dkd_error_value?.code || '').toLowerCase();
  return dkd_error_text_value.includes('pgrst202')
    || dkd_error_text_value.includes('could not find the function')
    || dkd_error_text_value.includes('function public.dkd_service_network_restaurant_wallet_pay_order_dkd')
    || dkd_error_text_value.includes('schema cache');
}

function dkd_paid_restaurant_response_value(dkd_rpc_data_value) {
  const dkd_response_value = Array.isArray(dkd_rpc_data_value) ? dkd_rpc_data_value[0] : dkd_rpc_data_value;
  if (dkd_response_value?.dkd_ok_value === false || dkd_response_value?.ok === false) {
    const dkd_reason_value = dkd_string_value(dkd_response_value?.dkd_reason_value || dkd_response_value?.reason || 'restaurant_order_payment_failed');
    return { data: null, error: new Error(dkd_reason_value) };
  }
  return { data: dkd_response_value || null, error: null };
}
...
export async function dkd_create_restaurant_order_value(dkd_input_value = {}) {
  try {
    const dkd_user_key_value = await dkd_active_user_key_value(dkd_input_value?.dkd_profile_value || {});
    const dkd_location_value = dkd_location_payload_value(dkd_input_value?.dkd_current_location_value || {});
    const dkd_product_value = dkd_input_value?.dkd_product_value || {};
    const dkd_selected_category_value = dkd_input_value?.dkd_selected_category_value || {};
    const dkd_payment_preview_value = dkd_input_value?.dkd_payment_preview_value || {};
    const dkd_product_original_category_value = dkd_string_value(dkd_product_value?.category || dkd_product_value?.business_category, 'Yemek');
    const dkd_service_category_title_value = dkd_string_value(
      dkd_input_value?.dkd_service_category_title_value || dkd_selected_category_value?.dkd_title_value,
      'Restoran siparişi'
    );
    const dkd_product_price_tl_value = dkd_order_money_value(
      dkd_input_value?.dkd_product_price_tl_value
        ?? dkd_payment_preview_value?.dkd_product_price_tl
        ?? dkd_product_price_value(dkd_product_value)
    );
    const dkd_delivery_fee_tl_value = dkd_order_money_value(
      dkd_input_value?.dkd_delivery_fee_tl_value
        ?? dkd_payment_preview_value?.dkd_delivery_fee_tl
        ?? dkd_product_business_delivery_fee_value(dkd_product_value)
    );
    const dkd_customer_charge_tl_value = dkd_order_money_value(
      dkd_input_value?.dkd_customer_charge_tl_value
        ?? dkd_payment_preview_value?.dkd_customer_charge_tl
        ?? (dkd_product_price_tl_value + dkd_delivery_fee_tl_value)
    );
    const dkd_payment_payload_value = dkd_json_value({
      dkd_payment_preview_value,
      dkd_payment_source_value: dkd_input_value?.dkd_use_wallet_payment_value ? 'wallet' : 'none',
      dkd_product_price_tl_value,
      dkd_delivery_fee_tl_value,
      dkd_customer_charge_tl_value,
    });

    if (dkd_input_value?.dkd_use_wallet_payment_value) {
      const dkd_rpc_result_value = await supabase.rpc('dkd_service_network_restaurant_wallet_pay_order_dkd', {
        dkd_param_user_id: dkd_user_key_value,
        dkd_param_business_id: dkd_string_value(dkd_product_value?.business_id),
        dkd_param_business_name: dkd_string_value(dkd_product_value?.business_name, 'İşletme'),
        dkd_param_business_address_text: dkd_string_value(dkd_product_value?.business_address_text),
        dkd_param_product_id: dkd_string_value(dkd_product_value?.id),
        dkd_param_product_title: dkd_string_value(dkd_product_value?.title, 'Ürün'),
        dkd_param_product_description: dkd_string_value(dkd_product_value?.description),
        dkd_param_product_image_url: dkd_string_value(dkd_product_value?.image_url || dkd_product_value?.product_image_url),
        dkd_param_product_category: dkd_service_category_title_value,
        dkd_param_product_price_tl: dkd_product_price_tl_value,
        dkd_param_currency_code: dkd_string_value(dkd_product_value?.product_price_currency || dkd_product_value?.currency_code, 'TRY'),
        dkd_param_delivery_fee_tl: dkd_delivery_fee_tl_value,
        dkd_param_customer_charge_tl: dkd_customer_charge_tl_value,
        dkd_param_delivery_address_text: dkd_string_value(dkd_input_value?.dkd_delivery_address_text_value),
        dkd_param_delivery_note: dkd_string_value(dkd_input_value?.dkd_delivery_note_value),
        dkd_param_lat: dkd_location_value.dkd_lat,
        dkd_param_lng: dkd_location_value.dkd_lng,
        dkd_param_payload_json: dkd_json_value({
          dkd_product_value,
          dkd_selected_category_value,
          dkd_service_category_title: dkd_service_category_title_value,
          dkd_product_original_category: dkd_product_original_category_value,
          dkd_payment_payload_value,
        }),
        dkd_param_payment_payload_json: dkd_payment_payload_value,
      });
      if (dkd_rpc_result_value?.error) {
        if (dkd_missing_restaurant_payment_rpc_value(dkd_rpc_result_value.error)) {
          return { data: null, error: new Error('Restoran cüzdan ödeme RPC bulunamadı. Güncel Supabase SQL dosyasını Supabase SQL Editor içinde çalıştırmalısın.') };
        }
        return dkd_rpc_result_value;
      }
      return dkd_paid_restaurant_response_value(dkd_rpc_result_value?.data);
    }

    const dkd_payload_value = {
      dkd_user_id: dkd_user_key_value,
      dkd_business_id: dkd_string_value(dkd_product_value?.business_id),
      dkd_business_name: dkd_string_value(dkd_product_value?.business_name, 'İşletme'),
      dkd_business_address_text: dkd_string_value(dkd_product_value?.business_address_text),
      dkd_product_id: dkd_string_value(dkd_product_value?.id),
      dkd_product_title: dkd_string_value(dkd_product_value?.title, 'Ürün'),
      dkd_product_description: dkd_string_value(dkd_product_value?.description),
      dkd_product_image_url: dkd_string_value(dkd_product_value?.image_url || dkd_product_value?.product_image_url),
      dkd_product_category: dkd_service_category_title_value,
    ...
    const dkd_result_value = await supabase
      .from('dkd_service_network_restaurant_orders')
      .insert(dkd_payload_value)
      .select('*')
      .maybeSingle();
