...
import { resolveUnifiedWalletTl } from './walletService';
...
export async function fetchProfile(userId) {
  ...
  const selV23 = 'user_id, dbg_id, social_last_seen_at, nickname, avatar_emoji, dkd_puan, shards, boss_tickets, energy, energy_max, energy_updated_at, task_state, boss_state, weekly_task_state, xp, level, rank_key, courier_status, courier_score, courier_completed_jobs';
  const selV22 = 'user_id, nickname, avatar_emoji, dkd_puan, shards, boss_tickets, energy, energy_max, energy_updated_at, task_state, boss_state, weekly_task_state, xp, level, rank_key, courier_status, courier_score, courier_completed_jobs';
  const selV21 = 'user_id, nickname, avatar_emoji, dkd_puan, shards, boss_tickets, energy, energy_max, energy_updated_at, task_state, boss_state, weekly_task_state, xp, level, rank_key';
  const selV20 = 'user_id, nickname, avatar_emoji, dkd_puan, shards, boss_tickets, energy, energy_max, energy_updated_at, task_state, boss_state, weekly_task_state';
  const selV18 = 'user_id, nickname, avatar_emoji, dkd_puan, shards, energy, energy_max, energy_updated_at, task_state, boss_state, weekly_task_state';
  const selV17 = 'user_id, nickname, avatar_emoji, dkd_puan, energy, energy_max, energy_updated_at, task_state, boss_state, weekly_task_state';
  const selV16 = 'user_id, dkd_puan, energy, energy_max, energy_updated_at, task_state, boss_state, weekly_task_state';
  const selNew = 'user_id, dkd_puan, energy, energy_max, energy_updated_at, task_state, boss_state';
  const selOld = 'user_id, dkd_puan, energy, energy_max, energy_updated_at';
  ...
  if (error) {
    ...
    msg = String(error?.message || error?.details || '');
    if (msg.includes('wallet_tl') || msg.includes('merchant_wallet_tl') || msg.includes('courier_city') || msg.includes('courier_zone') || msg.includes('courier_vehicle_type') || msg.includes('courier_profile_meta')) {
      resp = await supabase
        .from('dkd_profiles')
        .select(selV24)
        .eq('user_id', userId)
  ...
  const normalized = {
    ...
    merchant_wallet_tl: Number(data?.merchant_wallet_tl ?? 0),
