...
function dkd_mapbox_feature_score_value(dkd_feature_value, dkd_query_value) {
  ...
  return dkd_match_count_value * 45 + dkd_important_match_value * 100 + dkd_type_bonus_value - dkd_missing_important_penalty_value - dkd_mismatch_penalty_value;
}
...
export async function dkd_panel_geocode_delivery_address_value(dkd_address_value, dkd_options_value = {}) {
  ...
  try {
    const { data: dkd_verified_value } = await supabase.rpc('dkd_delivery_place_resolve_dkd', {
      ...
      dkd_param_city: dkd_context_city_value || null,
    });
  ...
  const dkd_query_with_context_value = /\b(türkiye|turkiye)\b/i.test(dkd_query_value)
    ? dkd_query_value
  ...
  if (!dkd_named_place_query_value(dkd_query_value) && dkd_proximity_value?.dkd_coordinate_value && dkd_context_city_value) {
...
export function dkd_panel_live_route_progress_value(dkd_route_value, dkd_current_point_value) {
  ...
  const dkd_ratio_value = dkd_initial_distance_value > 0 ? Math.min(1.25, Math.max(0, dkd_remaining_km_value / dkd_initial_distance_value)) : 1;
