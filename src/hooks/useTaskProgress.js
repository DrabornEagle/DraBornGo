...
export function useTaskProgress({ sessionUserId, profile, setProfile, userCardsRaw, collectionCards, grantXp, onAchievementAction }) {
  ...
  const applyReward = useCallback(async (dkd_puan_delta_value, energyDelta) => {
    if (!sessionUserId) return;

    const dkd_next_puan_value = Number(profile?.dkd_puan || 0) + Number(dkd_puan_delta_value || 0);
    ...
    const { error } = await supabase
      .from('dkd_profiles')
      .update({ dkd_puan: dkd_next_puan_value, energy: nextEnergy, energy_updated_at: nowIso })
      .eq('user_id', sessionUserId);

    if (error) throw error;
    setProfile((prev) => (prev
      ? { ...prev, dkd_puan: dkd_next_puan_value, puan: dkd_next_puan_value, energy: nextEnergy, energy_updated_at: nowIso }
      : prev));
  }, [sessionUserId, profile?.dkd_puan, profile?.energy, profile?.energy_max, setProfile]);
  ...
  const claimTask = useCallback(async (key) => {
    ...
    if (isWeekly) {
      ...
      try {
        ...
        setProfile((prev) => (prev ? {
          ...prev,
          dkd_puan: data.dkd_puan != null ? data.dkd_puan : prev.dkd_puan,
          puan: data.dkd_puan != null ? data.dkd_puan : prev.puan,
      ...
      try {
        await applyReward(weeklyDef.reward_puan || 0, weeklyDef.reward_energy || 0);
        if (Number(weeklyDef.reward_xp || 0) > 0) {
          await grantXp?.(Number(weeklyDef.reward_xp), `task_${dkd_key_value}`);
        }
        await onAchievementAction?.('taskClaim', 1);
    ...
    try {
      await applyReward(dailyDef.reward_puan || 0, dailyDef.reward_energy || 0);
      if (Number(dailyDef.reward_xp || 0) > 0) {
        await grantXp?.(Number(dailyDef.reward_xp), `task_${dkd_key_value}`);
      }
      await onAchievementAction?.('taskClaim', 1);
