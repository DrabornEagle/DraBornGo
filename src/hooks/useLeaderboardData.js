...
export function useLeaderboardData({ sessionUserId, isAdmin, refreshProfile }) {
  ...
  const preloadLeaderboard = useCallback(async (metric = 'courier', weekOffset = 0) => {
    ...
    try {
      ...
      console.log('[DraBornGo][leaderboard][preload]', dkd_error_value?.message || String(dkd_error_value));
      return null;
    }
  ...
  const claimWeeklyTopReward = useCallback(async (metric = 'dkd_puan') => {
    ...
    try {
      ...
      if (data?.ok) {
        clearLeaderboardCache();
        const dkd_reward_puan_value = Number(data?.reward_puan || 0);
        const dkd_reward_energy_value = Number(data?.reward_energy || 0);
        Alert.alert('Ödül', `Tebrikler! Rank #${data?.rank}\n+${dkd_reward_puan_value} puan${dkd_reward_energy_value ? ` • +${dkd_reward_energy_value} enerji` : ''}`);
  ...
  const checkCourierWeeklyRewardPopup = useCallback(async () => {
    ...
    try {
      ...
      const dkd_reward_card_name_value = String(data?.reward_card_name || 'Sürpriz Koleksiyon Kartı');
      ...
      Alert.alert(
        ...
        `Geçen hafta kurye modunda #${dkd_rank_value} oldun.\n\nKazandıkların:\n• ${dkd_reward_card_label_value}\n• ${dkd_reward_puan_value} puan\n• ${dkd_reward_shards_value} koleksiyon puanı\n• ${dkd_reward_boss_tickets_value} özel kart`,
      );

      return data;
    } catch (dkd_error_value) {
      console.log('[DraBornGo][courier_reward_popup]', dkd_error_value?.message || String(dkd_error_value));
      return null;
    } finally {
