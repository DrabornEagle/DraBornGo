...
async function buildRewardPayload(result, dropType, ticketReward = 0, ticketTotal = 0) {
  const base = {
    ok: true,
    dkd_puan: Number(result?.dkd_puan || 0),
    dkd_puan_mult: result?.dkd_puan_mult || null,
    drop_type: dropType,
  ...
  if (cardError || !card) {
    return {
      ...
      card_series: result?.card_series || 'DraBornGo',
...
export function useChestActions({
  ...
  const syncProfileFromOpen = useCallback((result, fallbackEnergyCost = 1) => {
    ...
    setProfile((prev) => {
      ...
      const dkd_next_puan_delta_value = readNumeric(result?.dkd_puan_delta, result?.dkd_puan, 0) || 0;
      const dkd_next_puan_absolute_value = readNumeric(result?.dkd_puan_after, result?.dkd_puan_balance, result?.dkd_puan_total);
      ...
      return {
        ...prev,
        dkd_puan: dkd_next_puan_absolute_value != null ? dkd_next_puan_absolute_value : Math.max(0, Number(prev.dkd_puan || 0) + dkd_next_puan_delta_value),
        puan: dkd_next_puan_absolute_value != null ? dkd_next_puan_absolute_value : Math.max(0, Number(prev.dkd_puan || 0) + dkd_next_puan_delta_value),
  ...
  const attachCampaignCoupon = useCallback(async ({ dropId, result, sourceType, qrText = null, codeText = null }) => {
    ...
    try {
      ...
      const coupon = await claimPlayerCampaignCouponByDrop({
        ...
        sourceType,
        meta: {
          reward_dkd_puan: Number(result?.dkd_puan || 0),
  ...
  const bossOpenChestNow = useCallback(async () => {
    ...
    if (!drop) {
      Alert.alert('Özel Hedef', 'Ödül noktası bulunamadı. Haritayı yenile.');
      return;
    }
  ...
  const openChestByQr = useCallback(async (qrText) => {
    ...
    if (!parsed) {
      Alert.alert('QR Hatası', 'QR formatı yanlış. Örn: id=UUID&secret=YOUR_SECRET');
      return;
    }
...
Okunan: ${parsed.dropId}`);
    ...
    if (!drop) {
      Alert.alert('Ödül Noktası Bulunamadı', `Nokta ID: ${parsed.dropId} bulunamadı.`);
      return;
    }
