...
export function useHistoryData({ sessionUserId } = {}) {
  ...
  const mapHistoryRows = useCallback((rows) => {
    return (rows || []).map((entry) => {
      ...
      const tone = card?.rarity ? rarityTone(card.rarity) : (entry.gained_puan ? 'good' : 'default');
      const badge = card?.rarity ? trRarity(card.rarity) : (entry.gained_puan ? `+${entry.gained_puan} puan` : trDropType(entry.drop_type));
      const title = card?.name ? card.name : (drop?.name || trDropType(entry.drop_type));
      return {
        id: entry.id,
        created_at: entry.created_at,
        gained_puan: entry.gained_puan,
        title,
        badge,
        tone,
        card: card ? {
    ...
    const request = (async () => {
      ...
      try {
        ...
        if (!background) {
          ...
          console.log('[DraBornGo][history][preload]', dkd_error_value?.message || String(dkd_error_value));
        }
        return [];
      } finally {
