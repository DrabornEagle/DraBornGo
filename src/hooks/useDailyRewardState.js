  ...
  return {
    ...
    banner: {
      ...
      subtitle: state?.claimedToday
        ...
        : `${Number(state?.rewardPreview?.puan || 0)} puan • ${Number(state?.rewardPreview?.xp || 0)} XP`,
...
function buildPulse({ title, subtitle, puan = 0, xp = 0, kind = 'reward' }) {
  return {
    title,
...
export function useDailyRewardState({ sessionUserId, profile, setProfile, refreshProfile, taskState, weeklyTaskState }) {
  ...
  const patchProfile = useCallback((profilePatch) => {
    ...
    setProfile((prev) => prev ? {
      ...prev,
      puan: Number(profilePatch?.puan ?? prev.puan ?? 0),
  ...
  const claimTodayReward = useCallback(async () => {
    ...
    try {
      ...
      setPulse(buildPulse({
        ...
        subtitle: `+${Number(result?.reward?.puan || 0)} puan • +${Number(result?.reward?.xp || 0)} XP`,
        puan: result?.reward?.puan,
        xp: result?.reward?.xp,
        kind: 'daily',
      }));
      setRewardModalOpen(false);
  ...
  const claimEventTask = useCallback(async (taskKey) => {
    ...
    try {
      ...
      setPulse(buildPulse({
        ...
        subtitle: `+${Number(result?.task?.reward_puan || 0)} puan • +${Number(result?.task?.reward_xp || 0)} XP`,
        puan: result?.task?.reward_puan,
  ...
  const claimStreakMilestoneReward = useCallback(async (days) => {
    ...
    try {
      ...
      setPulse(buildPulse({
        ...
        subtitle: cleanOpsText(result?.milestone?.rewardLabel, 'Puan ve XP ödülü eklendi.'),
        puan: result?.milestone?.puan,
  ...
  const trackDailyRewardAction = useCallback(async (actionKey, amount = 1) => {
    ...
    try {
      ...
      console.log('[DraBornGo][dailyReward][track]', error?.message || String(error));
      return null;
    }
  }, [progressSnapshot, sessionUserId]);
