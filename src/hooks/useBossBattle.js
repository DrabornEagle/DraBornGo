...
export function useBossBattle({
  ...
  const startBossSession = useCallback(async (mode = 'daily', dropId = null) => {
    ...
    if (mode === 'drop' && dropId && escapedDropIds.includes(String(dropId))) {
      Alert.alert('Özel Hedef Kapandı', `Bu özel hedef bugün kapandı. Tekrar geliş: ${nextBossReturnText()}`);
      return;
    }

    if (mode === 'drop' && dropId && solvedDropIds.includes(String(dropId))) {
      Alert.alert('Özel Hedef Tamamlandı', `Bu özel hedef bugün zaten tamamlandı. Tekrar geliş: ${nextBossReturnText()}`);
      return;
    }
    ...
    if (mode === 'drop' && dropId) {
      ...
      if (currentTickets < ticketCost) {
        Alert.alert('Özel Kart', `Bu özel hedef için ${ticketCost} özel kart gerekiyor. Mevcut: ${currentTickets}`);
        return;
      }
      const nextTickets = Math.max(0, currentTickets - ticketCost);
      next.ticket_cost = ticketCost;
      next.boss_name = String(data?.title || 'Özel Hedef');
  ...
  const openBoss = useCallback(() => {
    ...
    if (dailySolved) {
      Alert.alert('Özel Hedef Tamamlandı', `Günlük özel hedef bugün kapandı. Tekrar geliş: ${nextBossReturnText()}`);
      return;
    }
