...
function makeItems(defs, mode, daily, weekly, dailyClaims, weeklyClaims) {
  return defs.map((task) => {
    ...
    return {
      ...
      rewardText: `+${formatNum(task.reward_puan || 0)} puan${Number(task.reward_energy || 0) > 0 ? ` • +${formatNum(task.reward_energy)} enerji` : ''}${Number(task.reward_xp || 0) > 0 ? ` • +${formatNum(task.reward_xp)} XP` : ''}`,
    };
  });
}
          ...
          <ScrollView contentContainerStyle={styles.content}>
            ...
            <View style={styles.panelCard}>
              ...
              <View style={styles.infoChipRow}>
                ...
                {mode === 'daily'
                  ? <SkylineChip label={daily?.boss_solved ? 'Özel hedef çözüldü' : 'Özel hedef bekliyor'} tone="gold" />
