...
function BossIntroModal({ visible, payload, onClose, onStart }) {
  ...
  const rawBossName = String(payload?.title || bossMeta?.title || bossMeta?.boss_name || 'Özel Hedef');
  const bossSub = String(payload?.subtitle || bossMeta?.subtitle || bossMeta?.boss_subtitle || 'Şehir hedefi hazır. Soruları çöz, hedefi tamamla ve ödül akışına geç.');
  const bossDesc = String(payload?.description || bossMeta?.description || bossMeta?.boss_description || 'Sabit kurallı hedef sistemi açık. Doğru cevap verdikçe hedef ilerlemesi tamamlanır.');
  ...
  const rewardSummary = String(payload?.reward_summary || bossMeta?.reward_summary || 'Puan • Koleksiyon Kartı • Özel Kart');
  ...
  const bossName = useMemo(() => {
    ...
    if (clean === 'baskın başlat') {
      return String(bossMeta?.title || bossMeta?.boss_name || payload?.dropName || 'Özel Hedef');
    }
    return rawBossName;
      ...
      <View style={dkd_styles.backdrop}>
        ...
        <View style={dkd_styles.card}>
          ...
          <View style={dkd_styles.headerRow}>
            <View style={{ flex: 1 }}>
              <Text style={dkd_styles.kicker}>{isDrop ? 'DROP HEDEF // CANLI İLERLEME' : 'GÜNLÜK HEDEF // CANLI İLERLEME'}</Text>
            </View>
          ...
          <View style={dkd_styles.statRow}>
            ...
            <StatCard icon="cards-playing-diamond-outline" label="Özel Kart" value={ticketText} tone="gold" />
            <StatCard icon="heart-pulse" label="Hedef" value={hpText} tone="purple" />
          </View>
          ...
          <View style={dkd_styles.rewardRow}>
            {rewardParts.length ? rewardParts.map((label, index) => (
              <RewardPill
                key={`${label}-${index}`}
                icon={index === 0 ? 'star-four-points-circle-outline' : index === 1 ? 'cards-outline' : index === 2 ? 'cards-outline' : 'cards-playing-diamond-outline'}
                label={label}
                tone={index === 0 ? 'gold' : index === 2 ? 'purple' : 'cyan'}
              />
            )) : (
              <>
                <RewardPill icon="star-four-points-circle-outline" label="Puan" tone="gold" />
                <RewardPill icon="cards-outline" label="Koleksiyon" tone="cyan" />
          ...
          <View style={dkd_styles.actionWrap}>
            <PrimaryButton label="Hedefi Başlat" icon="sword-cross" onPress={onStart} />
