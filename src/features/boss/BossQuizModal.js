...
export default function BossQuizModal({ visible, onClose, bossState, onAnswer, onTryAgain, onOpenChest }) {
  ...
  const bossName = String(bossState?.boss_name || bossMeta?.title || bossMeta?.boss_name || bossState?.name || 'Özel Hedef');
  const bossSub = String(bossState?.boss_subtitle || bossMeta?.subtitle || 'Hedef çekirdeği aktif. Soruları çöz ve hedefi tamamla.');
  const bossArt = String(bossState?.boss_art_image_url || bossMeta?.art_image_url || bossMeta?.boss_art_url || '');
  const rewardSummary = String(bossState?.reward_summary || bossMeta?.reward_summary || 'Puan • Koleksiyon Kartı • Özel Kart');
        ...
        <LinearGradient colors={['#040811', '#08111A', '#070E18']} style={{ flex: 1 }}>
          <ScrollView bounces={false} showsVerticalScrollIndicator={false} contentContainerStyle={dkd_styles.scrollContent}>
            <View style={dkd_styles.headerRow}>
              <View style={{ flex: 1 }}>
                <Text style={dkd_styles.kicker}>{isDrop ? 'HEDEF CORE // DROP' : 'HEDEF CORE // GÜNLÜK'}</Text>
                <Text style={dkd_styles.title}>Özel hedef akışı</Text>
                <Text style={dkd_styles.sub}>Soruları çöz, doğru cevaplarla hedef ilerlemesini tamamla.</Text>
              </View>
            ...
            <View style={dkd_styles.heroCard}>
              <View style={dkd_styles.heroMedia}>
                ...
                <View style={dkd_styles.mediaBadge}><Text style={dkd_styles.mediaBadgeText}>{isDrop ? 'DROP HEDEF' : 'GÜNLÜK HEDEF'}</Text></View>
              </View>
              ...
              <View style={dkd_styles.heroInfoCard}>
                ...
                <CombatBar label="Hedef" value={bossHp} max={bossHpMax} colors={['#FFCA7A', '#FF7A1D']} icon="robot-angry-outline" tone="gold" />
            ...
            {!finished ? (
                ...
                {phase === 'result' ? (
                  <View style={dkd_styles.resultCard}>
                    <Text style={[dkd_styles.resultText, lastCorrect ? dkd_styles.resultOk : dkd_styles.resultBad]}>
                      {lastCorrect ? 'Doğru cevap. Hedef hattı açıldı, bir sonraki kilide geçebilirsin.' : 'Yanlış cevap. Kişisel zırhın hasar aldı, dikkatli ilerle.'}
                    </Text>
              ...
              <View style={dkd_styles.finishCard}>
                ...
                <Text style={dkd_styles.finishKicker}>{victory ? 'HEDEF TAMAM' : escaped ? 'HEDEF DURDU' : 'YENİDEN BAŞLAT'}</Text>
                <Text style={dkd_styles.finishTitle}>{victory ? 'Hedef tamamlandı' : escaped ? 'Bugünlük pencere kapandı' : 'Yeniden saldırı hazırlığı'}</Text>
                <Text style={dkd_styles.finishSub}>{victory ? 'Zafer sonrası ödül çekirdeği hazır. İstersen şimdi ganimete geç.' : escaped ? `Tekrar geliş: ${returnText}` : 'Akışı sıfırlayıp aynı hedef için tekrar deneyebilirsin.'}</Text>
                <View style={dkd_styles.finishRewardRow}>
                  {rewardSummary.split('•').map((item, index) => {
                    const label = String(item || '').trim();
                    if (!label) return null;
                    return <RewardChip key={`${label}-${index}`} icon={index === 0 ? 'star-four-points-circle-outline' : index === 1 ? 'cards-outline' : index === 2 ? 'cards-outline' : 'cards-playing-diamond-outline'} label={label} tone={index === 0 ? 'gold' : index === 2 ? 'purple' : 'cyan'} />;
                  })}
                </View>
