...
const FILTERS = [
  ...
  { key: 'boss', label: 'Özel Hedef', icon: 'crown-outline' },
...
function ActionButton(dkd_action_button_props) {
  const dkd_label_text = dkd_action_button_props.label;
  const dkd_on_press_callback = dkd_action_button_props.onPress;
  const dkd_primary_flag = dkd_action_button_props.primary === true;
  const dkd_disabled_flag = dkd_action_button_props.disabled === true;
  const dkd_icon_name = dkd_action_button_props.icon || 'arrow-right';
  const dkd_emphasized_flag = dkd_action_button_props.dkd_is_emphasized === true;
  const dkd_attention_progress_value = useRef(new Animated.Value(0)).current;

  useEffect(function () {
    if (!dkd_emphasized_flag || dkd_disabled_flag) {
      dkd_attention_progress_value.stopAnimation();
      dkd_attention_progress_value.setValue(0);
      return undefined;
    }

    const dkd_attention_loop_animation = Animated.loop(
      Animated.sequence([
        Animated.timing(dkd_attention_progress_value, {
          toValue: 1,
          duration: 1600,
    ...
    dkd_attention_loop_animation.start();

    return function dkd_stop_attention_loop() {
      dkd_attention_loop_animation.stop();
    };
  }, [dkd_attention_progress_value, dkd_disabled_flag, dkd_emphasized_flag]);

  const dkd_icon_color_value = dkd_primary_flag ? '#FFFFFF' : dkd_emphasized_flag ? '#07101A' : theme.colors.text;
  const dkd_text_style_value = dkd_primary_flag
    ? dkd_styles.primaryBtnText
    : dkd_emphasized_flag
      ? dkd_styles.emphasisBtnText
      : dkd_styles.ghostBtnText;
  const dkd_emphasis_scale_style = {
    transform: [
      {
        scale: dkd_attention_progress_value.interpolate({
          inputRange: [0, 0.5, 1],
  ...
  const dkd_emphasis_sweep_style = {
    transform: [
      {
        translateX: dkd_attention_progress_value.interpolate({
          inputRange: [0, 1],
          outputRange: [-92, 128],
        }),
      },
  ...
  const dkd_content_node = (
    <>
      <MaterialCommunityIcons name={dkd_icon_name} size={17} color={dkd_icon_color_value} />
      <Text style={dkd_text_style_value}>{dkd_label_text}</Text>
    </>
  );
  ...
  return (
    <Pressable
      onPress={function () {
        if (dkd_disabled_flag) return;
        tapFeedback();
        dkd_on_press_callback?.();
      }}
      disabled={dkd_disabled_flag}
      style={function (dkd_press_state) {
        return [
          dkd_styles.actionPressable,
          dkd_disabled_flag && dkd_styles.actionDisabled,
          dkd_press_state.pressed && !dkd_disabled_flag && dkd_styles.actionPressed,
        ];
      }}
    >
      {dkd_primary_flag ? (
        ...
        <Animated.View style={[dkd_styles.emphasisBtnWrap, dkd_emphasis_scale_style]}>
          <LinearGradient colors={['#FFE177', '#49E7FF', '#7B7CFF']} start={dkd_make_native_axis_point(0, 0)} end={dkd_make_native_axis_point(1, 1)} style={dkd_styles.emphasisBtn}>
            <Animated.View pointerEvents="none" style={[dkd_styles.emphasisSweep, dkd_emphasis_sweep_style]}>
              <LinearGradient
        ...
        <View style={dkd_styles.ghostBtn}>{dkd_content_node}</View>
      )}
    </Pressable>
  );
}
...
function buildReadyText(drop, cooldown) {
  ...
  if (String(drop?.type || '').toLowerCase() === 'boss') return 'Özel hedef hazır';
    ...
    <View style={dkd_styles.rowCard}>
      ...
      <View style={dkd_styles.actionRow}>
        <View style={[dkd_styles.actionSlot, dkd_styles.actionSlotGap]}>
          <ActionButton label="Konuma Git" icon="map-marker-outline" dkd_is_emphasized onPress={function () { onNavigate?.(drop); }} />
        </View>
...
function DropListModal({ visible, onClose, drops, getCooldown, isNear, onNavigate, onOpen, onLocate }) {
    ...
    return (
      <>
        ...
        <View style={dkd_styles.summaryRow}>
          ...
          <SummaryChip icon="crown-outline" label="Özel Hedef" value={String(bossCount)} tone="gold" />
        </View>
  ...
  }, [bossCount, filter, filterCounts, filtered.length, nearCount, onClose, onLocate, readyCount, rewardCount]);
...
const dkd_styles = StyleSheet.create({
  ...
  emphasisBtnText: { color: '#07101A', fontSize: 13, fontWeight: '900', marginLeft: 8 },
