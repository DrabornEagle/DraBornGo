...
function HeaderOverlay({ profile, energyUI, puanScale, puanText, walletText, dailyRewardState, onOpenDailyReward, onOpenTasks, compact = false }) {
  const prog = useMemo(() => normalizeProgression(profile || {}), [profile]);
  ...
  const tokenAnimatedStyle = useMemo(() => ({ transform: [{ scale: puanScale || 1 }] }), [puanScale]);
      ...
      >
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          <View style={{ flex: 1, marginLeft: 12, paddingRight: 10 }}>
            <Text style={{ color: ui.colors.cyan, fontSize: 10, fontWeight: '900', letterSpacing: 0.8 }}>DraBornGo</Text>
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginTop: 12 }}>
          ...
          <MetricChip icon="star-four-points-circle-outline" label="PUAN" value={puanText || '0'} animatedStyle={tokenAnimatedStyle} />
          <MetricChip icon="cards-playing-diamond-outline" label="ÖZEL KART" value={dkd_boss_card_text_value} />
        </View>
...
function areHeaderOverlayPropsEqual(prev, next) {
  return prev.puanScale === next.puanScale
    && prev.puanText === next.puanText
    ...
    && Number(prev?.profile?.boss_tickets || 0) === Number(next?.profile?.boss_tickets || 0)
