...
function toneForLog(item) {
  ...
  if (Number(item?.gained_puan) > 0) return 'good';
  return 'default';
}
...
function EmptyState() {
  return (
    <View style={styles.emptyWrap}>
      ...
      <Text style={styles.emptySub}>Sandık açınca, puan kazanınca ve kart düşünce bu ekran dolacak.</Text>
    </View>
  );
}
  ...
  return (
    <SectionCard style={styles.rowCardWrap}>
      <View style={styles.rowTop}>
        <View style={styles.rowCopy}>
          ...
          <Text style={styles.rowSub}>
            ...
            {Number(item?.gained_puan) > 0 ? ` • +${Number(item.gained_puan)} puan` : ''}
          </Text>
        </View>
      ...
      <View style={styles.pillsRow}>
        ...
        {Number(item?.gained_puan) > 0 ? <Pill text={`+${Number(item.gained_puan)} PUAN`} tone="good" /> : null}
      </View>
    </SectionCard>
  );
});
...
export default function HistoryModal({ visible, onClose, logs, loading, onRefresh, onOpenCard }) {
  ...
  const stats = useMemo(() => {
    ...
    const puans = safeLogs.reduce((sum, item) => sum + Number(item?.gained_puan || 0), 0);
    return { total, cards, puans };
  }, [safeLogs]);
      ...
      <SafeScreen style={styles.screen}>
        ...
        <LinearGradient colors={[theme.colors.bgTop, theme.colors.bgMid, theme.colors.bgBottom]} style={styles.wrap}>
          ...
          <SectionCard>
            ...
            <Text style={styles.heroSubtitle}>Kart, puan ve sandık kayıtlarını tek panelden takip et.</Text>
            ...
            <View style={styles.statsRow}>
              ...
              <StatBox label="Puan" value={String(stats.puans)} icon="cash-multiple" accent="rgba(246,197,95,0.88)" />
            </View>
