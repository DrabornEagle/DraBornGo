...
function ToneBadge({ text, tone = 'cyan' }) {
  ...
  return (
    <View style={[styles.badge, { backgroundColor: current.bg, borderColor: current.border }]}>
      <Text style={[styles.badgeText, { color: current.color }]} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.62}>{text}</Text>
    </View>
  );
}

function dkd_account_deletion_status_label_value(dkd_status_value) {
  const dkd_status_key_value = String(dkd_status_value || '').toLowerCase();
  if (dkd_status_key_value === 'approved') return 'Silme onaylandı';
  if (dkd_status_key_value === 'rejected') return 'Talep reddedildi';
  if (dkd_status_key_value === 'pending') return 'Admin onayı bekliyor';
  return 'Talep yok';
}
...
function ProfileModal({ visible, onClose, profile, onSave, onOpenPlayerCard }) {
  ...
  const [dkd_account_deletion_request_value, dkd_set_account_deletion_request_value] = useState(null);
  const [dkd_account_deletion_loading_value, dkd_set_account_deletion_loading_value] = useState(false);
  const [dkd_account_deletion_submitting_value, dkd_set_account_deletion_submitting_value] = useState(false);
  ...
  useEffect(() => {
    ...
    const dkd_user_id_value = String(profile?.id || profile?.user_id || '').trim();
    if (!dkd_user_id_value) {
      dkd_set_logistics_application_value(null);
      return undefined;
    }
    ...
    dkd_fetch_my_logistics_application_value({
      ...
      .then((dkd_result_value) => {
        if (!dkd_cancelled_value) dkd_set_logistics_application_value(dkd_result_value?.data || null);
      })
      .catch(() => {
        if (!dkd_cancelled_value) dkd_set_logistics_application_value(null);
      });
    return () => {
  ...
  useEffect(() => {
    if (!visible) return undefined;
    const dkd_user_id_value = String(profile?.user_id || profile?.id || '').trim();
    if (!dkd_user_id_value) {
      dkd_set_account_deletion_request_value(null);
      return undefined;
    }
    let dkd_cancelled_value = false;
    dkd_set_account_deletion_loading_value(true);
    dkd_fetch_my_account_deletion_request_value(dkd_user_id_value)
      .then((dkd_result_value) => {
        if (!dkd_cancelled_value) dkd_set_account_deletion_request_value(dkd_result_value?.dkd_data_value || null);
      })
      .catch(() => {
        if (!dkd_cancelled_value) dkd_set_account_deletion_request_value(null);
      })
      .finally(() => {
        if (!dkd_cancelled_value) dkd_set_account_deletion_loading_value(false);
      });
    return () => {
      dkd_cancelled_value = true;
    };
  ...
  const dkd_logistics_status_key_value = String(
    profileView?.dkd_logistics_status
    || profileView?.logistics_status
    || profileView?.dkd_transporter_status
    || profileView?.transporter_status
    || profileView?.dkd_nakliye_status
    || profileView?.nakliye_status
    || dkd_logistics_application_value?.dkd_status
    || dkd_logistics_application_value?.status
    || '',
  ).toLowerCase();
  const dkd_logistics_license_active_value = dkd_is_logistics_approved_value(profileView, dkd_logistics_application_value)
    || ['approved', 'onayli', 'onaylı'].includes(dkd_logistics_status_key_value);
  ...
  const dkd_identity_subtitle_text = `${prog.rankLabel} Özel Hedef • Lvl ${prog.level}`;
  ...
  const dkd_submit_account_deletion_request_handler_value = useCallback(() => {
    const dkd_user_id_value = String(profile?.user_id || profile?.id || profile?.dkd_user_id_value || '').trim();
    if (!dkd_user_id_value || dkd_account_deletion_submitting_value) return;
    Alert.alert(
      'Hesabımı Sil',
      'Bu talep Admin başvurularına düşer. Admin onayladığında kullanıcıya bağlı veriler, yasal saklama zorunluluğu bulunan sınırlı kayıtlar hariç silme akışına alınır. Talep oluşturulsun mu?',
      [
        { text: 'Vazgeç', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              dkd_set_account_deletion_submitting_value(true);
              ...
              const dkd_refresh_result_value = await dkd_fetch_my_account_deletion_request_value(dkd_user_id_value);
              dkd_set_account_deletion_request_value(dkd_refresh_result_value?.dkd_data_value || { dkd_status_value: 'pending' });
              Alert.alert('Hesabımı Sil', 'Talebiniz gün içerisinde incelenecektir. Onaylandığında yasal saklama zorunluluğu bulunan sınırlı kayıtlar hariç silme akışına alınacaktır.');
            } catch (dkd_error_value) {
              Alert.alert('Hesabımı Sil', dkd_error_value?.message || 'Talep oluşturulamadı. Supabase SQL dosyasını çalıştırdığından emin ol.');
            } finally {
              dkd_set_account_deletion_submitting_value(false);
            }
          },
        },
      ],
    );
  }, [dkd_account_deletion_submitting_value, nick, profile]);

  const dkd_cancel_account_deletion_request_handler_value = useCallback(() => {
    const dkd_user_id_value = String(profile?.user_id || profile?.id || profile?.dkd_user_id_value || '').trim();
    if (!dkd_user_id_value || dkd_account_deletion_submitting_value) return;
    Alert.alert(
      'Talebi İptal Et',
      'Bekleyen hesap silme talebin iptal edilecek. Devam edilsin mi?',
      [
        { text: 'Vazgeç', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              if (dkd_result_value?.dkd_error_value) throw dkd_result_value.dkd_error_value;
              dkd_set_account_deletion_request_value(null);
              Alert.alert('Talep iptal edildi', 'Hesap silme talebin iptal edildi.');
            } catch (dkd_error_value) {
              Alert.alert('Talep iptal edilemedi', dkd_error_value?.message || 'Tekrar dene.');
            } finally {
              dkd_set_account_deletion_submitting_value(false);
            }
          },
  ...
  }, [dkd_account_deletion_submitting_value, profile]);

  const dkd_account_deletion_status_text_value = dkd_account_deletion_status_label_value(dkd_account_deletion_request_value?.dkd_status_value);
      ...
      <SafeScreen style={styles.screen}>
        ...
        <LinearGradient colors={[theme.colors.bgTop, theme.colors.bgMid, theme.colors.bgBottom]} style={styles.wrap}>
          ...
          <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
            <SectionCard style={styles.heroCard}>
              ...
              <View style={styles.heroTop}>
                ...
                <View style={styles.heroCopy}>
                  <Text style={styles.heroEyebrow}>KULLANICI KİMLİĞİ</Text>
              ...
              <View style={styles.heroPillRow}>
                ...
                {dkd_logistics_license_active_value ? <ToneBadge text="Nakliyeci Lisanslı" tone="green" /> : null}
              </View>
            ...
            <SectionCard tone="red" style={styles.dkdAccountDeletionCard}>
              <View style={styles.sectionHeader}>
                <View style={styles.dkdAccountDeletionCopy}>
                  <Text style={styles.sectionTitle}>Hesabımı Sil</Text>
                  <Text style={styles.sectionSub}>Hesap ve veri silme talebini Admin başvurularına gönder.</Text>
                </View>
                <ToneBadge text={dkd_account_deletion_status_text_value} tone={dkd_account_deletion_request_value?.dkd_status_value === 'pending' ? 'gold' : dkd_account_deletion_request_value?.dkd_status_value === 'rejected' ? 'red' : 'cyan'} />
              </View>
              <View style={styles.dkdAccountDeletionNotice}>
                <MaterialCommunityIcons name="account-remove-outline" size={22} color="#FFD6D6" />
                <Text style={styles.dkdAccountDeletionNoticeText}>Talebiniz gün içerisinde incelenecektir. Onaylandığında kullanıcıya bağlı profil, cüzdan, sipariş, başvuru ve uygulama kayıtları; yasal saklama zorunluluğu bulunan sınırlı kayıtlar hariç silme akışına alınacaktır.</Text>
              </View>
              <View style={styles.singleActionRow}>
                <ActionButton
                  label={dkd_account_deletion_submitting_value ? 'Gönderiliyor…' : dkd_account_deletion_loading_value ? 'Kontrol ediliyor…' : dkd_account_deletion_request_value?.dkd_status_value === 'pending' ? 'Talep Admin Onayında' : 'Silme Talebi Oluştur'}
                  icon="account-cancel-outline"
                  onPress={dkd_submit_account_deletion_request_handler_value}
                  disabled={dkd_account_deletion_loading_value || dkd_account_deletion_submitting_value || dkd_account_deletion_request_value?.dkd_status_value === 'pending'}
                />
              </View>
              {dkd_account_deletion_request_value?.dkd_status_value === 'pending' ? (
                <View style={styles.singleActionRow}>
                  <ActionButton
                    label={dkd_account_deletion_submitting_value ? 'İptal ediliyor…' : 'Talebi İptal Et'}
                    icon="close-circle-outline"
                    onPress={dkd_cancel_account_deletion_request_handler_value}
                    disabled={dkd_account_deletion_loading_value || dkd_account_deletion_submitting_value}
                    secondary
                  />
                </View>
              ) : null}
...
const styles = StyleSheet.create({
  ...
  heroPillRow: { flexDirection: 'row', gap: 5, flexWrap: 'nowrap', marginTop: 14, alignItems: 'stretch' },
  ...
  dkdAccountDeletionCard: {
    borderColor: 'rgba(255,139,139,0.20)',
  },
  dkdAccountDeletionCopy: { flex: 1, paddingRight: 10 },
  dkdAccountDeletionNotice: {
    marginTop: 12,
    borderRadius: 18,
    borderWidth: 1,
  ...
  dkdAccountDeletionNoticeText: { color: 'rgba(255,255,255,0.80)', flex: 1, fontSize: 12, fontWeight: '700', lineHeight: 18 },
  dkdIdentityCard: {
    marginTop: -2,
  ...
  badgeText: { fontSize: 10.2, fontWeight: '900', textAlign: 'center' },
  input: {
    marginTop: 12,
    borderRadius: 16,
    paddingHorizontal: 14,
