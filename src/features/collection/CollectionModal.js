...
import { dkd_collection_craft_options_value, dkd_collection_upgrade_options_value } from '../../constants/game';
import { buildCollectionPointSummary, computeCollectionPower } from '../../utils/collection';
...
export default function CollectionModal({
  ...
  const summary = useMemo(() => buildCollectionPointSummary(safeCards), [safeCards]);
  const wallet = Number(profile?.shards || 0);
  const dkd_boss_card_count_value = Number(profile?.boss_tickets || 0);
    ...
    <View>
      <View style={{ borderRadius: 24, backgroundColor: ui.colors.panel2, borderWidth: 1, borderColor: ui.colors.line, padding: 16, marginBottom: 12, flexDirection: 'row', alignItems: 'flex-start' }}>
        <View style={{ flex: 1, paddingRight: 12 }}>
          ...
          <Text style={{ color: ui.colors.soft, fontSize: 13, fontWeight: '700', marginTop: 8, lineHeight: 20 }}>Kart gücü, koleksiyon puanı ekonomisi ve craft hattını aynı panelde yönet.</Text>
        </View>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
        ...
        <View style={styles.heroChip}>
          <Text style={styles.heroChipText}>{formatNum(wallet)} koleksiyon puanı rezervi</Text>
        </View>
      </View>

      <View style={{ flexDirection: 'row', gap: 8, marginBottom: 8 }}>
        <StatCard icon="shield-star-outline" label="Koleksiyon gücü" value={formatNum(collectionPower)} accent={ui.colors.gold} />
        <StatCard icon="puzzle-star-outline" label="Koleksiyon Puanı" value={formatNum(wallet)} accent={ui.colors.cyan} />
      </View>
      <View style={{ flexDirection: 'row', gap: 8, marginBottom: 16 }}>
        <StatCard icon="cards-outline" label="Toplam kart" value={formatNum(totalCards)} accent={ui.colors.purple} />
        <StatCard icon="cards-playing-diamond-outline" label="Özel Kart" value={formatNum(dkd_boss_card_count_value)} accent={ui.colors.green} />
      </View>
      ...
      <View style={{ borderRadius: 22, backgroundColor: ui.colors.panel, borderWidth: 1, borderColor: ui.colors.line, padding: 14, marginBottom: 16 }}>
        <SectionHead
          ...
          side={`${formatNum(summary.potentialCollectionPoints || 0)} olası koleksiyon puanı`}
        />

        <ActionTile
          icon="recycle"
          title="Kopyaları dönüştür"
          sub="Fazla kartları otomatik temizler ve koleksiyon puanına çevirir."
          meta={`Tahmini kazanç: ${formatNum(summary.potentialCollectionPoints || 0)} koleksiyon puanı`}
          disabled={!duplicateCount || !!recycling}
          onPress={() => onRecycle?.()}
          buttonText={recycling ? 'Dönüştürülüyor…' : 'Dönüştür'}
        />
        <ActionTile
          icon="cash-multiple"
          title="40 koleksiyon puanı → 100 puan"
          sub="Harita ve market için hızlı puan dönüşümü."
          meta="Hızlı ekonomik takviye"
          disabled={!!exchangeLoading || wallet < 40}
          onPress={() => onExchange?.('puan_100')}
          accent={ui.colors.gold}
          buttonText={exchangeLoading === 'puan_100' ? 'Bekle…' : 'Puan al'}
        />
        <ActionTile
          icon="lightning-bolt"
          title="30 koleksiyon puanı → +1 enerji stoku"
        ...
        <ActionTile
          icon="sword-cross"
          title="Özel Kart üret"
          sub="Sabit kurallı koleksiyon hedefi için özel kart oluşturur."
          meta="Maliyet: 90 koleksiyon puanı"
          disabled={!!bossTicketLoading || wallet < 90}
          onPress={() => onCraftBossTicket?.()}
          accent={ui.colors.purple}
          buttonText={bossTicketLoading ? 'Üretiliyor…' : 'Özel Kart üret'}
        />
      </View>
      ...
      <View style={{ borderRadius: 22, backgroundColor: ui.colors.panel, borderWidth: 1, borderColor: ui.colors.line, padding: 14, marginBottom: 16 }}>
        <SectionHead
          kicker="FORGE"
          title="Kart üretim ve geliştirme hattı"
          sub="Doğrudan kart üret ya da alt seviye kartı yukarı taşı."
        />

        {dkd_collection_craft_options_value.map((dkd_collection_craft_option_value) => {
          const dkd_collection_craft_rarity_value = getLootRarityFrame(dkd_collection_craft_option_value.rarity);
          const dkd_collection_craft_disabled_value = !!craftLoading || wallet < dkd_collection_craft_option_value.cost;
          return (
            <ActionTile
              key={`craft-${dkd_collection_craft_option_value.key}`}
              icon="creation"
              title={`${dkd_collection_craft_rarity_value.label} üret`}
              sub={dkd_collection_craft_option_value.desc}
              meta={`Maliyet: ${formatNum(dkd_collection_craft_option_value.cost)} koleksiyon puanı`}
              disabled={dkd_collection_craft_disabled_value}
              onPress={() => onCraft?.(dkd_collection_craft_option_value.rarity)}
              accent={dkd_collection_craft_rarity_value.accent}
              buttonText={craftLoading === dkd_collection_craft_option_value.rarity ? 'Üretiliyor…' : 'Üret'}
            />
          );
        })}

        {dkd_collection_upgrade_options_value.map((dkd_collection_upgrade_option_value) => {
          const dkd_collection_upgrade_owned_value = Number(summary.countsByRarity?.[dkd_collection_upgrade_option_value.from] || 0);
          const dkd_collection_upgrade_rarity_value = getLootRarityFrame(dkd_collection_upgrade_option_value.to);
          const dkd_collection_upgrade_disabled_value = !!upgradeLoading || wallet < dkd_collection_upgrade_option_value.cost || dkd_collection_upgrade_owned_value < 1;
          return (
            <ActionTile
              key={`upgrade-${dkd_collection_upgrade_option_value.key}`}
              icon="arrow-up-bold-circle-outline"
              title={`${trRarity(dkd_collection_upgrade_option_value.from)} → ${trRarity(dkd_collection_upgrade_option_value.to)}`}
              sub={`Sende ${formatNum(dkd_collection_upgrade_owned_value)} adet ${trRarity(dkd_collection_upgrade_option_value.from).toLowerCase()} kart var.`}
              meta={`Maliyet: ${formatNum(dkd_collection_upgrade_option_value.cost)} koleksiyon puanı`}
              disabled={dkd_collection_upgrade_disabled_value}
              onPress={() => onUpgrade?.(dkd_collection_upgrade_option_value.from)}
              accent={dkd_collection_upgrade_rarity_value.accent}
              buttonText={upgradeLoading === dkd_collection_upgrade_option_value.from ? 'Yükseliyor…' : 'Yükselt'}
            />
          );
        })}
      </View>
