...
function DkdSafetyActions({ dkd_row_value, dkd_on_report_value, dkd_on_block_value }) {
  return (
    <View style={dkd_styles.inlineActionStack}>
      <Pressable
        onPress={(dkd_press_event_value) => {
          dkd_press_event_value?.stopPropagation?.();
          dkd_on_report_value?.(dkd_row_value);
        }}
        style={[dkd_styles.miniIconBtn, dkd_styles.safetyReportBtn]}
      >
        <MaterialCommunityIcons name="alert-octagon-outline" size={18} color="#FFD16C" />
      </Pressable>
      <Pressable
        onPress={(dkd_press_event_value) => {
          dkd_press_event_value?.stopPropagation?.();
          dkd_on_block_value?.(dkd_row_value);
        }}
        style={[dkd_styles.miniIconBtn, dkd_styles.safetyBlockBtn]}
      >
        <MaterialCommunityIcons name="account-cancel-outline" size={18} color="#FF8AA8" />
      </Pressable>
    </View>
  );
}

function SearchRow({ row, onAdd, onReport, onBlock }) {
  ...
  return (
    <LinearGradient colors={['rgba(18,27,45,0.98)', 'rgba(12,18,31,0.98)']} style={dkd_styles.listCard}>
      ...
      <View style={dkd_styles.rowEdge}>
        ...
        <DkdSafetyActions dkd_row_value={row} dkd_on_report_value={onReport} dkd_on_block_value={onBlock} />
        {row?.is_friend ? (
...
function RequestRow({ row, mode = 'incoming', onAccept, onReject, onReport, onBlock }) {
...
function ThreadRow({ row, onOpen, onRemove, onReport, onBlock }) {
                ...
                <LinearGradient colors={['rgba(95,235,255,0.22)', 'rgba(119,119,255,0.18)', 'rgba(255,108,203,0.14)']} style={dkd_styles.chatHeroModern}>
                  ...
                  <View style={[dkd_styles.chatHeroIdentityShell, dkd_styles.chatHeroIdentityShellCompactTop]}>
                    ...
                    <View style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <View style={dkd_styles.chatSafetyRow}>
                        <Pressable onPress={() => dkd_report_user_value(activeChat)} style={dkd_styles.chatSafetyButton}>
                          <MaterialCommunityIcons name="alert-octagon-outline" size={16} color="#FFD16C" />
                          <Text style={dkd_styles.chatSafetyButtonText}>Şikayet et</Text>
                        </Pressable>
                        <Pressable onPress={() => dkd_block_user_value(activeChat)} style={[dkd_styles.chatSafetyButton, dkd_styles.chatSafetyButtonDanger]}>
                          <MaterialCommunityIcons name="account-cancel-outline" size={16} color="#FF8AA8" />
                          <Text style={dkd_styles.chatSafetyButtonText}>Engelle</Text>
                        </Pressable>
                      </View>
                    </View>
                  </View>
                  ...
                  <>
                    ...
                    <View style={dkd_styles.panel}>
                      ...
                      <View style={dkd_styles.stackGap}>
                        {threadRows.length
                          ? threadRows.map((row) => <ThreadRow key={String(row?.user_id)} row={row} onOpen={openThread} onRemove={removeFriend} onReport={dkd_report_user_value} onBlock={dkd_block_user_value} />)
                  ...
                  <View style={dkd_styles.panel}>
                    ...
                    <View style={dkd_styles.stackGap}>
                      {searchResults.length
                        ? searchResults.map((row) => <SearchRow key={String(row?.user_id)} row={row} onAdd={handleAdd} onReport={dkd_report_user_value} onBlock={dkd_block_user_value} />)
...
const dkd_styles = StyleSheet.create({
  ...
  safetyBlockBtn: {
    backgroundColor: 'rgba(255,107,143,0.10)',
    borderColor: 'rgba(255,107,143,0.22)',
  },
  ...
  chatSafetyButtonDanger: {
    borderColor: 'rgba(255,107,143,0.24)',
    backgroundColor: 'rgba(255,107,143,0.10)',
  },
  chatSafetyButtonText: { color: '#F5FBFF', fontSize: 12, fontWeight: '900' },
  heroMetaPill: {
    minHeight: 32,
    paddingHorizontal: 12,
    borderRadius: 999,
