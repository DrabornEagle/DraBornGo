...
function formatInt(value) {
  let dkd_iteration_value = Number(value || 0);
...
function HeroStat({ icon, label, value, tone }) {
  let color = tone === 'gold' ? theme.colors.goldSoft : tone === 'purple' ? theme.colors.purple : theme.colors.cyanSoft;
  return (
...
function IdentityChip({ label, value, tone }) {
  let bg = tone === 'gold' ? 'rgba(255,209,102,0.12)' : tone === 'cyan' ? 'rgba(141,235,255,0.12)' : 'rgba(255,255,255,0.08)';
  return (
...
function CompareRow({ row }) {
  let badge = row && row.winner === 'tie' ? 'Berabere' : row && row.winner === 'local' ? 'Sen önde' : 'Dost önde';
  let badgeColor = row && row.winner === 'tie' ? 'rgba(255,255,255,0.72)' : row && row.winner === 'local' ? theme.colors.green : theme.colors.goldSoft;
  return (
...
function HistoryCard({ item, onPress }) {
  let when = item && item.comparedAt ? new Date(item.comparedAt) : null;
  let whenText = when && !Number.isNaN(when.getTime())
    ? when.toLocaleDateString('tr-TR', { day: '2-digit', month: '2-digit' })
    : 'Bugün';
  let badgeColor = item && item.overall === 'local' ? theme.colors.green : item && item.overall === 'remote' ? theme.colors.goldSoft : theme.colors.text;
  let badgeText = item && item.overall === 'local' ? 'Sen öndeydin' : item && item.overall === 'remote' ? 'Dost öndeydi' : 'Berabere';
          ...
          <ScrollView contentContainerStyle={dkd_styles.content}>
            <LinearGradient colors={['rgba(103,227,255,0.16)', 'rgba(103,227,255,0.04)']} style={dkd_styles.hero}>
              ...
              <View style={dkd_styles.heroStats}>
                ...
                <HeroStat icon="currency-usd" label="Puan" value={formatInt(localCard && localCard.puan ? localCard.puan : 0)} tone="gold" />
              </View>
            </LinearGradient>
