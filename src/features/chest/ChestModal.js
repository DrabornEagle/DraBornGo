...
import { formatInt, trRarity, trTheme } from '../../utils/text';
  ...
  return (
    <View style={dkd_styles.centerWrap}>
      ...
      <View style={dkd_styles.statusRow}>
        ...
        <StatusChip icon="star-four-points" label="Ödül Yükleniyor" tone="gold" />
        ...
        <View style={dkd_styles.rewardArtFrame}>
          {artUri ? (
            ...
            <LinearGradient colors={['#193251', '#0C1420']} style={dkd_styles.rewardFallbackArt}>
              ...
              <Text style={dkd_styles.rewardFallbackText}>Yeni ödül hazır</Text>
            </LinearGradient>
          )}
...
function ResultScene({ payload, rarity, onClose }) {
  ...
  const dkd_puan_value = Number(payload?.dkd_puan || 0);
  const dkd_collection_card_count_value = payload?.card_name ? 1 : 0;
  ...
  return (
    <ScrollView bounces={false} showsVerticalScrollIndicator={false} contentContainerStyle={dkd_styles.resultScroll}>
      <SectionTitle
        ...
        sub="Yeni ödül tamamlandı. Kart şimdi odakta, detaylar altta ve kazanım net görünür."
      />
      ...
      <View style={dkd_styles.summaryPanel}>
        ...
        <View style={dkd_styles.summaryGrid}>
          <RewardMetric icon="star-four-points-circle-outline" label="Puan" value={`+${formatInt(dkd_puan_value)}`} tone="gold" />
          <RewardMetric icon="cards-outline" label="Koleksiyon Kartı" value={`+${formatInt(dkd_collection_card_count_value)}`} tone="cyan" />
          <RewardMetric icon="cards-playing-diamond-outline" label="Özel Kart" value={ticketText} tone="green" />
        </View>
      </View>
