...
function ActionMenuModal({
  ...
  onBusiness,
  onLegalCenter,
  dkd_on_app_update_center_value,
  onLogout,
}) {
  const handlePlayerCard = onPlayerCard || onSocialCard;

  const groups = useMemo(() => {
    const open = (dkd_callback_value) => {
      onClose?.();
      dkd_callback_value?.();
    };
    ...
    const base = [
      {
        ...
        items: [
          ...
          onWalletTopup ? {
            ...
            label: 'Bakiye Yükle',
            sub: 'Cüzdanına Para Yükle ekranını aç',
            tone: 'green',
          ...
          isAdmin && handlePlayerCard ? {
            key: 'card',
            icon: 'card-account-details-outline',
            label: 'Oyuncu kartı',
            sub: 'Admin özel sosyal vitrin ve karşılaştırma alanı',
            tone: 'purple',
          ...
          onTasks ? {
            ...
            label: 'Görevler',
            sub: 'Günlük ve haftalık görev akışını aç',
            tone: 'cyan',
            onPress: () => open(onTasks),
          } : null,
          {
            key: 'history',
            icon: 'history',
            label: 'Geçmiş',
            sub: 'Son ödül, market ve görev kayıtları',
            tone: 'gold',
          ...
          onAchievements ? {
            ...
            icon: 'trophy-outline',
            label: 'Başarımlar',
            sub: 'Rozetler, ödüller ve başarı ilerlemesi',
            tone: 'green',
      ...
      },
      {
        ...
        sub: 'Güvenlik ve yönetim araçları',
        items: [
          onLegalCenter ? {
            key: 'dkd_legal_center',
            icon: 'shield-account-outline',
            label: 'Gizlilik ve Veri Merkezi',
            sub: 'Data Safety, veri silme, izin ve cüzdan açıklamaları',
            tone: 'green',
            onPress: () => onLegalCenter?.(),
          } : null,
          isAdmin && dkd_on_app_update_center_value ? {
            key: 'dkd_app_update_center',
            icon: 'cellphone-arrow-down',
            label: 'Sürüm ve Güncelleme Merkezi',
            sub: 'Resmi APK güncellemesini kontrol et ve indirme sayfasını aç',
            tone: 'cyan',
            onPress: () => dkd_on_app_update_center_value?.(),
          } : null,
          isAdmin ? {
            key: 'admin',
...
function areActionMenuModalPropsEqual(prev, next) {
  return !!prev.visible === !!next.visible
    ...
    && prev.onLegalCenter === next.onLegalCenter
    && prev.dkd_on_app_update_center_value === next.dkd_on_app_update_center_value
