...
import {
  dkd_create_business_product_point_order,
...
function ShopPackCard({ item, onPress, busy }) {
  ...
  const rewardText = 'Fiziksel teslimat ürünü';
    ...
    <View style={[styles.packCard, { minHeight: dkd_metrics.minHeight }]}>
      ...
      <View style={styles.packTop}>
        ...
        <LinearGradient colors={accent.chip} style={styles.packBadge}>
          <Text style={styles.packBadgeText}>
            {String(item?.badge_label || "Fiziksel Ürün")}
          </Text>
        </LinearGradient>
      </View>

      <Text style={[styles.packTitle, { fontSize: dkd_metrics.titleSize }]}>
        {String(item?.title || "Fiziksel Ürün")}
      </Text>
      <Text style={styles.packSubtitle}>
        {String(item?.subtitle || "Fiziksel sipariş ürünü")}
      </Text>
      <Text
        style={[
      ...
      <View style={styles.packMetaRow}>
        <View style={styles.packPriceRow}>
          ...
          <Text style={styles.packPriceText}>
            {formatNum(item?.dkd_reward_puan || 0)} puan
          </Text>
        </View>
      ...
      >
        ...
        <Text style={styles.packActionText}>
          {busy ? "Bekle…" : "Fiziksel sipariş puanıyla kullan"}
        </Text>
      </Pressable>
    </View>
  );
...
function ListingCard({ item, mine, onOpenCard, onCancel, onBuy }) {
  ...
  return (
    <View style={styles.listingCard}>
      ...
      <View style={styles.listingTopRow}>
        ...
        <View style={styles.pricePill}>
          <MaterialCommunityIcons
            ...
            color={ui.colors.gold}
          />
          <Text style={styles.priceText}>Kapalı</Text>
        </View>
      </View>
      ...
      <Pressable
        onPress={() => (mine ? onCancel?.(item.id) : onOpenCard?.(card))}
        style={({ pressed }) => [
      ...
      >
        ...
        <Text style={styles.listingActionText}>
          {mine ? "İlanı kaldır" : "Kart takası kapalı"}
        </Text>
      </Pressable>
    </View>
  );
...
function DkdMarketInlineCollectionPanel({
  dkd_collection_entry_values,
  dkd_collection_total_count_value,
  dkd_collection_unique_count_value,
  dkd_profile_value,
  dkd_on_open_card_value,
}) {
  const dkd_visible_collection_values = dkd_collection_entry_values.slice(0, 10);

  return (
    <View style={styles.dkdCollectionPanelMinimal}>
      <LinearGradient colors={["rgba(19,24,44,0.98)", "rgba(10,13,24,0.98)"]} style={StyleSheet.absoluteFill} />
      <View style={styles.dkdCollectionHeaderRow}>
        <View style={styles.dkdCollectionHeaderIcon}>
          <MaterialCommunityIcons name="cards-outline" size={22} color="#BFEFFF" />
        </View>
        <View style={styles.dkdCollectionHeaderCopy}>
          <Text style={styles.panelEyebrow}>KOLEKSİYON</Text>
          <Text style={styles.dkdCollectionMinimalTitle}>Kart koleksiyonu</Text>
          <Text style={styles.dkdCollectionMinimalSub}>
            Kart albümünü sade liste halinde incele; burada gerçek para ile dijital ürün satışı yapılmaz.
          </Text>
        </View>
      </View>

      {dkd_visible_collection_values.length ? (
        <View style={styles.dkdCollectionListWrap}>
          {dkd_visible_collection_values.map((dkd_card_entry_value, dkd_card_index_value) => {
            const dkd_card_value = cardFromEntry(dkd_card_entry_value);
            const dkd_rarity_value = rarityAccent(dkd_card_value?.rarity);
            const dkd_image_uri_value = dkd_card_value?.art_image_url || dkd_card_value?.image_url || '';
            const dkd_serial_value = dkd_card_value?.serial_code || `DKD-${String(dkd_card_index_value + 1).padStart(3, '0')}`;
            return (
              <Pressable
                key={`dkd-market-collection-${String(dkd_card_entry_value?.id || dkd_serial_value || dkd_card_value?.name || dkd_card_index_value)}`}
                onPress={() => dkd_on_open_card_value?.(dkd_card_value)}
                style={({ pressed }) => [
                  styles.dkdCollectionListItem,
                  pressed && styles.actionPressed,
                ]}
              >
                <View style={styles.dkdCollectionThumbWrap}>
                  {dkd_image_uri_value ? (
                    <Image source={{ uri: dkd_image_uri_value }} style={styles.dkdCollectionThumbImage} />
                  ) : (
                    <MaterialCommunityIcons name="cards-outline" size={22} color="rgba(255,255,255,0.68)" />
                  )}
                </View>
                <View style={styles.dkdCollectionListCopy}>
                  <View style={styles.dkdCollectionListTopRow}>
                    <Text style={styles.dkdCollectionListName} numberOfLines={1}>{dkd_card_value?.name || 'Kart'}</Text>
                    <View style={[styles.dkdCollectionRarityPill, { borderColor: dkd_rarity_value.color }]}> 
                      <Text style={[styles.dkdCollectionRarityPillText, { color: dkd_rarity_value.color }]} numberOfLines={1}>{dkd_rarity_value.label}</Text>
                    </View>
                  </View>
                  <Text style={styles.dkdCollectionListMeta} numberOfLines={1}>
                    {dkd_card_value?.series || 'Genel'} • {dkd_serial_value}
                  </Text>
                </View>
        ...
        <View style={styles.emptyWrap}>
          <MaterialCommunityIcons name="cards-outline" size={24} color="rgba(255,255,255,0.56)" />
          <Text style={styles.emptyTitle}>Koleksiyon boş</Text>
          <Text style={styles.emptySub}>Kartlar geldikçe bu alan sade liste halinde dolacak.</Text>
        </View>
      )}
    </View>
  );
...
function dkd_resolve_food_market_mode_value(dkd_product_value) {
  const dkd_category_blob_value = [
    ...
    .map((dkd_text_value) => String(dkd_text_value || "").toLocaleLowerCase("tr-TR"))
    .join(" ");
  const dkd_food_keywords_value = [
    "yemek",
    "restoran",
    "restaurant",
    "menü",
  ...
  const dkd_market_keywords_value = [
    "market",
    "bakkal",
    "grocery",
    "manav",
  ...
  if (dkd_food_keywords_value.some((dkd_keyword_value) => dkd_category_blob_value.includes(dkd_keyword_value))) return "food";
  if (dkd_market_keywords_value.some((dkd_keyword_value) => dkd_category_blob_value.includes(dkd_keyword_value))) return "market";
  return "market";
}

function dkd_render_courier_order_category_panel_value(dkd_mode_value, dkd_on_open_courier_board_value) {
  const dkd_is_urgent_value = dkd_mode_value === "urgent";
  const dkd_palette_value = dkd_is_urgent_value
    ? {
      dkd_kicker_value: "ACİL KURYE SİPARİŞLERİ",
      dkd_title_value: "Hızlı ihtiyaç ve al-götür görev merkezi",
      dkd_subtitle_value: "Fırın, market, belge veya küçük paket gibi acil teslimatları kurye akışına bağlayan ayrı sipariş kategorisi.",
      dkd_icon_value: "bike-fast",
      dkd_colors_value: ["rgba(126,42,38,0.98)", "rgba(42,15,39,0.98)", "rgba(13,20,32,0.98)"],
      dkd_accent_value: "#FFB86B",
      dkd_button_value: "Acil kurye panelini aç",
      dkd_tags_value: ["Yeni sipariş", "Kurye teklifi", "Canlı takip"],
      dkd_steps_value: [
        "Müşteri ihtiyaç listesini ve teslimat adresini girer.",
        "Kurye uygun taşıma ücretini teklif eder.",
        "Müşteri onayından sonra canlı takip açılır.",
      ],
    }
    : {
      dkd_kicker_value: "KARGO SİPARİŞLERİ",
      dkd_title_value: "Paket, gönderi ve rota takip vitrini",
      dkd_subtitle_value: "Kargo oluşturma, kurye kabulü, alım noktası ve teslimat adımlarını ayrı kategori içinde temiz şekilde toplar.",
      dkd_icon_value: "package-variant-closed-check",
      dkd_colors_value: ["rgba(30,67,130,0.98)", "rgba(14,33,68,0.98)", "rgba(7,17,30,0.98)"],
      dkd_accent_value: "#8FD8FF",
      dkd_button_value: "Kargo panelini aç",
      dkd_tags_value: ["Kargo oluştur", "Alım rotası", "Teslimat kanıtı"],
      dkd_steps_value: [
        "Gönderici paket ve alım adresini tanımlar.",
        "Kurye görevi kabul edip alım noktasına gider.",
        "Teslimat tamamlanınca takip kaydı kapanır.",
      ],
    };

  function dkd_handle_open_courier_board_value() {
    if (dkd_on_open_courier_board_value) {
      dkd_on_open_courier_board_value(dkd_mode_value);
      return;
    }
    Alert.alert("Kurye-Kargo", "Bu kategori ana sayfadaki Kurye-Kargo paneline bağlı.");
  }

  return (
    <View style={styles.dkdCourierCategoryPanel}>
      <LinearGradient colors={dkd_palette_value.dkd_colors_value} style={StyleSheet.absoluteFill} />
      <View style={styles.dkdCourierCategoryGlow} />
      <View style={styles.dkdCourierCategoryTopRow}>
        <View
          style={[
            styles.dkdCourierCategoryIconWrap,
            {
              borderColor: `${dkd_palette_value.dkd_accent_value}44`,
              backgroundColor: `${dkd_palette_value.dkd_accent_value}18`,
            },
          ]}
        >
          <MaterialCommunityIcons name={dkd_palette_value.dkd_icon_value} size={30} color={dkd_palette_value.dkd_accent_value} />
        </View>
        <View style={styles.dkdCourierCategoryCopyWrap}>
          <Text style={styles.panelEyebrow}>{dkd_palette_value.dkd_kicker_value}</Text>
          <Text style={styles.dkdCourierCategoryTitle}>{dkd_palette_value.dkd_title_value}</Text>
          <Text style={styles.dkdCourierCategorySub}>{dkd_palette_value.dkd_subtitle_value}</Text>
        </View>
      </View>
      <View style={styles.dkdCourierCategoryTagRow}>
        {dkd_palette_value.dkd_tags_value.map((dkd_tag_value) => (
          <View key={dkd_tag_value} style={styles.dkdCourierCategoryTag}>
            <Text style={styles.dkdCourierCategoryTagText}>{dkd_tag_value}</Text>
          </View>
        ))}
      </View>
      <View style={styles.dkdCourierCategoryStepGrid}>
        {dkd_palette_value.dkd_steps_value.map((dkd_step_value, dkd_step_index_value) => (
          <View key={dkd_step_value} style={styles.dkdCourierCategoryStepCard}>
            <Text style={[styles.dkdCourierCategoryStepNo, { color: dkd_palette_value.dkd_accent_value }]}>
              {`0${dkd_step_index_value + 1}`}
            </Text>
            <Text style={styles.dkdCourierCategoryStepText}>{dkd_step_value}</Text>
          </View>
        ))}
      </View>
      <Pressable
        onPress={dkd_handle_open_courier_board_value}
        style={({ pressed }) => [
          styles.dkdCourierCategoryAction,
          pressed && styles.actionPressed,
        ]}
      >
        <Text style={styles.dkdCourierCategoryActionText}>{dkd_palette_value.dkd_button_value}</Text>
...
export default function MarketModal({
  ...
  const [dkd_market_section, dkd_set_market_section] = useState("collection");
  const [dkd_food_market_mode_value, dkd_set_food_market_mode_value] = useState("food");
  ...
  const dkd_market_list_ref = useRef(null);
  const dkd_market_scroll_timeout_ref = useRef(null);
  const listReady = useDeferredVisibility(visible, true);

  const marketListings = useMemo(() => (Array.isArray(listings) ? listings : []), [listings]);
  ...
  const marketShopDefs = useMemo(() => {
    const rows = (
      ...
      .filter((item) => {
        const dkd_reward_kind_value = String(item?.reward_kind || '').toLowerCase();
        const dkd_pack_key_value = String(item?.pack_key || '');
        return item?.is_active !== false && (dkd_reward_kind_value === 'merchant_product' || dkd_pack_key_value.startsWith('merchant_product:'));
      })
      .sort(
  ...
  const dkd_food_product_count_value = useMemo(
    () => (Array.isArray(merchantProducts) ? merchantProducts : []).filter((dkd_product_value) => dkd_resolve_food_market_mode_value(dkd_product_value) === "food").length,
    [merchantProducts],
  );
  const dkd_market_product_count_value = useMemo(
    () => (Array.isArray(merchantProducts) ? merchantProducts : []).filter((dkd_product_value) => dkd_resolve_food_market_mode_value(dkd_product_value) === "market").length,
    [merchantProducts],
  );
  const dkd_food_market_mode_cards_value = useMemo(
    () => [
      {
        dkd_key_value: "food",
        dkd_title_value: "Mağaza Yemek Siparişleri",
        dkd_subtitle_value: "Restoran, fırın ve hızlı menü akışı",
        dkd_icon_value: "silverware-fork-knife",
        dkd_count_value: dkd_food_product_count_value,
        dkd_is_locked_value: false,
      },
      {
        dkd_key_value: "market",
        dkd_title_value: "Mağaza marketi siparişleri",
        dkd_subtitle_value: "Tüm market siparişleri için kilitli ön izleme",
        dkd_icon_value: "cart-variant",
        dkd_count_value: dkd_market_product_count_value,
        dkd_is_locked_value: true,
      },
    ],
    [dkd_food_product_count_value, dkd_market_product_count_value],
  );
  const dkd_filtered_merchant_sections_value = useMemo(
    () => merchantSections
      .map((dkd_section_value) => ({
        ...dkd_section_value,
        items: dkd_section_value.items.filter((dkd_product_value) => dkd_resolve_food_market_mode_value(dkd_product_value) === dkd_food_market_mode_value),
      }))
      .filter((dkd_section_value) => dkd_section_value.items.length),
    [dkd_food_market_mode_value, merchantSections],
  );
  const dkd_collection_entry_values = useMemo(
    () => (Array.isArray(userCards) ? userCards.slice() : []),
    [userCards],
  );
  const dkd_collection_total_count_value = dkd_collection_entry_values.length;
  const dkd_collection_unique_count_value = useMemo(() => {
    const dkd_collection_key_values = new Set();
    dkd_collection_entry_values.forEach((dkd_card_entry_value) => {
      const dkd_card_value = cardFromEntry(dkd_card_entry_value);
      dkd_collection_key_values.add(String(dkd_card_value?.serial_code || dkd_card_value?.name || dkd_card_entry_value?.id || 'dkd_card'));
    });
    return dkd_collection_key_values.size;
  }, [dkd_collection_entry_values]);
  ...
  const dkd_market_list_data = useMemo(() => {
    ...
    return marketListings.slice().sort((dkd_left_listing, dkd_right_listing) => {
      ...
      if (dkd_rarity_delta !== 0) return dkd_rarity_delta;
      const dkd_price_delta = Number(dkd_left_listing?.dkd_reward_puan || 0) - Number(dkd_right_listing?.dkd_reward_puan || 0);
  ...
  function normalizeMerchantBuyReason(reason) {
    ...
    if (!key) return "Sipariş oluşturulamadı.";
    if (key === "delivery_address_required") {
      return "Teslimat adresini yazmadan fiziksel sipariş başlayamaz. GPS tek başına yeterli değil.";
    }
    if (key === "location_required") {
      return "İşletme ürününü almak için cihaz konumu açık olmalı.";
    }
    if (key === "insufficient_token") return "Yeterli puan yok.";
  ...
  async function handleMerchantBuy(productId) {
    ...
    try {
      ...
      const { data, error } = await dkd_create_business_product_point_order({
        dkd_point_order_key: `merchant_product:${productId}`,
      ...
      if (data?.ok) {
        Alert.alert(
          "İşletme Marketi",
          `${data?.spent_points || 0} kazanılmış puan kullanıldı.
${data?.business_name || "İşletme"} • ${data?.product_name || "Ürün"} için fiziksel teslimat siparişi oluşturuldu.`,
        );
  ...
  function dkd_handle_market_section_card_press_value(dkd_section_key_value) {
    dkd_set_market_section(dkd_section_key_value);

    if (dkd_market_scroll_timeout_ref.current) {
      clearTimeout(dkd_market_scroll_timeout_ref.current);
    }

    dkd_market_scroll_timeout_ref.current = setTimeout(() => {
      dkd_market_list_ref.current?.scrollToOffset?.({
        offset: 520,
        animated: true,
      });
      dkd_market_scroll_timeout_ref.current = null;
    }, 140);
  }
  void dkd_handle_market_section_card_press_value;
  ...
  const header = (
    <View style={styles.headerWrap}>
      ...
      {dkd_market_section === "shop" ? (
        <View style={styles.panelCard}>
          <LinearGradient
            ...
            style={StyleSheet.absoluteFill}
          />
          <Text style={styles.panelEyebrow}>FİZİKSEL HİZMET PUANI</Text>
          <Text style={styles.panelTitle}>Fiziksel hizmet puanı vitrini</Text>
          <Text style={styles.panelSub}>
            Puan yalnızca görev, hizmet ve etkinlik akışlarından kazanılır; TL ile puan, koleksiyon, özel kart veya enerji satın alınmaz.
          </Text>

          <View style={styles.dkdQuickInfoRow}>
            <Tag
              label={`${formatNum(marketShopDefs.length)} fiziksel ürün`}
              tone="cyan"
            />
            <Tag label={`${liveEnergy}/${liveEnergyMax} enerji`} tone="gold" />
            <Tag
              label={`${formatNum(profile?.boss_tickets || 0)} özel kart`}
              tone="purple"
            />
          </View>
          ...
          <View style={styles.packGrid}>
            {shopRows.map((row, rowIndex) => (
              <View key={`shop-row-${rowIndex}`} style={styles.packRow}>
                {row.map((item, dkd_shop_cell_index) => (
                  <View
                    key={`dkd-shop-${String(item?.id || item?.pack_key || rowIndex)}-${String(item?.pack_key || item?.title || dkd_shop_cell_index)}`}
      ...
      {dkd_market_section === "merchant" ? (
        <View style={styles.panelCard}>
          <LinearGradient
            ...
            style={StyleSheet.absoluteFill}
          />
          <Text style={styles.panelEyebrow}>YEMEK/MARKET SİPARİŞLERİ</Text>
          <Text style={styles.panelTitle}>Yemek/Market Siparişleri</Text>
          <Text style={styles.panelSub}>
            Yiyecek ve Market Siparişlerinizi sipariş verin.
          </Text>
          ...
          <View style={styles.deliveryPanel}>
            <View style={styles.deliveryHeroRow}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.deliverySub}>
                  Adres ve teslimat notunu tek bloktan gir. Sipariş oluştururken
            ...
            <Text style={styles.deliveryHint}>
              {hasLiveLocation
                ? "Sipariş için açık adres ve canlı konum birlikte gönderilir."
                : "Sipariş oluşturmadan önce cihaz konumunu açman gerekiyor."}
            </Text>
          </View>

          <View style={styles.dkdFoodMarketModeGrid}>
            {dkd_food_market_mode_cards_value.map((dkd_mode_card_value) => {
              const dkd_is_active_mode_value = dkd_food_market_mode_value === dkd_mode_card_value.dkd_key_value;
              return (
                <Pressable
                  key={dkd_mode_card_value.dkd_key_value}
                  onPress={() => {
                    if (dkd_mode_card_value.dkd_is_locked_value) {
                      Alert.alert("Çok Yakında", "Çok Yakında hizmetinizde");
                      return;
                    }
                    dkd_set_food_market_mode_value(dkd_mode_card_value.dkd_key_value);
                  }}
                  style={({ pressed }) => [
                    styles.dkdFoodMarketModeCard,
                    dkd_mode_card_value.dkd_is_locked_value && styles.dkdFoodMarketModeCardLocked,
                    dkd_is_active_mode_value && styles.dkdFoodMarketModeCardActive,
                    pressed && styles.actionPressed,
                  ]}
                >
                  {dkd_mode_card_value.dkd_is_locked_value ? (
                    <View style={styles.dkdFoodMarketModeLockBadge}>
                      <MaterialCommunityIcons name="lock" size={15} color="#FFFFFF" />
                    </View>
                  ) : null}
                  <View style={styles.dkdFoodMarketModeIconWrap}>
                    <MaterialCommunityIcons name={dkd_mode_card_value.dkd_icon_value} size={20} color="#FFFFFF" />
                  </View>
                  <View style={styles.dkdFoodMarketModeCopyWrap}>
                    <Text style={styles.dkdFoodMarketModeTitle}>{dkd_mode_card_value.dkd_title_value}</Text>
                    <Text style={styles.dkdFoodMarketModeSub}>{dkd_mode_card_value.dkd_subtitle_value}</Text>
                  </View>
                  <View style={styles.dkdFoodMarketModeBadge}>
                    <Text style={styles.dkdFoodMarketModeBadgeText}>{formatNum(dkd_mode_card_value.dkd_count_value)} ürün</Text>
                  </View>
                </Pressable>
              );
            })}
          ...
          ) : dkd_filtered_merchant_sections_value.length ? (
            <View style={styles.merchantSectionWrap}>
              {dkd_filtered_merchant_sections_value.map((section) => (
                  ...
                  <View style={styles.merchantBusinessHead}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.merchantBusinessSub}>
                        {section.businessCategory || "genel"} • kazanılmış puan
                        vitrini
                      </Text>
                    </View>
                        ...
                        <View style={styles.merchantBody}>
                          ...
                          <View style={styles.merchantBuyRowModern}>
                            <View style={styles.merchantPriceInlineCluster}>
                              ...
                              <View style={styles.merchantPuanDockCompact}>
                                <MaterialCommunityIcons
                                  name="star-four-points-circle-outline"
                                  size={13}
                                  color="#F7FBFF"
                                />
                                <Text
                                  style={styles.merchantPuanDockTextCompact}
                                >
                                  {formatNum(item?.dkd_reward_puan || 0)} puan
                                </Text>
                              </View>
      ...
      {dkd_market_section === "collection" ? (
        <DkdMarketInlineCollectionPanel
          dkd_collection_entry_values={dkd_collection_entry_values}
          dkd_collection_total_count_value={dkd_collection_total_count_value}
          dkd_collection_unique_count_value={dkd_collection_unique_count_value}
          dkd_profile_value={profile}
          dkd_on_open_card_value={onOpenCard}
        />
      ) : null}


      {dkd_market_section === "urgent" ? (
        dkd_render_courier_order_category_panel_value("urgent", dkd_on_open_courier_board_value)
      ) : null}

      {dkd_market_section === "cargo" ? (
        dkd_render_courier_order_category_panel_value("cargo", dkd_on_open_courier_board_value)
      ) : null}
      ...
      {dkd_market_section === "sell" ? (
        <>
          {dkd_sell_page_mode === "overview" ? (
            <View style={styles.panelCard}>
              <LinearGradient
                ...
                style={StyleSheet.absoluteFill}
              />
              <Text style={styles.panelEyebrow}>KART VİTRİNİ</Text>
            ...
            <View style={styles.panelCard}>
              ...
              <View style={styles.listFormCard}>
                ...
                <View style={styles.formRow}>
                  ...
                  <Pressable
                    onPress={() =>
                      Alert.alert("Market", "Kart satış/ilan akışı mağaza sürümünde kapalıdır.")
                    }
                    style={({ pressed }) => [
                      styles.formAction,
                      pressed && styles.actionPressed,
                  ...
                  >
                    <LinearGradient
                      ...
                      style={StyleSheet.absoluteFill}
                    />
                    <Text style={styles.formActionText}>İlan akışı kapalı</Text>
                  </Pressable>
                </View>
              </View>
            </View>
          ...
          {ownListings.length ? (
            <View style={styles.panelCard}>
              <LinearGradient
                ...
                style={StyleSheet.absoluteFill}
              />
              <Text style={styles.panelEyebrow}>KAPALI İLAN AKIŞI</Text>
              <Text style={styles.panelTitle}>Kart ilanları mağaza sürümünde kapalı</Text>
                ...
                {ownListings.map((item, index) => (
                  ...
                  >
                    ...
                    <Text style={styles.ownListingSub}>
                      {item.card_series} • Kart ilan akışı kapalı
                    </Text>
                    <Pressable
          ...
          <Text style={styles.listEyebrow}>KART PAZARI KAPALI</Text>
          <Text style={styles.listTitle}>Kart satış/puan takası kapalı</Text>
          <Text style={styles.listSub}>
            Kart satış, ilan ve puanla takas akışları mağaza sürümünde kapalıdır; koleksiyon yalnızca vitrin olarak gösterilir.
          </Text>
        </>
      ) : null}
    </View>
        ...
        >
          {(loading && !marketListings.length) || !listReady ? (
            <View style={styles.loadingWrap}>
              <ActivityIndicator color={ui.colors.cyan} />
              <Text style={styles.loadingText}>Koleksiyonun hazırlanıyor…</Text>
            </View>
          ) : (
            <FlatList
              ref={dkd_market_list_ref}
              key="market-grid-2"
...
const styles = StyleSheet.create({
  ...
  dkdCollectionPanelMinimal: {
    borderRadius: 24,
    borderWidth: 1,
    borderColor: "rgba(191,239,255,0.16)",
    padding: 16,
    marginBottom: 16,
    overflow: "hidden",
  },
  dkdCollectionHeaderRow: {
    flexDirection: "row",
    gap: 12,
    alignItems: "flex-start",
  },
  dkdCollectionHeaderIcon: {
    width: 48,
    height: 48,
    borderRadius: 16,
  ...
  dkdCollectionHeaderCopy: {
    flex: 1,
    minWidth: 0,
  },
  dkdCollectionMinimalTitle: {
    color: theme.colors.text,
    fontSize: 20,
    fontWeight: "900",
    marginTop: 5,
  },
  dkdCollectionMinimalSub: {
  ...
  dkdCollectionListWrap: {
    gap: 10,
    marginTop: 14,
  },
  dkdCollectionListItem: {
    minHeight: 76,
    borderRadius: 18,
    padding: 10,
    flexDirection: "row",
  ...
  dkdCollectionThumbWrap: {
    ...
    borderWidth: 1,
    borderColor: "rgba(108,203,255,0.16)",
    alignItems: "center",
    justifyContent: "center",
  },
  dkdCollectionThumbImage: {
    width: "100%",
    height: "100%",
  },
  dkdCollectionListCopy: {
    flex: 1,
    minWidth: 0,
  },
  dkdCollectionListTopRow: {
    flexDirection: "row",
    alignItems: "center",
    gap: 8,
  },
  dkdCollectionListName: {
    flex: 1,
    color: theme.colors.text,
    fontSize: 15,
    fontWeight: "900",
  },
  dkdCollectionRarityPill: {
    maxWidth: 92,
    borderRadius: 999,
    paddingHorizontal: 9,
    paddingVertical: 5,
  ...
  dkdCollectionRarityPillText: {
    fontSize: 9,
    fontWeight: "900",
    letterSpacing: 0.4,
    textTransform: "uppercase",
  },
  dkdCollectionListMeta: {
    color: "rgba(255,255,255,0.56)",
    fontSize: 11,
    fontWeight: "800",
    marginTop: 5,
  ...
  dkdFoodMarketModeCardActive: {
    borderColor: "rgba(116,233,255,0.30)",
    backgroundColor: "rgba(116,233,255,0.10)",
  },
  dkdFoodMarketModeCardLocked: {
    borderColor: "rgba(255,219,127,0.32)",
    backgroundColor: "rgba(255,219,127,0.08)",
  },
  dkdFoodMarketModeLockBadge: {
    position: "absolute",
    right: 10,
    top: 10,
    width: 30,
  ...
  dkdFoodMarketModeCopyWrap: {
    marginTop: 10,
  },
  dkdFoodMarketModeTitle: {
    color: theme.colors.text,
    fontSize: 15,
    fontWeight: "900",
  },
  dkdFoodMarketModeSub: {
    color: theme.colors.textSoft,
    fontSize: 11,
    lineHeight: 16,
    fontWeight: "700",
  ...
  dkdFoodMarketModeBadgeText: {
    color: "#FFFFFF",
    fontSize: 10,
    fontWeight: "900",
  },
  dkdCourierCategoryPanel: {
    borderRadius: 26,
    borderWidth: 1,
    borderColor: "rgba(255,255,255,0.12)",
    padding: 16,
  ...
  dkdCourierCategoryTopRow: {
    flexDirection: "row",
    alignItems: "flex-start",
    gap: 12,
  },
  dkdCourierCategoryIconWrap: {
    width: 62,
    height: 62,
    borderRadius: 22,
    borderWidth: 1,
  ...
  dkdCourierCategoryCopyWrap: {
    flex: 1,
    minWidth: 0,
  },
  dkdCourierCategoryTitle: {
    color: theme.colors.text,
    fontSize: 20,
    lineHeight: 25,
    fontWeight: "900",
  ...
  dkdCourierCategoryTagRow: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 8,
    marginTop: 14,
  },
  dkdCourierCategoryTag: {
    borderRadius: 999,
    borderWidth: 1,
    borderColor: "rgba(255,255,255,0.14)",
    backgroundColor: "rgba(255,255,255,0.08)",
  ...
  dkdCourierCategoryTagText: {
    color: "#FFFFFF",
    fontSize: 11,
    fontWeight: "900",
  },
  dkdCourierCategoryStepGrid: {
    gap: 10,
    marginTop: 14,
  },
  dkdCourierCategoryStepCard: {
    borderRadius: 18,
    borderWidth: 1,
    borderColor: "rgba(255,255,255,0.10)",
  ...
  dkdCourierCategoryStepNo: {
    fontSize: 13,
    fontWeight: "900",
    letterSpacing: 0.7,
  },
  dkdCourierCategoryStepText: {
    flex: 1,
    color: "rgba(255,255,255,0.82)",
    fontSize: 12,
    lineHeight: 18,
  ...
  merchantPuanDockText: {
    color: "#F7FBFF",
    fontSize: 13,
    fontWeight: "900",
    letterSpacing: 0.2,
  },
  merchantPuanDockTextCompact: {
    color: "#F7FBFF",
    fontSize: 11,
    fontWeight: "900",
    letterSpacing: 0.1,
