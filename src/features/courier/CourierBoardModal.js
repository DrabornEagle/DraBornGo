...
function DkdJobCardValue({ dkd_job_value, dkd_user_id_value, dkd_busy_job_id_value, dkd_on_action_value }) {
  ...
  const dkd_open_flag = dkd_open_value(dkd_job_value);
  ...
  return (
    <View style={dkd_styles_value.jobCard}>
      <View style={dkd_styles_value.jobHead}>
        <View style={dkd_styles_value.jobIcon}><MaterialCommunityIcons name={String(dkd_job_value?.job_type || '').toLowerCase() === 'cargo' ? 'package-variant-closed' : 'bike-fast'} size={22} color="#7EEBFF" /></View>
        <View style={{ flex: 1 }}><Text style={dkd_styles_value.jobTitle}>{dkd_job_value?.product_title || dkd_job_value?.title || 'Kurye Görevi'}</Text><Text style={dkd_styles_value.jobStatus}>{dkd_status_label_value(dkd_job_value)}</Text></View>
      ...
      <View style={dkd_styles_value.routeBox}>
        <View style={dkd_styles_value.routeRow}><MaterialCommunityIcons name="map-marker-outline" size={18} color="#7EEBFF" /><View style={{ flex: 1 }}><Text style={dkd_styles_value.routeLabel}>Alım adresi</Text><Text style={dkd_styles_value.routeText}>{dkd_job_value?.pickup || 'Adres bekleniyor'}</Text></View></View>
        <View style={dkd_styles_value.routeRow}><MaterialCommunityIcons name="map-marker-check-outline" size={18} color="#7CE8B6" /><View style={{ flex: 1 }}><Text style={dkd_styles_value.routeLabel}>Teslim / varış</Text><Text style={dkd_styles_value.routeText}>{dkd_job_value?.dropoff || dkd_job_value?.delivery_address_text || 'Adres bekleniyor'}</Text></View></View>
      </View>
      {dkd_job_value?.delivery_note ? <Text style={dkd_styles_value.note}>{dkd_job_value.delivery_note}</Text> : null}
      {dkd_busy_flag ? <ActivityIndicator color="#7EEBFF" style={{ marginTop: 12 }} /> : (
        <View style={dkd_styles_value.actions}>
          {dkd_open_flag ? <><Pressable onPress={() => dkd_on_action_value('accept', dkd_job_id_value)} style={dkd_styles_value.primary}><Text style={dkd_styles_value.primaryText}>Görevi Kabul Et</Text></Pressable><Pressable onPress={() => dkd_on_action_value('reject', dkd_job_id_value)} style={dkd_styles_value.secondary}><Text style={dkd_styles_value.secondaryText}>Geç</Text></Pressable></> : null}
          {dkd_can_pickup_flag ? <Pressable onPress={() => dkd_on_action_value('pickup', dkd_job_id_value)} style={dkd_styles_value.primary}><Text style={dkd_styles_value.primaryText}>Teslim Aldım</Text></Pressable> : null}
          {dkd_can_complete_flag ? <Pressable onPress={() => dkd_on_action_value('complete', dkd_job_id_value)} style={dkd_styles_value.complete}><Text style={dkd_styles_value.completeText}>Teslim Ettim</Text></Pressable> : null}
        </View>
      )}
    </View>
  );
...
function CourierBoardModal({ visible, onClose, profile, currentLocation, sessionUserId, setProfile, dkd_initial_panel_value = 'default' }) {
  const [dkd_panel_value, dkd_set_panel_value] = useState(() => dkd_panel_key_value(dkd_initial_panel_value));
  ...
  useEffect(() => { if (visible) dkd_set_panel_value(dkd_panel_key_value(dkd_initial_panel_value)); }, [visible, dkd_initial_panel_value]);
  ...
  const dkd_load_jobs_value = useCallback(async (dkd_force_value = false) => {
    ...
    } catch (dkd_error_value) { Alert.alert('Kurye', dkd_error_value?.message || 'Görevler alınamadı.'); }
    finally { dkd_set_loading_value(false); dkd_set_refreshing_value(false); }
  }, []);
  ...
  const dkd_run_action_value = useCallback(async (dkd_action_key_value, dkd_job_id_value) => {
    ...
    } catch (dkd_error_value) { Alert.alert('Kurye', dkd_error_value?.message || 'Görev güncellenemedi.'); }
    finally { dkd_set_busy_job_id_value(null); }
  ...
  const dkd_toggle_online_value = useCallback(async () => {
    ...
    } catch (dkd_error_value) { Alert.alert('Kurye', dkd_error_value?.message || 'Kurye durumu değiştirilemedi.'); }
    finally { dkd_set_online_busy_value(false); }
...
const dkd_styles_value = StyleSheet.create({
  screen: { flex: 1, backgroundColor: '#030914' }, panelBody: { flex: 1 }, header: { padding: 18, flexDirection: 'row', gap: 12, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,.1)' }, kicker: { color: '#7EEBFF', fontSize: 10, fontWeight: '900', letterSpacing: 1.2 }, title: { color: '#FFF', fontSize: 28, fontWeight: '900', marginTop: 4 }, sub: { color: 'rgba(235,244,255,.64)', fontSize: 12, marginTop: 5 }, close: { width: 44, height: 44, borderRadius: 16, backgroundColor: 'rgba(255,255,255,.08)', alignItems: 'center', justifyContent: 'center' }, content: { padding: 16, paddingBottom: 50 }, back: { alignSelf: 'flex-start', minHeight: 44, borderRadius: 15, paddingHorizontal: 13, flexDirection: 'row', alignItems: 'center', gap: 7, backgroundColor: '#7EEBFF', marginBottom: 12 }, backText: { color: '#031019', fontSize: 12, fontWeight: '900' }, shortcutRow: { flexDirection: 'row', gap: 9, marginBottom: 10 }, shortcut: { flex: 1, minHeight: 52, borderRadius: 17, backgroundColor: 'rgba(126,235,255,.08)', borderWidth: 1, borderColor: 'rgba(126,235,255,.16)', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7 }, shortcutText: { color: '#E8F8FF', fontSize: 11, fontWeight: '900' },
  statusCard: { borderRadius: 24, padding: 15, backgroundColor: 'rgba(255,255,255,.055)', borderWidth: 1, borderColor: 'rgba(126,235,255,.16)', flexDirection: 'row', alignItems: 'center', gap: 11 }, statusDot: { width: 12, height: 12, borderRadius: 6, backgroundColor: '#74819A' }, statusDotOnline: { backgroundColor: '#76E7B5' }, statusTitle: { color: '#FFF', fontSize: 15, fontWeight: '900' }, statusText: { color: 'rgba(235,244,255,.60)', fontSize: 11, lineHeight: 16, marginTop: 3 }, statusButton: { minHeight: 42, borderRadius: 14, paddingHorizontal: 12, backgroundColor: '#7EEBFF', alignItems: 'center', justifyContent: 'center' }, statusButtonText: { color: '#031019', fontSize: 11, fontWeight: '900' }, section: { color: '#A9EEFF', fontSize: 10, fontWeight: '900', letterSpacing: 1.1, marginTop: 18, marginBottom: 8 },
  jobCard: { marginTop: 10, borderRadius: 24, padding: 15, backgroundColor: 'rgba(7,22,42,.92)', borderWidth: 1, borderColor: 'rgba(255,255,255,.10)' }, jobHead: { flexDirection: 'row', alignItems: 'center', gap: 10 }, jobIcon: { width: 46, height: 46, borderRadius: 16, backgroundColor: 'rgba(126,235,255,.10)', alignItems: 'center', justifyContent: 'center' }, jobTitle: { color: '#FFF', fontSize: 16, fontWeight: '900' }, jobStatus: { color: '#9DECBF', fontSize: 11, fontWeight: '800', marginTop: 3 }, fee: { color: '#FFE48D', fontSize: 14, fontWeight: '900' }, routeBox: { marginTop: 13, borderRadius: 18, padding: 12, backgroundColor: 'rgba(255,255,255,.045)', gap: 10 }, routeRow: { flexDirection: 'row', gap: 8, alignItems: 'flex-start' }, routeLabel: { color: 'rgba(203,226,246,.65)', fontSize: 10, fontWeight: '900' }, routeText: { color: '#FFF', fontSize: 12, fontWeight: '700', lineHeight: 18, marginTop: 2 }, note: { marginTop: 10, color: 'rgba(235,244,255,.64)', fontSize: 11, lineHeight: 17 }, actions: { flexDirection: 'row', gap: 8, marginTop: 13 }, primary: { flex: 1, minHeight: 48, borderRadius: 16, backgroundColor: '#7EEBFF', alignItems: 'center', justifyContent: 'center' }, primaryText: { color: '#031019', fontWeight: '900', fontSize: 12 }, secondary: { minWidth: 76, minHeight: 48, borderRadius: 16, backgroundColor: 'rgba(255,255,255,.08)', alignItems: 'center', justifyContent: 'center' }, secondaryText: { color: '#FFF', fontWeight: '900', fontSize: 12 }, complete: { flex: 1, minHeight: 48, borderRadius: 16, backgroundColor: '#76E7B5', alignItems: 'center', justifyContent: 'center' }, completeText: { color: '#031019', fontWeight: '900', fontSize: 12 }, empty: { marginTop: 10, borderRadius: 22, padding: 24, backgroundColor: 'rgba(255,255,255,.04)', alignItems: 'center' }, emptyTitle: { color: '#FFF', fontSize: 16, fontWeight: '900', marginTop: 8 }, emptyText: { color: 'rgba(235,244,255,.58)', fontSize: 11, marginTop: 4, textAlign: 'center' },
});
