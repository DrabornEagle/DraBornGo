...
function dkd_find_active_job_value(dkd_rows_value, dkd_profile_value) {
  const dkd_user_id_value = dkd_trim_text_value(dkd_profile_value?.user_id);
  const dkd_safe_rows_value = Array.isArray(dkd_rows_value) ? dkd_rows_value : [];
  return dkd_safe_rows_value.find((dkd_row_value) => (
...
export default function DkdCourierOnlineGlobalWatcher({
  ...
  const [, setDkdHasActiveJobStateValue] = useState(false);
  ...
  const dkd_scan_for_offer_value = useCallback(async () => {
    ...
    try {
      ...
      const dkd_existing_active_job_value = dkd_find_active_job_value(dkd_existing_rows_value, dkd_profile_value);
      if (dkd_existing_active_job_value) {
        const dkd_existing_active_job_id_value = dkd_existing_active_job_value?.id || null;
        ...
        dkd_set_profile_value?.((dkd_previous_profile_value) => (dkd_previous_profile_value ? {
          ...
          dkd_courier_auto_assigned_job_id: dkd_existing_active_job_id_value,
        } : dkd_previous_profile_value));
        return;
      }
      ...
      const dkd_next_active_job_value = dkd_find_active_job_value(dkd_rows_value, dkd_profile_value);
      const dkd_next_has_active_job_value = Boolean(dkd_next_active_job_value);
      setDkdHasActiveJobStateValue(dkd_next_has_active_job_value);
      setDkdOfferJobValue(dkd_next_has_active_job_value ? null : (dkd_next_offer_value || null));
      dkd_set_profile_value?.((dkd_previous_profile_value) => (dkd_previous_profile_value ? {
        ...dkd_previous_profile_value,
        dkd_courier_online: !dkd_next_has_active_job_value,
        ...
        dkd_courier_auto_assigned_job_id: dkd_next_has_active_job_value ? (dkd_next_active_job_value?.id || null) : (dkd_next_offer_value?.id || null),
      } : dkd_previous_profile_value));
  ...
  const dkd_accept_offer_value = useCallback(async () => {
    ...
    try {
      ...
      const dkd_delivery_lock_result_value = await dkd_lock_courier_delivery_state_value(dkd_offer_job_value.id);
      ...
      dkd_set_profile_value?.((dkd_previous_profile_value) => (dkd_previous_profile_value ? {
        ...dkd_previous_profile_value,
        dkd_courier_online: false,
        dkd_courier_auto_assigned_job_id: dkd_offer_job_value.id,
      } : dkd_previous_profile_value));
  ...
  const dkd_reject_offer_value = useCallback(async () => {
    ...
    try {
      ...
      setTimeout(() => dkd_scan_for_offer_value(), 360);
      setTimeout(() => dkd_scan_for_offer_value(), 1650);
    } catch (dkd_error_value) {
...
const dkd_styles = StyleSheet.create({
  ...
  dkd_mini_sub: { color: 'rgba(213,248,255,0.78)', fontSize: 12.5, fontWeight: '850', marginTop: 2 },
  ...
  dkd_offer_alert_strip_text: { color: '#06111A', fontSize: 12.5, fontWeight: '950', letterSpacing: 0.8 },
  ...
  dkd_offer_eyebrow: { color: '#7EEBFF', fontSize: 12.5, fontWeight: '950', letterSpacing: 0.8 },
  ...
  dkd_offer_price_label: { color: 'rgba(232,245,255,0.70)', fontSize: 12.5, fontWeight: '950', letterSpacing: 0.7 },
  ...
  dkd_offer_action_sub: { color: 'rgba(255,255,255,0.76)', fontSize: 12.5, fontWeight: '850', marginTop: 2 },
  dkd_offer_accept_text: { color: '#FFFFFF', fontSize: 16, fontWeight: '950' },
  dkd_offer_accept_sub: { color: 'rgba(255,255,255,0.80)', fontSize: 12.5, fontWeight: '850', marginTop: 2 },
