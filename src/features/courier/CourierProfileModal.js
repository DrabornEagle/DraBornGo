...
function CourierProfileModal({ visible, onClose, profile, setProfile }) {
  ...
  const summary = useMemo(() => {
    const dkd_now_value = new Date();
    const dkd_today_key_value = dayKey(dkd_now_value);
    const dkd_current_month_value = monthKey(dkd_now_value);
    const dkd_history_total_earnings_value = completedTasks.reduce((dkd_sum_value, dkd_task_value) => dkd_sum_value + Number(dkd_task_value?.fee_tl || 0), 0);
    const dkd_history_today_earnings_value = completedTasks
      .filter((dkd_task_value) => dayKey(getCompletedAt(dkd_task_value)) === dkd_today_key_value)
      .reduce((dkd_sum_value, dkd_task_value) => dkd_sum_value + Number(dkd_task_value?.fee_tl || 0), 0);
    const dkd_history_month_earnings_value = completedTasks
      .filter((dkd_task_value) => monthKey(getCompletedAt(dkd_task_value)) === dkd_current_month_value)
      .reduce((dkd_sum_value, dkd_task_value) => dkd_sum_value + Number(dkd_task_value?.fee_tl || 0), 0);
    const dkd_profile_wallet_value = Number(dkd_effective_profile?.courier_wallet_tl ?? dkd_effective_profile?.wallet_tl ?? 0);
    const dkd_profile_total_earnings_value = Number(dkd_effective_profile?.total_earned_tl ?? dkd_effective_profile?.courier_total_earned_tl ?? Number.NaN);
    const dkd_profile_today_earnings_value = Number(dkd_effective_profile?.today_earnings_tl ?? Number.NaN);
    const dkd_profile_month_earnings_value = Number(dkd_effective_profile?.month_earnings_tl ?? Number.NaN);
    const dkd_profile_average_fee_value = Number(dkd_effective_profile?.avg_fee_tl ?? Number.NaN);
    const dkd_profile_active_days_value = Number(dkd_effective_profile?.active_days ?? dkd_effective_profile?.courier_active_days ?? Number.NaN);
    const dkd_fastest_eta_value = Number(dkd_effective_profile?.fastest_eta_min ?? dkd_effective_profile?.courier_fastest_eta_min ?? Number.NaN);
    const dkd_history_fastest_eta_value = completedTasks
      .map((dkd_task_value) => Number(dkd_task_value?.eta_min || 0))
      .filter((dkd_eta_value) => dkd_eta_value > 0)
      .sort((dkd_left_value, dkd_right_value) => dkd_left_value - dkd_right_value)[0] || null;
    const dkd_total_earnings_value = Number.isFinite(dkd_profile_total_earnings_value) ? dkd_profile_total_earnings_value : dkd_history_total_earnings_value;
    const dkd_average_fee_value = Number.isFinite(dkd_profile_average_fee_value) ? dkd_profile_average_fee_value : (completedTasks.length ? dkd_history_total_earnings_value / completedTasks.length : 0);
    const dkd_active_days_value = Number.isFinite(dkd_profile_active_days_value) ? dkd_profile_active_days_value : new Set(completedTasks.map((dkd_task_value) => dayKey(getCompletedAt(dkd_task_value))).filter(Boolean)).size;
    const dkd_fastest_eta_text_value = Number.isFinite(dkd_fastest_eta_value) && dkd_fastest_eta_value > 0
      ? `${intText(dkd_fastest_eta_value)} dk`
      : dkd_history_fastest_eta_value ? `${intText(dkd_history_fastest_eta_value)} dk` : '-';
    const dkd_rating_value = completedTasks.length || Number(dkd_effective_profile?.rating_count ?? 0) > 0 ? `${Number(dkd_effective_profile?.rating_avg ?? 5).toFixed(1)} / 5` : '-';
    return {
      cashBalance: Number.isFinite(dkd_profile_wallet_value) ? dkd_profile_wallet_value : 0,
      totalEarnings: dkd_total_earnings_value,
      todayEarnings: Number.isFinite(dkd_profile_today_earnings_value) ? dkd_profile_today_earnings_value : dkd_history_today_earnings_value,
      monthEarnings: Number.isFinite(dkd_profile_month_earnings_value) ? dkd_profile_month_earnings_value : dkd_history_month_earnings_value,
      fastestEtaText: dkd_fastest_eta_text_value,
      averageFee: dkd_average_fee_value,
      activeDays: dkd_active_days_value,
      rating: dkd_rating_value,
    };
  }, [completedTasks, dkd_effective_profile]);

  useEffect(() => {
    if (!setProfile || !visible) return;
    const dkd_profile_completed_jobs_value = Number(dkd_effective_profile?.completed_jobs ?? dkd_effective_profile?.courier_completed_jobs ?? Number.NaN);
    if (!Number.isFinite(dkd_profile_completed_jobs_value)) return;
    setProfile((dkd_previous_profile_value) => (dkd_previous_profile_value ? {
      ...dkd_previous_profile_value,
      courier_completed_jobs: Math.max(0, dkd_profile_completed_jobs_value),
    } : dkd_previous_profile_value));
  }, [dkd_effective_profile?.completed_jobs, dkd_effective_profile?.courier_completed_jobs, setProfile, visible]);

  const dkd_profile_delivery_count_value = Number(dkd_effective_profile?.completed_jobs ?? dkd_effective_profile?.courier_completed_jobs ?? Number.NaN);
  const deliveryCountText = intText(Number.isFinite(dkd_profile_delivery_count_value) ? dkd_profile_delivery_count_value : completedTasks.length);
