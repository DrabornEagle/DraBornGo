...
const dkd_cargo_default_delivery_note_value = 'Teşekkür Ederim';
const dkd_package_content_option_values = [
  { dkd_key_value: 'dkd_box', dkd_label_value: 'Kutu', dkd_icon_name_value: 'package-variant-closed' },
  { dkd_key_value: 'dkd_document', dkd_label_value: 'Belge/Evrak', dkd_icon_name_value: 'file-document-outline' },
  { dkd_key_value: 'dkd_accessory', dkd_label_value: 'Aksesuar', dkd_icon_name_value: 'watch-variant' },
  { dkd_key_value: 'dkd_electronic', dkd_label_value: 'Elektronik', dkd_icon_name_value: 'cellphone-link' },
  { dkd_key_value: 'dkd_textile', dkd_label_value: 'Giyim/Tekstil', dkd_icon_name_value: 'tshirt-crew-outline' },
  { dkd_key_value: 'dkd_book', dkd_label_value: 'Kitap/Defter', dkd_icon_name_value: 'book-open-page-variant-outline' },
  { dkd_key_value: 'dkd_gift', dkd_label_value: 'Hediye', dkd_icon_name_value: 'gift-outline' },
  { dkd_key_value: 'dkd_sample', dkd_label_value: 'Numune', dkd_icon_name_value: 'flask-outline' },
  { dkd_key_value: 'dkd_other', dkd_label_value: 'Diğer', dkd_icon_name_value: 'pencil-plus-outline' },
];
function dkd_default_form_state() {
  return {
    ...
    dkd_package_content_choice_key: '',
...
function dkd_package_content_label_value(dkd_form_value = {}) {
  return String(dkd_form_value?.dkd_package_content_text || '').trim();
}

function dkd_package_content_order_title_value(dkd_content_value) {
...
function dkd_package_content_fixed_label_values() {
  return dkd_package_content_option_values
    .filter((dkd_option_value) => dkd_option_value.dkd_key_value !== 'dkd_other')
    .map((dkd_option_value) => dkd_option_value.dkd_label_value.toLocaleLowerCase('tr-TR'));
}

function dkd_package_content_is_fixed_value(dkd_text_value) {
  return dkd_package_content_fixed_label_values().includes(String(dkd_text_value || '').trim().toLocaleLowerCase('tr-TR'));
}
...
function DkdTrackingPulseIcon({ dkd_is_active_value, dkd_theme_value }) {
...
function DkdTrackingActionChip({ dkd_is_active_value, dkd_disabled_value, dkd_on_press_value }) {
  const dkd_pulse_anim_value = useRef(new Animated.Value(1)).current;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text style={[dkd_styles.dkd_shipmentActionChipText, dkd_is_active_value && dkd_styles.dkd_shipmentActionChipTextActive]}>{dkd_is_active_value ? 'Canlı Takip TIKLA' : 'Takip Bekleniyor'}</Text>
        {dkd_is_active_value ? (
          <Animated.View
            style={[
...
export function DkdCargoShipmentDetailReplica({ dkd_shipment_value, dkd_on_open_live_map_value }) {
  const dkd_status_key_value = String(dkd_shipment_value?.status || 'open').toLowerCase();
  ...
  const dkd_open_live_map_handler_value = () => {
    dkd_on_open_live_map_value?.(dkd_shipment_value?.id, dkd_shipment_value);
  };

  return (
    <View style={dkd_styles.dkd_shipmentCard}>
      ...
      <View style={dkd_styles.dkd_routeStack}>
        ...
        <View style={dkd_styles.dkd_addressCard}>
          ...
          <Text style={dkd_styles.dkd_addressCardText}>{dkd_shipment_value?.delivery_note || dkd_cargo_default_delivery_note_value}</Text>
        </View>
      </View>
...
function dkd_missing_form_field_labels_value(dkd_form_value) {
  const dkd_missing_labels_value = [];
  if (!String(dkd_form_value?.dkd_customer_first_name || '').trim()) dkd_missing_labels_value.push('gönderici adı');
  if (!String(dkd_form_value?.dkd_customer_last_name || '').trim()) dkd_missing_labels_value.push('gönderici soyadı');
  if (dkd_normalize_digits(dkd_form_value?.dkd_customer_national_id, 11).length !== 11) dkd_missing_labels_value.push('TC kimlik no');
  if (!dkd_is_valid_turkiye_phone_value(dkd_form_value?.dkd_customer_phone_text)) dkd_missing_labels_value.push('telefon no');
  if (String(dkd_form_value?.dkd_pickup_address_text || '').trim().length < 10) dkd_missing_labels_value.push('gönderici adresi');
  if (String(dkd_form_value?.dkd_delivery_address_text || '').trim().length < 10) dkd_missing_labels_value.push('teslimat adresi');
  if (dkd_package_content_label_value(dkd_form_value).length < 2) dkd_missing_labels_value.push('paket içeriği');
  if ((dkd_safe_number(dkd_form_value?.dkd_package_weight_kg) ?? 0) <= 0) dkd_missing_labels_value.push('ortalama ağırlık');
  if (!String(dkd_form_value?.dkd_package_image_uri || '').trim()) dkd_missing_labels_value.push('paket görseli');
  return dkd_missing_labels_value;
}

function dkd_missing_form_alert_message_value(dkd_form_value) {
  const dkd_missing_labels_value = dkd_missing_form_field_labels_value(dkd_form_value);
  if (!dkd_missing_labels_value.length) return '';
  const dkd_missing_text_value = dkd_missing_labels_value.join(', ');
  return dkd_missing_labels_value.length === 1
    ? `Eksik alan: ${dkd_missing_text_value}.`
    : `Eksik alanlar: ${dkd_missing_text_value}.`;
}
...
export default function DkdCargoSenderPanel({
  ...
  const dkd_select_package_content_option_value = useCallback((dkd_option_value) => {
    setDkdFormValue((dkd_previous_value) => {
      if (dkd_option_value?.dkd_key_value === 'dkd_other') {
        const dkd_previous_content_value = String(dkd_previous_value?.dkd_package_content_text || '').trim();
        return {
          ...dkd_previous_value,
          dkd_package_content_choice_key: 'dkd_other',
          dkd_package_content_text: dkd_package_content_is_fixed_value(dkd_previous_content_value) ? '' : dkd_previous_content_value,
        };
      }
      return {
        ...dkd_previous_value,
        dkd_package_content_choice_key: dkd_option_value?.dkd_key_value || '',
        dkd_package_content_text: dkd_option_value?.dkd_label_value || '',
      };
    });
  }, []);
  ...
  const dkd_load_shipments = useCallback(async ({ dkd_apply_initial_filter_value = false, dkd_silent_value = false } = {}) => {
    ...
    try {
      ...
      if (!dkd_silent_value) {
        Alert.alert('Paket', dkd_error_value?.message || 'Paket gönderileri yüklenemedi.');
      }
    } finally {
  ...
  }, [dkd_current_location_value, dkd_form_value?.dkd_pickup_address_text, dkd_visible_value]);
  ...
  const dkd_choose_image = useCallback(async () => {
    try {
      ...
      Alert.alert('Paket', dkd_error_value?.message || 'Paket görseli seçilemedi.');
    }
  }, [dkd_set_form_field]);
      ...
      <View style={dkd_styles.dkd_panelCard}>
        ...
        <View style={dkd_styles.dkd_panelHead}>
          <View style={dkd_styles.dkd_panelHeadCopy}>
            <Text style={dkd_styles.dkd_panelTitle}>Gönderi Oluştur</Text>
        ...
        <View style={dkd_styles.dkd_packageOptionGrid}>
          {dkd_package_content_option_values.map((dkd_option_value) => {
            const dkd_selected_value = dkd_form_value.dkd_package_content_choice_key === dkd_option_value.dkd_key_value;
            return (
              <Pressable
                key={dkd_option_value.dkd_key_value}
                onPress={() => dkd_select_package_content_option_value(dkd_option_value)}
                style={({ pressed: dkd_pressed_value }) => [
                  dkd_styles.dkd_packageOptionChip,
                  dkd_selected_value && dkd_styles.dkd_packageOptionChipSelected,
                  dkd_pressed_value && dkd_styles.dkd_packageOptionChipPressed,
                ]}
              >
                <MaterialCommunityIcons name={dkd_option_value.dkd_icon_name_value} size={16} color={dkd_selected_value ? '#06111A' : '#BAE6FD'} />
                <Text style={[dkd_styles.dkd_packageOptionText, dkd_selected_value && dkd_styles.dkd_packageOptionTextSelected]}>{dkd_option_value.dkd_label_value}</Text>
              </Pressable>
            );
          })}
        </View>
        {dkd_form_value.dkd_package_content_choice_key === 'dkd_other' ? (
          <DkdFieldInput
            dkd_value={dkd_form_value.dkd_package_content_text}
            dkd_on_change_value={(dkd_next_value) => dkd_set_form_field('dkd_package_content_text', dkd_next_value)}
            dkd_placeholder_value="Kendi paket içeriğini yaz"
            dkd_max_length_value={80}
          />
        ) : null}
        ...
        <View style={dkd_styles.dkd_fieldRow}>
          ...
          <View style={dkd_styles.dkd_fieldCol}>
            <DkdFieldLabel dkd_required_value>Paket Görseli</DkdFieldLabel>
...
const dkd_styles = StyleSheet.create({
  ...
  dkd_packageOptionGrid: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: 8,
    marginBottom: 10,
  },
  dkd_packageOptionChip: {
    minWidth: '30%',
    flexGrow: 1,
    flexBasis: '30%',
    minHeight: 44,
  ...
  dkd_packageOptionChipSelected: {
    backgroundColor: '#BAE6FD',
    borderColor: '#7DD3FC',
  },
  dkd_packageOptionChipPressed: {
    transform: [{ scale: 0.98 }],
    opacity: 0.9,
  },
  dkd_packageOptionText: {
    color: '#EAF5FF',
    fontSize: 11.5,
    fontWeight: '900',
    textAlign: 'center',
  },
  dkd_packageOptionTextSelected: {
    color: '#06111A',
  },
  dkd_packageOptionHint: {
    marginTop: -2,
    marginBottom: 12,
