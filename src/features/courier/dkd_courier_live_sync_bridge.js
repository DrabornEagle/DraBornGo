...
function dkd_job_phase_value(dkd_task_value) {
  ...
  if (dkd_status_key_value === 'accepted' || dkd_status_key_value === 'assigned' || dkd_status_key_value === 'to_pickup') return 'to_pickup';
  return 'open';
}
...
function dkd_task_list_fingerprint_value(dkd_task_list_value) {
  return (Array.isArray(dkd_task_list_value) ? dkd_task_list_value : []).map((dkd_task_value) => ([
    String(dkd_task_value?.id || ''), String(dkd_task_value?.status || ''), String(dkd_task_value?.pickup_status || ''),
    String(dkd_task_value?.assigned_user_id || ''), String(dkd_task_value?.updated_at || ''),
    String(dkd_task_value?.pickup_lat ?? ''), String(dkd_task_value?.pickup_lng ?? ''),
    String(dkd_task_value?.dropoff_lat ?? ''), String(dkd_task_value?.dropoff_lng ?? ''),
  ].join('|'))).join('||');
}

function dkd_target_for_task_value(dkd_task_value) {
  const dkd_phase_value = dkd_job_phase_value(dkd_task_value);
  if (dkd_phase_value === 'to_customer') return { dkd_phase_value, dkd_lat_value: dkd_safe_number_value(dkd_task_value?.dropoff_lat), dkd_lng_value: dkd_safe_number_value(dkd_task_value?.dropoff_lng) };
  if (dkd_phase_value === 'to_pickup') return { dkd_phase_value, dkd_lat_value: dkd_safe_number_value(dkd_task_value?.pickup_lat), dkd_lng_value: dkd_safe_number_value(dkd_task_value?.pickup_lng) };
  return { dkd_phase_value, dkd_lat_value: null, dkd_lng_value: null };
}
...
export default function DkdCourierLiveSyncBridge({ dkd_profile_value, dkd_current_location_value, dkd_session_user_id_value }) {
  ...
  const dkd_active_task_value = useMemo(() => {
    const dkd_task_row_value = (Array.isArray(dkd_task_list_value) ? dkd_task_list_value : []).find((dkd_loop_task_value) => {
      const dkd_is_active_phase_value = ['to_pickup', 'to_customer'].includes(dkd_job_phase_value(dkd_loop_task_value));
    ...
    return dkd_task_row_value ? { ...dkd_task_row_value, ...dkd_target_for_task_value(dkd_task_row_value) } : null;
  ...
  useEffect(() => {
    ...
    return () => { dkd_cancelled_value = true; clearInterval(dkd_interval_value); dkd_app_state_subscription_value?.remove?.(); };
  ...
  useEffect(() => {
    ...
    if (dkd_current_lat_value == null || dkd_current_lng_value == null || !dkd_active_task_value) return undefined;
    let dkd_cancelled_value = false;
    async function dkd_push_live_location_value() {
      if (dkd_app_state_ref_value.current !== 'active') return;
      try {
        const dkd_distance_km_value = dkd_haversine_km_between_value(dkd_current_lat_value, dkd_current_lng_value, dkd_active_task_value?.dkd_lat_value, dkd_active_task_value?.dkd_lng_value);
