...
export default function DkdUrgentCourierPanel({
  ...
  useEffect(() => {
    ...
    const dkd_requested_tab_value = dkd_queue_only_value ? 'courier' : (dkd_default_tab_value || 'create');
    setDkdActiveTabValue(dkd_hide_courier_tab_value && dkd_requested_tab_value === 'courier' ? 'create' : dkd_requested_tab_value);
  }, [dkd_default_tab_value, dkd_hide_courier_tab_value, dkd_queue_only_value, dkd_visible_value]);

  useEffect(() => {
    if (dkd_hide_courier_tab_value && dkd_active_tab_value === 'courier') {
      setDkdActiveTabValue('create');
    }
  }, [dkd_active_tab_value, dkd_hide_courier_tab_value]);
  ...
  const dkd_submit_order_value = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('Acil Kurye', 'Sipariş oluşturuldu. DraBornGo mobil operasyon paneli aynı siparişi görecek.');
    } catch (dkd_error_value) {
    ...
    <View key={dkd_group_value.dkd_group_key} style={dkd_styles.dkd_store_group_card}>
      ...
      <View style={dkd_styles.dkd_store_grid}>
        {dkd_group_value.dkd_store_values.map((dkd_store_name_value) => {
          ...
          const dkd_store_placeholder_text_value = 'Alınacak ürünleri yaz';
          return (
    ...
    <View style={dkd_styles.dkd_panel_card}>
      <View style={dkd_styles.dkd_panel_head_row}>
        ...
        <View style={dkd_styles.dkd_panel_head_copy}>
          ...
          <Text style={dkd_styles.dkd_panel_subtitle}>Market, Fırın ve günlük ihtiyaçlarınız kapınıza gelsin.</Text>
        </View>
      </View>
          ...
          <View style={dkd_styles.dkd_inline_input_row}>
            ...
            <Pressable
              onPress={() => {
                ...
                dkd_run_action_value(`offer_fee_${dkd_order_id_value}`, async () => {
                  const dkd_offer_action_result_value = await dkd_offer_fee_with_live_location_value(dkd_order_id_value, dkd_fee_value);
                  if (!dkd_offer_action_result_value?.error) setDkdActiveTabValue('courier');
                  return dkd_offer_action_result_value;
                }, 'Taşıma ücreti müşteriye gönderildi. Canlı kurye konumu müşteri takip ekranına bağlandı.');
              }}
      ...
      {dkd_queue_only_value ? (
        ...
        <>
          <LinearGradient colors={['rgba(255,216,88,0.26)', 'rgba(255,92,149,0.22)', 'rgba(108,242,255,0.16)', 'rgba(124,255,217,0.12)']} start={dkd_make_native_axis_point(0, 0)} end={dkd_make_native_axis_point(1, 1)} style={dkd_styles.dkd_hero_card}>
            <View style={dkd_styles.dkd_hero_top_row}>
              ...
              <View style={dkd_styles.dkd_hero_copy}>
                ...
                <Text style={dkd_styles.dkd_hero_text}>Market, Fırın ve günlük ihtiyaçlarınız kapınıza gelsin.</Text>
              </View>
            </View>
          ...
          <View style={dkd_styles.dkd_tab_row}>
            ...
            {dkd_hide_courier_tab_value ? null : <DkdUrgentTab dkd_tab_key_value="courier" dkd_active_tab_value={dkd_active_tab_value} dkd_label_value="Kurye İşleri" dkd_icon_name_value="bike-fast" dkd_on_press_value={setDkdActiveTabValue} />}
          </View>
        </>
      )}
