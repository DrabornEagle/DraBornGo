  ...
  return (
    <Pressable onPress={onPress} style={[dkd_styles.sourceRow, active && dkd_styles.sourceRowActive]}>
      ...
      <View style={{ flex: 1, minWidth: 0 }}>
        <Text style={dkd_styles.sourceTitle} numberOfLines={1}>{row?.drop_name || row?.title || 'Özel Hedef'}</Text>
        <Text style={dkd_styles.sourceSub} numberOfLines={2}>{hasDefinition ? 'Tanım hazır' : 'Tanım yok'} • {ticketCost} özel kart • {questionCount} soru</Text>
      </View>
...
function QuestionCard({ index, item, onChange, onRemove }) {
  ...
  return (
    <View style={dkd_styles.questionCard}>
      ...
      <Field label="Soru" value={String(item?.dkd_query_value || '')} onChangeText={(dkd_query_value) => onChange({ ...item, dkd_query_value })} placeholder="Özel hedef sorusu" multiline />
...
function CropEditorModal({ visible, asset, onClose, onApply }) {
  ...
  async function applyCrop() {
    try {
      ...
      Alert.alert('Özel Hedef', 'Görsel serbest kırpılırken sorun oluştu. Daha küçük bir görsel deneyebilirsin.');
    } finally {
      setBusy(false);
    }
  }
    ...
    <Modal visible={visible} animationType="fade" transparent onRequestClose={onClose}>
      <View style={dkd_styles.cropBackdrop}>
        <View style={dkd_styles.cropCard}>
          ...
          <Text style={dkd_styles.cropSub}>Görseli sürükle, yakınlaştır ve özel hedef alanına tam oturt.</Text>
          ...
          <View style={dkd_styles.cropHintGrid}>
            ...
            <Text style={dkd_styles.cropHint}>• Sonuç özel hedef kapak alanına tam sığacak şekilde hazırlanır.</Text>
          </View>
...
export default memo(function AdminBossModal({ visible, onClose, loading, bossDefs, bossDrops, onRefresh, onSave, onDelete }) {
  ...
  const dkd_refresh_ref_value = useRef(onRefresh);

  useEffect(() => {
    dkd_refresh_ref_value.current = onRefresh;
  }, []);
  ...
  const refreshBossSources = useCallback(async (force = true) => {
    ...
    try {
      const [{ data: dropRows, error: dropErr }] = await Promise.all([
        ...
        Promise.resolve(dkd_refresh_ref_value.current?.(force)),
      ]);
      ...
      Alert.alert('Özel Hedef', String(dkd_error_value?.message || dkd_error_value || 'Özel hedef noktaları yüklenemedi.'));
      return [];
    } finally {
  ...
  const mergedRows = useMemo(() => drops.map((drop) => {
    ...
    return {
      ...
      drop_name: drop?.name || found?.title || 'Özel Hedef',
      title: found?.title || drop?.name || 'Özel Hedef',
  ...
  async function pickImageManual() {
    try {
      const dkd_picker_result_value = await launchBossPicker();
      ...
      if (!dkd_selected_asset_value?.uri) return;
      setCropAsset(dkd_selected_asset_value);
      setCropEditorOpen(true);
    } catch {
      Alert.alert('Özel Hedef', 'Özel hedef görseli seçilirken sorun oluştu.');
    }
  }
  ...
  async function pickImageAuto() {
    try {
      ...
      const dkd_selected_asset_value = Array.isArray(dkd_picker_result_value?.assets) ? dkd_picker_result_value.assets[0] : null;
      if (!dkd_selected_asset_value?.uri) return;
      const dkd_prepared_uri_value = await prepareAutoBossImage(dkd_selected_asset_value);
      setDraft((dkd_previous_draft_value) => ({ ...dkd_previous_draft_value, art_image_url: dkd_prepared_uri_value }));
      Alert.alert('Özel Hedef', 'Görsel otomatik 16:9 kapak olarak hazırlandı.');
    } catch {
      Alert.alert('Özel Hedef', 'Otomatik kapak hazırlanırken sorun oluştu.');
    }
  }
  ...
  function hydrateFromRow(row) {
    ...
    setDraft({
      ...
      title: String(row?.title || row?.drop_name || 'Özel Hedef'),
  ...
  async function saveBoss() {
    ...
    if (!draft.drop_id) return Alert.alert('Özel Hedef', 'Önce bir özel hedef noktası seç.');
    if (String(draft.title || '').trim().length < 2) return Alert.alert('Özel Hedef', 'Özel hedef adı gerekli.');
    if (cleanQuestions.length < 3) return Alert.alert('Özel Hedef', 'En az 3 tam soru gerekli.');
      ...
      <SafeScreen style={{ flex: 1, backgroundColor: cityLootTheme.colors.bgTop }}>
          ...
          <ScrollView contentContainerStyle={dkd_styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            <GlassPanel style={dkd_styles.heroPanel}>
              <View style={dkd_styles.heroRow}>
                ...
                <View style={dkd_styles.heroCopy}>
                  <Text style={dkd_styles.kicker}>ADMIN ÖZEL HEDEF CONTROL</Text>
                  <Text style={dkd_styles.title}>Özel Hedef Komuta Merkezi</Text>
                </View>
                <SecondaryButton label="Kapat" onPress={onClose} size="compact" fullWidth={false} />
              </View>
              <View style={dkd_styles.metricRow}>
                <MetricCard icon="crown-outline" label="Özel Hedef" value={String(mergedRows.length)} />
            ...
            <GlassPanel style={{ marginTop: 14 }}>
              <View style={dkd_styles.sectionHead}>
                <Text style={dkd_styles.sectionTitle}>Özel Hedef Noktaları</Text>
              ...
              {!loading && !mergedRows.length ? <Text style={dkd_styles.emptyText}>Henüz özel hedef noktası yok.</Text> : null}
              {(loading || sourceLoading) ? <ActivityIndicator color={cityLootTheme.colors.cyanSoft} style={{ marginTop: 16 }} /> : null}
            </GlassPanel>

            <GlassPanel style={{ marginTop: 14 }}>
              <View style={dkd_styles.sectionHead}>
                <Text style={dkd_styles.sectionTitle}>{draft.drop_id ? 'Özel Hedef Düzenle' : 'Özel Hedef İçeriği Oluştur'}</Text>
                <SecondaryButton label="Temizle" icon="broom" onPress={resetDraft} size="compact" fullWidth={false} />
              </View>

              <Field label="Bağlı Özel Hedef Noktası" value={selectedSource?.drop_name || ''} onChangeText={() => {}} placeholder="Üstten özel hedef seç" editable={false} />
              <View style={dkd_styles.grid2}>
                <Field label="Özel Hedef Adı" value={draft.title} onChangeText={(title) => setDraft((prev) => ({ ...prev, title }))} placeholder="Örn: Sincan Özel Hedef" />
                <Field label="Özel Hedef Key" value={draft.boss_key} onChangeText={(boss_key) => setDraft((prev) => ({ ...prev, boss_key }))} placeholder="ozel-hedef-sincan" />
              </View>
              <Field label="Alt Başlık" value={draft.subtitle} onChangeText={(subtitle) => setDraft((prev) => ({ ...prev, subtitle }))} placeholder="Özel hedef açıklama satırı" />
              ...
              <Field label="Ödül Özeti" value={draft.reward_summary} onChangeText={(reward_summary) => setDraft((prev) => ({ ...prev, reward_summary }))} placeholder="Puan • Koleksiyon Puanı • Kart • Özel Kart" />
              <View style={dkd_styles.grid2}>
                <Field label="Giriş Özel Kartı" value={draft.ticket_cost} onChangeText={(ticket_cost) => setDraft((prev) => ({ ...prev, ticket_cost }))} placeholder="1" keyboardType="number-pad" />
                <Field label="Hedef Gücü" value={draft.boss_hp_display} onChangeText={(boss_hp_display) => setDraft((prev) => ({ ...prev, boss_hp_display }))} placeholder="985000" keyboardType="number-pad" />
              </View>
              ...
              <Text style={dkd_styles.fieldLabel}>Özel Hedef Görseli</Text>
            ...
            <GlassPanel style={{ marginTop: 14 }}>
              <View style={dkd_styles.sectionHead}>
                <Text style={dkd_styles.sectionTitle}>Özel Hedef Soru Seti</Text>
            ...
            <View style={dkd_styles.actionRow}>
              <PrimaryButton label={editingDropId ? 'Özel Hedef Güncelle' : 'Özel Hedef Kaydet'} icon="content-save-outline" onPress={saveBoss} style={dkd_styles.flexBtn} />
        ...
        <CropEditorModal
          ...
          onApply={(uri) => {
            ...
            setCropEditorOpen(false);
            setCropAsset(null);
            Alert.alert('Özel Hedef', 'Görsel serbest kırpma ile hazırlandı.');
          }}
        />
      </SafeScreen>
    </Modal>
