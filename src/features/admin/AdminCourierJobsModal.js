...
import { dkd_delete_admin_courier_application_value, dkd_fetch_admin_courier_applications } from '../../services/adminService';
import dkd_admin_courier_payouts_modal from './dkd_admin_courier_payouts_modal';
...
export default function AdminCourierJobsModal({ visible, onClose, loading, jobs, onRefresh, onSave, onDelete }) {
  ...
  const [dkd_courier_payouts_open_value, dkd_set_courier_payouts_open_value] = useState(false);
  ...
  function dkd_handle_job_delete_value(dkd_job_id_value) {
    Alert.alert('Görevi Sil', `#${dkd_job_id_value} numaralı kurye görevini silmek istiyor musun?`, [
      ...
      { text: 'SİL', style: 'destructive', onPress: () => onDelete?.(dkd_job_id_value) },
    ]);
  }
  ...
  function dkd_handle_application_delete_value(dkd_application_row_value) {
    const dkd_application_id_value = String(dkd_application_row_value?.application_id || dkd_application_row_value?.id || '').trim();
    const dkd_user_id_value = String(dkd_application_row_value?.user_id || dkd_application_row_value?.applicant_user_id || '').trim();
    const dkd_application_name_value = dkd_full_name_value(dkd_application_row_value);

    if (!dkd_application_id_value && !dkd_user_id_value) {
      Alert.alert('Kurye Başvuruları', 'Silinecek başvuru ID veya kullanıcı bilgisi bulunamadı.');
      return;
    }

    Alert.alert(
      'Başvuruyu Sil',
      `${dkd_application_name_value} başvurusu silinsin mi? Bu işlem başvuru kaydını kaldırır ve kurye lisans durumunu başvuru yok durumuna çeker.`,
      [
        { text: 'Vazgeç', style: 'cancel' },
        {
          ...
          onPress: async () => {
            setDkdApplicationLoadingValue(true);
            try {
              const dkd_delete_response_value = await dkd_delete_admin_courier_application_value({
                dkd_application_id_value,
                dkd_user_id_value,
              });
              if (dkd_delete_response_value?.error) throw dkd_delete_response_value.error;
              setDkdSelectedApplicationIdValue('');
              setDkdApplicationRowsValue((dkd_previous_rows_value) => (Array.isArray(dkd_previous_rows_value) ? dkd_previous_rows_value : []).filter((dkd_previous_row_value) => {
                const dkd_previous_application_id_value = String(dkd_previous_row_value?.application_id || dkd_previous_row_value?.id || '').trim();
                const dkd_previous_user_id_value = String(dkd_previous_row_value?.user_id || dkd_previous_row_value?.applicant_user_id || '').trim();
                return dkd_previous_application_id_value !== dkd_application_id_value && dkd_previous_user_id_value !== dkd_user_id_value;
              }));
              Alert.alert('Kurye Başvuruları', 'Başvuru silindi.');
              await dkd_refresh_application_rows_value();
            } catch (dkd_error_value) {
              Alert.alert('Kurye Başvuruları', dkd_error_value?.message || 'Başvuru silinemedi. SQL dosyasını Supabase üzerinde çalıştırdığından emin ol.');
            } finally {
              setDkdApplicationLoadingValue(false);
            }
          },
  ...
  return (
    <>
          ...
          <ScrollView contentContainerStyle={dkd_styles.dkd_scroll_content} keyboardShouldPersistTaps="handled">
            ...
            {dkd_admin_page_value === 'dashboard' ? (
              <>
                ...
                <DkdCenterActionCard
                  dkd_title_value="Kurye Ödeme Paneli"
                  dkd_badge_value="KAZANÇ / ÖDEME"
                  dkd_meta_value="Kurye cüzdanı, toplam kazanç, ödenmiş tutar ve dönemsel ödeme takibini aç."
                  dkd_colors_value={['#FFB240', '#FF7A2E']}
                  dkd_icon_name_value="bank-transfer-out"
                  dkd_on_press_value={() => dkd_set_courier_payouts_open_value(true)}
                />
                      ...
                      <Pressable key={String(dkd_job_row_value?.id || Math.random())} style={[dkd_styles.dkd_job_card, dkd_is_selected_value && dkd_styles.dkd_job_card_on]} onPress={() => dkd_pick_job_row_value(dkd_job_row_value)}>
                        <LinearGradient colors={dkd_is_selected_value ? ['rgba(246,181,78,0.16)', 'rgba(255,255,255,0.04)'] : ['rgba(255,255,255,0.06)', 'rgba(255,255,255,0.03)']} style={dkd_styles.dkd_job_fill}>
                          ...
                          <View style={dkd_styles.dkd_job_aside}>
                            ...
                            <SecondaryButton label="SİL" icon="trash-can-outline" onPress={() => dkd_handle_job_delete_value(dkd_job_row_value?.id)} size="compact" fullWidth={false} style={dkd_styles.dkd_row_button} />
                          </View>
                        </LinearGradient>
                      </Pressable>
                    );
                ...
                <View style={[dkd_styles.dkd_panel, { marginTop: 12 }]}> 
                  ...
                  {dkd_selected_application_row_value ? (
                    <>
                      ...
                      <View style={dkd_styles.dkd_application_danger_panel}>
                        <View style={{ flex: 1 }}>
                          <Text style={dkd_styles.dkd_application_danger_title}>Başvuru kaydını sil</Text>
                          <Text style={dkd_styles.dkd_application_danger_text}>Yanlış, test veya gereksiz kurye başvurularını inceleme ekranından kaldır.</Text>
                        </View>
                        <Pressable style={dkd_styles.dkd_application_delete_button} onPress={() => dkd_handle_application_delete_value(dkd_selected_application_row_value)}>
                          <MaterialCommunityIcons name="trash-can-outline" size={18} color="#FFE8E8" />
                          <Text style={dkd_styles.dkd_application_delete_text}>Başvuruyu Sil</Text>
                        </Pressable>
                      </View>
                    </>
                  ) : (
      ...
      {React.createElement(dkd_admin_courier_payouts_modal, {
        visible: dkd_courier_payouts_open_value,
        onClose: () => dkd_set_courier_payouts_open_value(false),
      })}
    </>
  );
}
...
const dkd_styles = StyleSheet.create({
  ...
  dkd_application_danger_panel: {
    marginTop: 12,
    borderRadius: 18,
    borderWidth: 1,
    borderColor: 'rgba(255,88,112,0.34)',
    backgroundColor: 'rgba(255,88,112,0.10)',
    padding: 12,
    flexDirection: 'row',
    alignItems: 'center',
    gap: 10,
  ...
  dkd_application_danger_title: {
    color: '#FFE8E8',
    fontSize: 13,
    fontWeight: '900',
  },
  dkd_application_danger_text: {
    color: 'rgba(255,232,232,0.74)',
    fontSize: 11,
    lineHeight: 15,
    marginTop: 3,
  },
  dkd_application_delete_button: {
    borderRadius: 14,
    borderWidth: 1,
    borderColor: 'rgba(255,88,112,0.50)',
    backgroundColor: 'rgba(255,88,112,0.24)',
    paddingHorizontal: 12,
    paddingVertical: 10,
    flexDirection: 'row',
    alignItems: 'center',
