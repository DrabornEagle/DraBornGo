...
function dkd_account_deletion_status_label_value(dkd_status_value) {
  const dkd_key_value = String(dkd_status_value || 'pending').toLowerCase();
  if (dkd_key_value === 'approved') return 'Silme onaylandı';
  if (dkd_key_value === 'rejected') return 'Reddedildi';
  if (dkd_key_value === 'deleted') return 'Silindi';
...
function DkdAdminAccountDeletionRequestCard({ dkd_item_value, dkd_on_approve_value, dkd_on_reject_value }) {
  const dkd_status_value = String(dkd_item_value?.dkd_status_value || 'pending').toLowerCase();
  const dkd_user_text_value = dkd_text_value(dkd_item_value?.dkd_display_name_value || dkd_item_value?.dkd_user_email_value || dkd_item_value?.dkd_user_id_value, 'Kullanıcı');
  const dkd_requested_at_value = dkd_text_value(dkd_item_value?.dkd_requested_at_value, 'Tarih yok');

  return (
    <View style={dkd_styles.dkdApplicationCard}>
      <LinearGradient colors={["rgba(255,88,112,0.10)", "rgba(255,255,255,0.025)"]} style={StyleSheet.absoluteFill} />
      <View style={dkd_styles.dkdCardHeaderRow}>
        <View style={[dkd_styles.dkdCardIconShell, dkd_styles.dkdDangerIconShell]}>
          <MaterialCommunityIcons name="account-remove-outline" size={22} color="#FFD6D6" />
        </View>
        <View style={dkd_styles.dkdCardTitleWrap}>
          <Text style={dkd_styles.dkdCardKicker}>HESAP VE VERİ SİLME TALEBİ</Text>
          <Text style={dkd_styles.dkdCardTitle}>{dkd_user_text_value}</Text>
          <Text style={dkd_styles.dkdCardSub}>{dkd_text_value(dkd_item_value?.dkd_user_id_value)} • {dkd_requested_at_value}</Text>
        </View>
        <View style={dkd_styles.dkdStatusPill}>
          <Text style={dkd_styles.dkdStatusText}>{dkd_account_deletion_status_label_value(dkd_status_value)}</Text>
        </View>
      </View>
      <View style={dkd_styles.dkdMiniGrid}>
        <DkdAdminApplicationField dkd_label_value="E-posta" dkd_value={dkd_item_value?.dkd_user_email_value} />
        <DkdAdminApplicationField dkd_label_value="Not" dkd_value={dkd_item_value?.dkd_request_note_value} />
        <DkdAdminApplicationField dkd_label_value="Durum" dkd_value={dkd_account_deletion_status_label_value(dkd_status_value)} />
      </View>
      <View style={dkd_styles.dkdActionRow}>
        <Pressable
          style={({ pressed }) => [dkd_styles.dkdDeleteDataButton, pressed && dkd_styles.dkdPressed, dkd_status_value !== 'pending' && dkd_styles.dkdDisabledButton]}
          disabled={dkd_status_value !== 'pending'}
          onPress={() => dkd_on_approve_value?.(dkd_item_value)}
        >
          <MaterialCommunityIcons name="database-remove-outline" size={16} color="#FFE8E8" />
          <Text style={dkd_styles.dkdRejectButtonText}>Verileri Sil ve Onayla</Text>
        </Pressable>
        <Pressable
          style={({ pressed }) => [dkd_styles.dkdGhostButton, pressed && dkd_styles.dkdPressed, dkd_status_value !== 'pending' && dkd_styles.dkdDisabledButton]}
          disabled={dkd_status_value !== 'pending'}
          onPress={() => dkd_on_reject_value?.(dkd_item_value)}
        >
          <MaterialCommunityIcons name="close-octagon-outline" size={16} color="#EAFBFF" />
          <Text style={dkd_styles.dkdGhostButtonText}>Reddet</Text>
        </Pressable>
      </View>
    </View>
  );
...
export default function DkdAdminApplicationsModal({ visible, onClose }) {
  ...
  const [dkd_account_deletion_rows_value, dkd_set_account_deletion_rows_value] = useState([]);
  const [dkd_selected_value, dkd_set_selected_value] = useState(null);

  const dkd_active_rows_value = dkd_tab_value === 'courier'
    ? dkd_courier_rows_value
    : dkd_tab_value === 'logistics'
      ? dkd_logistics_rows_value
      : dkd_account_deletion_rows_value;
  const dkd_pending_count_value = dkd_active_rows_value.filter((dkd_row_value) => String(dkd_row_value?.dkd_status_value || dkd_row_value?.dkd_status || dkd_row_value?.status || 'pending').toLowerCase() === 'pending').length;
  ...
  const dkd_load_value = useCallback(async () => {
    try {
      ...
      const dkd_account_deletion_result_value = await dkd_fetch_admin_account_deletion_requests_value();
      if (dkd_account_deletion_result_value?.dkd_error_value) throw dkd_account_deletion_result_value.dkd_error_value;
      dkd_set_account_deletion_rows_value(dkd_account_deletion_result_value?.dkd_data_value || []);
  ...
  const dkd_approve_account_deletion_value = useCallback((dkd_item_value) => {
    const dkd_request_id_value = String(dkd_item_value?.dkd_id_value || '').trim();
    if (!dkd_request_id_value) return;
    Alert.alert(
      'Hesap ve Veri Silme',
      'Bu talep onaylanınca kullanıcıya bağlı uygulama verileri ve auth kaydı silinir. Devam edilsin mi?',
      [
        { text: 'Vazgeç', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              dkd_set_loading_value(true);
              const dkd_result_value = await dkd_approve_account_deletion_request_value({
                dkd_request_id_value,
                dkd_admin_note_value: 'Admin başvurular sayfasından onaylandı.',
              });
              if (dkd_result_value?.dkd_error_value) throw dkd_result_value.dkd_error_value;
              Alert.alert('Hesap ve Veri Silme', 'Kullanıcı verileri silindi ve talep onaylandı.');
              await dkd_load_value();
            } catch (dkd_error_value) {
              Alert.alert('Hesap ve Veri Silme', dkd_error_value?.message || 'Silme işlemi tamamlanamadı. SQL dosyasını Supabase üzerinde çalıştırdığından emin ol.');
            } finally {
              dkd_set_loading_value(false);
            }
          },
  ...
  const dkd_reject_account_deletion_value = useCallback((dkd_item_value) => {
    const dkd_request_id_value = String(dkd_item_value?.dkd_id_value || '').trim();
    if (!dkd_request_id_value) return;
    Alert.alert('Hesap ve Veri Silme', 'Bu silme talebi reddedilsin mi?', [
      { text: 'Vazgeç', style: 'cancel' },
      {
        text: 'Reddet',
        style: 'destructive',
        onPress: async () => {
          try {
            dkd_set_loading_value(true);
            const dkd_result_value = await dkd_reject_account_deletion_request_value({
              dkd_request_id_value,
              dkd_admin_note_value: 'Admin başvurular sayfasından reddedildi.',
            });
            if (dkd_result_value?.dkd_error_value) throw dkd_result_value.dkd_error_value;
            Alert.alert('Hesap ve Veri Silme', 'Talep reddedildi.');
            await dkd_load_value();
          } catch (dkd_error_value) {
            Alert.alert('Hesap ve Veri Silme', dkd_error_value?.message || 'Talep reddedilemedi.');
          } finally {
            dkd_set_loading_value(false);
          }
        },
      ...
      <View style={dkd_styles.dkdBackdrop}>
        <LinearGradient colors={["#04101A", "#0B1426", "#090E18"]} style={dkd_styles.dkdCard}>
          ...
          <View style={dkd_styles.dkdStatsRow}>
            ...
            <DkdAdminApplicationStat dkd_icon_value="account-remove-outline" dkd_label_value="Silme" dkd_value={String(dkd_account_deletion_rows_value.length)} dkd_accent_value="#FFD6D6" />
          ...
          <View style={dkd_styles.dkdTabRow}>
            ...
            <Pressable
              style={[dkd_styles.dkdTabButton, dkd_tab_value === 'account_deletion' && dkd_styles.dkdTabButtonActive]}
              onPress={() => { dkd_set_tab_value('account_deletion'); dkd_set_selected_value(null); }}
            >
              <MaterialCommunityIcons name="account-remove-outline" size={18} color={dkd_tab_value === 'account_deletion' ? '#06121D' : '#EAFBFF'} />
              <View style={dkd_styles.dkdTabCopyWrap}>
                <Text style={[dkd_styles.dkdTabText, dkd_tab_value === 'account_deletion' && dkd_styles.dkdTabTextActive]}>Hesap Silme Talepleri</Text>
                <Text style={[dkd_styles.dkdTabSubText, dkd_tab_value === 'account_deletion' && dkd_styles.dkdTabSubTextActive]}>Profil sayfasından gelen veri silme başvuruları</Text>
              </View>
              <Text style={[dkd_styles.dkdTabCountText, dkd_tab_value === 'account_deletion' && dkd_styles.dkdTabCountTextActive]}>{dkd_account_deletion_rows_value.length}</Text>
            </Pressable>
          </View>
          ...
          {dkd_loading_value ? (
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={dkd_styles.dkdScrollContent}>
              {dkd_active_rows_value.length ? dkd_active_rows_value.map((dkd_item_value) => (
                dkd_tab_value === 'account_deletion' ? (
                  <DkdAdminAccountDeletionRequestCard
                    key={`dkd_account_deletion_${dkd_item_value?.dkd_id_value || dkd_item_value?.dkd_user_id_value}`}
                    dkd_item_value={dkd_item_value}
                    dkd_on_approve_value={dkd_approve_account_deletion_value}
                    dkd_on_reject_value={dkd_reject_account_deletion_value}
                  />
                ) : (
              ...
              {dkd_tab_value !== 'account_deletion' && dkd_selected_value ? (
...
const dkd_styles = StyleSheet.create({
  ...
  dkdDangerIconShell: { backgroundColor: 'rgba(255,88,112,0.18)', borderColor: 'rgba(255,88,112,0.34)' },
  ...
  dkdDeleteDataButton: { flexGrow: 1, borderRadius: 14, backgroundColor: 'rgba(255,88,112,0.38)', borderWidth: 1, borderColor: 'rgba(255,88,112,0.62)', paddingVertical: 10, paddingHorizontal: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6 },
  dkdDisabledButton: { opacity: 0.48 },
