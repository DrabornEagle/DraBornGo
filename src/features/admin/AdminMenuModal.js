    ...
    <Pressable style={dkd_styles.actionCard} onPress={onPress}>
      <LinearGradient colors={['rgba(255,255,255,0.08)', 'rgba(255,255,255,0.03)']} style={dkd_styles.actionFill}>
        ...
        <View style={dkd_styles.actionTextWrap}>
          ...
          {sub ? <Text style={dkd_styles.actionSub}>{sub}</Text> : null}
        </View>
...
export default function AdminMenuModal({ visible, onClose, onLoot, onDrops, onUsers, onCourierJobs, onBroadcast, onBoss, onMarket, onApplications }) {
  ...
  const [dkd_social_moderation_open, dkd_set_social_moderation_open] = useState(false);
  ...
  return (
    <>
        ...
        <View style={dkd_styles.backdrop}>
          <LinearGradient colors={['#04101A', '#0B1426', '#090E18']} style={dkd_styles.card}>
            ...
            <View style={dkd_styles.statsRow}>
              <GlassStat label="MODÜL" value="11" />
              <GlassStat label="ÖZEL" value="HAZIR" accent={cityLootTheme.colors.goldSoft} />
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={dkd_styles.scrollContent}>
              <View style={dkd_styles.grid}>
                ...
                <AdminAction
                  ...
                  sub="DM, sosyal ve arkadaşlık şikayetlerini incele; raporları çözüldü veya kapatıldı olarak işaretle."
                  onPress={() => dkd_set_social_moderation_open(true)}
                  tone="gold"
                />
                ...
                <AdminAction icon="map-marker-path" title="Saha Drop" sub="Harita noktaları, özel hedefler ve saha akışını yönet" onPress={onDrops} tone="gold" />
                <AdminAction icon="robot-angry-outline" title="Özel Hedef Komuta" sub="Özel hedef görseli, soru seti, giriş kartı ve ödül tanımı" onPress={onBoss} tone="gold" />
                <AdminAction icon="store-cog-outline" title="Market Komuta" sub="Kazanılmış puan vitrini, güvenli ödül içerikleri ve görünüm sırası" onPress={onMarket} tone="cyan" />
                ...
                <AdminAction icon="account-group-outline" title="Oyuncu Kontrolü" sub="XP, level, puan, özel kart ve kurye statülerini düzenle" onPress={onUsers} />
      ...
      {React.createElement(dkd_social_moderation_admin_modal, {
        visible: dkd_social_moderation_open,
        onClose: () => dkd_set_social_moderation_open(false),
      })}
    </>
  );
}
