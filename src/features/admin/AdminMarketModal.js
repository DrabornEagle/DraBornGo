...
const EMPTY_UI = {
  hero_kicker: 'KAZANILMIŞ PUAN VİTRİNİ',
  hero_title: 'Görevden kazan, uygulamada kullan',
  hero_subtitle: 'Puan satın alınmaz; yalnızca görev, hizmet ve etkinlik akışlarından kazanılır. TL cüzdan dijital ürün veya oyun içi avantaj satın almak için kullanılmaz.',
  logic_title: 'Paket mantığı',
  logic_body: 'Koleksiyon puanı, özel kart ve enerji yalnızca kazanılmış puan/görev akışıyla yönetilir; gerçek para karşılığı dijital içerik satışı yapılmaz.',
...
const EMPTY_REWARD_TYPE = {
  id: '',
  reward_kind: 'koleksiyon_puani',
  title: 'Koleksiyon Puanı',
  subtitle: 'Kazanılmış kaynak',
  resource_target: 'koleksiyon_puani',
  icon_name: 'puzzle-star-outline',
  accent_key: 'cyan',
  is_active: true,
};
...
const RESOURCE_TARGETS = ['koleksiyon_puani', 'special_cards', 'energy', 'puan', 'xp'];
  ...
  return (
    <Pressable onPress={onPress} style={[dkd_styles.listCard, active && dkd_styles.listCardActive]}>
      ...
      <View style={dkd_styles.metaRow}>
        ...
        <MetaPill text={`${String(item?.price_puan ?? 0)} kazanılmış puan`} />
...
export default memo(function AdminMarketModal({
  ...
  useEffect(() => {
    ...
    setPackDraft(firstPack ? {
      ...
      price_puan: String(firstPack?.price_puan ?? '0'),
  ...
  function pickPack(item) {
    ...
    setPackDraft({
      ...
      ...item,
      id: String(item?.id || ''),
      price_puan: String(item?.price_puan ?? '0'),
  ...
  async function handleSavePack() {
    ...
    const pricePuan = Number(packDraft.price_puan || 0);
    const rewardAmount = Number(packDraft.reward_amount || 0);
    const sortOrder = Number(packDraft.sort_order || 0);
    if (!Number.isFinite(pricePuan) || pricePuan < 0) return Alert.alert('Admin', 'Fiyat 0 veya daha büyük olmalı.');
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={dkd_styles.content}>
            <GlassPanel style={dkd_styles.heroCard}>
              <View style={dkd_styles.heroTop}>
                ...
                <View style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <Text style={dkd_styles.sub}>Kazanılmış puan vitrinini, ödül tiplerini, görselleri ve görünüm sırasını tek merkezden yönet.</Text>
                </View>
            ...
            <GlassPanel>
              <SectionHeader
                ...
                subtitle="Kazanılmış puan vitrini kahraman alanı, ikon, preset ve arka plan görseli burada yönetilir."
                stacked={isNarrow}
                actions={[
              ...
              <View style={dkd_styles.rowWrap}>
                <Field label="Kicker" value={uiDraft.hero_kicker} onChangeText={(value) => setUiDraft((prev) => ({ ...prev, hero_kicker: value }))} placeholder="PUAN MAĞAZASI" style={isNarrow ? dkd_styles.fieldFull : dkd_styles.fieldHalf} />
            ...
            <GlassPanel>
              ...
              <View style={dkd_styles.chipRow}>
                {(rewardKindOptions.length ? rewardKindOptions : ['koleksiyon_puani', 'special_cards', 'energy']).map((kind) => (
                  <ChoiceChip key={kind} label={kind} active={packDraft.reward_kind === kind} onPress={() => setPackDraft((prev) => ({ ...prev, reward_kind: kind }))} />
                ))}
              </View>

              <View style={dkd_styles.rowWrap}>
                <Field label="Fiyat puan" value={packDraft.price_puan} onChangeText={(value) => setPackDraft((prev) => ({ ...prev, price_puan: value }))} keyboardType="numeric" placeholder="120" style={isNarrow ? dkd_styles.fieldFull : dkd_styles.fieldThird} />
              ...
              <View style={dkd_styles.switchRow}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={dkd_styles.switchSub}>Kapalı paket markette görünmez ve kullanıma açılmaz.</Text>
                </View>
            ...
            <GlassPanel>
              ...
              <View style={dkd_styles.switchRow}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={dkd_styles.switchSub}>Kapalı tür, yeni paketlerde görünür ama kullanım için pasif kalır.</Text>
                </View>
