...
const SERIES_PRESETS = ['GENERAL', 'SKYLINE', 'NEON', 'ORBIT', 'APEX', 'OZEL'];
const THEME_PRESETS = [
  'City Core',
  'Neon Harbor',
  'Retail Megaplex',
...
export default memo(function AdminLootModal({ visible, onClose, loading, cardDefs, cardSearch, setCardSearch, onAdd, onDelete }) {
  ...
  async function pickImage(mode = 'card') {
    ...
    patchForm('art_image_url', dkd_selected_asset_value.uri);
  }
...
Bu işlem kartı sistemden tamamen siler. Kart havuzu, oyuncu envanterleri ve market kayıtları da temizlenir.`,
      [
