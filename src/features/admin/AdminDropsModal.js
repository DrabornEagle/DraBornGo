...
const dkd_drop_type_icon_values = {
  qr: 'qrcode-scan',
  map: 'map-marker-radius-outline',
  boss: 'crown-circle-outline',
  cafe: 'coffee-outline',
...
function dkd_drop_type_icon_name_value(dkd_drop_type_value) {
  const dkd_drop_type_key_value = String(dkd_drop_type_value || '').toLowerCase();
  return dkd_drop_type_icon_values[dkd_drop_type_key_value] || 'map-marker-radius-outline';
}
    ...
    <View style={styles.dropRow}>
      <View style={styles.dropIconWrap}>
        <MaterialCommunityIcons
          name={dkd_drop_type_icon_name_value(item?.type)}
          size={22}
      ...
      <SafeScreen style={{ flex: 1, backgroundColor: cityLootTheme.colors.bgTop }}>
        ...
        <LinearGradient colors={[cityLootTheme.colors.bgTop, cityLootTheme.colors.bgMid, cityLootTheme.colors.bgBottom]} style={styles.screen}>
          <FlatList
              ...
              <View>
                <GlassPanel style={styles.heroPanel}>
                  ...
                  <View style={styles.metricRow}>
                    ...
                    <MetricCard icon="crown-outline" label="Özel" value={formatInt(bossCount)} tone="gold" />
                ...
                <GlassPanel style={{ marginTop: 14 }}>
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipRowContent}>
                    {DROP_TYPES.map((dkd_drop_type_value) => (
                      <Chip
                        key={dkd_drop_type_value}
                        active={draft.type === dkd_drop_type_value}
                        label={trDropType(dkd_drop_type_value)}
                        icon={dkd_drop_type_icon_name_value(dkd_drop_type_value)}
                        onPress={() => setDraft((dkd_previous_drop_draft) => ({ ...dkd_previous_drop_draft, type: dkd_drop_type_value }))}
                      />
                    ))}
                  </ScrollView>
                ...
                <View style={styles.listHead}>
                  <Text style={styles.sectionTitle}>Kayıtlı Ödül Noktası Listesi</Text>
            ...
            ListEmptyComponent={loading ? <ActivityIndicator color={cityLootTheme.colors.cyanSoft} style={{ marginTop: 24 }} /> : <Text style={styles.emptyText}>Kayıtlı ödül noktası yok.</Text>}
          />
        </LinearGradient>
      </SafeScreen>
    </Modal>
