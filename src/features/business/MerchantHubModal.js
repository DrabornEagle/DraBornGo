      ...
      <View style={dkd_styles.productRow}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={dkd_styles.productMeta} numberOfLines={1}>
            {(product?.category || 'genel')} • {Number(product?.price_puan || 0)} fiziksel sipariş puanı • ürün {product?.price_cash == null ? '-' : Number(product?.price_cash || 0)} TL • kurye {Number(product?.delivery_fee_tl || 0)} TL • stok {Number(product?.stock || 0)}
          </Text>
...
export default function MerchantHubModal({ visible, onClose }) {
  ...
  const pickImage = async () => {
    try {
      ...
      setProductDraft((dkd_previous_product_draft) => ({ ...normalizeProductDraft(dkd_previous_product_draft), imageUrl: dkd_selected_asset_value.uri }));
    } catch (error) {
          ...
          {!hasBusinesses ? (
            ...
            <>
              ...
              <View style={dkd_styles.tabsRow}>
                ...
                <TabButton active={tab === 'cashier'} icon="cards-playing-diamond-outline" label="Kasa" onPress={() => setTab('cashier')} />
                  ...
                  <>
                    ...
                    >
                      ...
                      <View style={dkd_styles.dualRow}>
                        ...
                        <Field label="Fiziksel sipariş puanı" value={String(productDraft.pricePuan || '')} onChangeText={(dkd_price_puan_value) => setProductDraft((dkd_previous_product_draft) => ({ ...normalizeProductDraft(dkd_previous_product_draft), pricePuan: dkd_price_puan_value }))} placeholder="120" containerStyle={dkd_styles.fieldHalf} keyboardType="numeric" autoCapitalize="none" />
                      </View>
                      <View style={dkd_styles.dualRow}>
