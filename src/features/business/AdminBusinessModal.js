  ...
  return (
    <Pressable style={dkd_styles.productCard} onPress={() => onEdit?.(item)}>
      ...
      <View style={dkd_styles.productBody}>
        ...
        <Text style={dkd_styles.productMeta} numberOfLines={1}>{item?.category || 'genel'} • {Number(item?.price_puan || 0)} fiziksel sipariş puanı • ürün {item?.price_cash == null ? '-' : Number(item?.price_cash || 0)} TL • kurye {Number(item?.delivery_fee_tl || 0)} TL • stok {Number(item?.stock || 0)}</Text>
...
export default function AdminBusinessModal({ visible, onClose }) {
  ...
  const chooseProductToEdit = useCallback((product) => {
    ...
    setProductDraft({
      ...
      pricePuan: String(product.price_puan || 0),
  ...
  const pickProductImage = useCallback(async () => {
    try {
      ...
      const dkd_selected_asset_value = dkd_picker_result_value?.assets?.[0];
      if (!dkd_selected_asset_value?.uri) return;
      setProductDraft((dkd_previous_product_draft) => ({ ...dkd_previous_product_draft, imageUrl: dkd_selected_asset_value.uri }));
    } catch (error) {
  ...
  const saveProductNow = useCallback(async () => {
    ...
    try {
      ...
      await upsertMerchantBusinessProduct({
        ...
        pricePuan: Number(productDraft?.pricePuan || 0),
  ...
  }, [selectedBusiness, selectedBusinessId, productDraft, loadProducts, resetProductDraft, setMessage]);
  ...
  }, [loadProducts, productDraft?.id, resetProductDraft, selectedBusinessId, setMessage]);
      ...
      <View style={dkd_styles.backdrop}>
        <LinearGradient colors={['#06101D', '#091525', '#03070D']} style={dkd_styles.shell}>
          ...
          <View style={dkd_styles.header}>
            <View style={{ flex: 1 }}>
              <Text style={dkd_styles.kicker}>DraBornGo BUSINESS ADMIN</Text>
          ...
          <View style={dkd_styles.tabsRow}>
            ...
            <Tab active={tab === 'cashier'} icon="cards-playing-diamond-outline" label="Kasa" onPress={() => setTab('cashier')} />
          </View>
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={dkd_styles.scrollContent}>
            {tab === 'setup' ? (
              <>
                <Section title="1) İşletme kartı" sub="Yeni işletme için alanlar boş gelir. Mevcut işletmeyi seçersen bilgileri burada düzenlersin.">
                  <Field label="İşletme adı" value={businessDraft.name} onChangeText={(value) => setBusinessDraft((dkd_business_draft) => ({ ...dkd_business_draft, name: value }))} placeholder="DraBornGo Cafe" />
                  <Field label="Slug" value={businessDraft.slug} onChangeText={(value) => setBusinessDraft((dkd_business_draft) => ({ ...dkd_business_draft, slug: value }))} placeholder="dkd_draborngo_cafe" autoCapitalize="none" />
                ...
                <Section title="2) Bağlı ödül noktaları" sub="İşletmeye bağlı noktaları istersen buradan kaldırabilirsin.">
            ...
            {tab === 'market' ? (
              <>
                <Section title="İşletme market ayarları" sub="İşletme panelindeki fiziksel ürün/hizmet ayarlarını buradan da yönet. Ürünler işletme adına göre gruplanır ve kazanılmış fiziksel hizmet puanıyla sipariş akışına eklenir.">
                  ...
                  <View style={dkd_styles.dualRow}>
                    <Field label="Fiziksel sipariş puanı" value={productDraft.pricePuan} onChangeText={(value) => setProductDraft((dkd_product_draft) => ({ ...dkd_product_draft, pricePuan: value }))} placeholder="0" containerStyle={dkd_styles.fieldHalf} keyboardType="numeric" />
                  ...
                  <View style={dkd_styles.previewCard}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={dkd_styles.previewSub}>{productDraft.category || 'genel'} • {Number(productDraft.pricePuan || 0)} fiziksel sipariş puanı • ürün {productDraft.priceCash === '' ? '-' : Number(productDraft.priceCash || 0)} TL • kurye {Number(productDraft.deliveryFeeTl || 0)} TL</Text>
