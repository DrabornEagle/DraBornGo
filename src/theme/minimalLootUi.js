...
export function dropAccent(drop = {}) {
  ...
  if (type === 'boss' || type.includes('boss')) {
    return { icon: 'crown-outline', tone: 'boss', title: 'Özel Hedef', color: '#F6CD67', bg: 'rgba(246,205,103,0.16)', border: 'rgba(246,205,103,0.30)' };
  }
  if (type === 'qr') {
  ...
  if (name.includes('metro')) {
    return { icon: 'train', tone: 'metro', title: 'Metro', color: '#8DCCFF', bg: 'rgba(141,205,255,0.14)', border: 'rgba(141,205,255,0.28)' };
  }
