...
if (dkd_app_value?.expo?.version !== '0.0.15') dkd_fail_value('app.json expo.version must be 0.0.15');
if (Number(dkd_app_value?.expo?.android?.versionCode) !== 3) dkd_fail_value('Android versionCode must be 3');
if (dkd_package_value?.version !== '0.0.15') dkd_fail_value('package.json version must be 0.0.15');
if (dkd_app_value?.expo?.android?.package !== 'com.draborneagle.draborngo') dkd_fail_value('Android package changed unexpectedly');

console.log('DraBornGo v0.0.15 / Android versionCode 3 identity: PASS');
