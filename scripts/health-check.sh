...
grep -n '"scheme": "draborngo"' app.json >/dev/null 2>&1 && echo "[OK] scheme draborngo" || echo "[WARN] scheme draborngo bulunamadi"

echo "== cleanup checks =="
find . -path './archive' -prune -o -type f \( -name '*.bak' -o -name '*.bak_*' \) -print >./audit/phase23/dkd_draborngo_bak_list.txt || true
BAK_COUNT=$(wc -l < ./audit/phase23/dkd_draborngo_bak_list.txt | tr -d ' ')
