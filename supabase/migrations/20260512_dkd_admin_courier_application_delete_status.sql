...
drop function if exists public.dkd_admin_courier_application_delete_value(text, text);
...
declare
  ...
  dkd_has_online_at_column_value boolean := false;
begin
  ...
  if to_regclass('public.dkd_profiles') is null then
    raise exception 'dkd_profiles_missing';
  end if;
  ...
  dkd_deleted_summary_value as (
    select
      count(*)::integer as dkd_deleted_count,
      (array_agg(dkd_deleted_rows_value.user_id))[1] as dkd_deleted_user_id
    from dkd_deleted_rows_value
  )
  select
    dkd_deleted_summary_value.dkd_deleted_count,
    dkd_deleted_summary_value.dkd_deleted_user_id
  into
    dkd_deleted_count_value,
    dkd_deleted_user_id_value
  from dkd_deleted_summary_value;
  ...
  if dkd_deleted_user_id_value is not null then
    ...
    select exists (
      ...
      where dkd_column_scope.table_schema = 'public'
        and dkd_column_scope.table_name = 'dkd_profiles'
        and dkd_column_scope.column_name = 'dkd_courier_last_online_at'
    )
    into dkd_has_online_at_column_value;

    if dkd_has_online_column_value and dkd_has_online_at_column_value then
    ...
    elsif dkd_has_online_column_value then
      execute 'update public.dkd_profiles set dkd_courier_online = false where user_id = $1'
      using dkd_deleted_user_id_value;
    end if;
  end if;
...
comment on function public.dkd_admin_courier_application_delete_value(text, uuid) is 'Kurye başvuru silme RPC max(uuid) hatasını kaldırır, array_agg ile güvenli kullanıcı seçer ve mevcut online kolonlarına göre lisans durumunu kapatır.';
