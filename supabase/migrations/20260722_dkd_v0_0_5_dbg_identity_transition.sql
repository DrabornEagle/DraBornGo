...
begin
  if to_regclass('public.idx_dkd_profiles_ally_id') is not null
    ...
    alter index public.idx_dkd_profiles_ally_id
      rename to idx_dkd_profiles_dbg_id;
  end if;
end
$dkd$;
...
revoke all on function public.dkd_generate_dbg_id() from public;
grant execute on function public.dkd_generate_dbg_id() to public, authenticated, service_role;
...
create function public.dkd_social_search_profiles(
  dkd_param_query text,
...
returns table(
  ...
  rank_key text,
  social_status text,
  social_last_seen_at timestamp with time zone,
  is_friend boolean,
...
create function public.dkd_social_admin_moderation_queue(
...
returns table(
  ...
  dkd_reporter_dbg_id text,
  dkd_reporter_nickname text,
  dkd_reported_user_id uuid,
  dkd_reported_dbg_id text,
  dkd_reported_nickname text,
  dkd_admin_note_text text,
  dkd_created_at timestamp with time zone,
  dkd_updated_at timestamp with time zone,
  dkd_resolved_at timestamp with time zone
)
language plpgsql
security definer
set search_path to 'public'
...
begin
  ...
  left join public.dkd_profiles dkd_reporter_profile_row
    on dkd_reporter_profile_row.user_id = dkd_report_row.dkd_reporter_user_id
  left join public.dkd_profiles dkd_reported_profile_row
    on dkd_reported_profile_row.user_id = dkd_report_row.dkd_reported_user_id
...
begin
  ...
  dkd_friend_rows as (
    ...
    join public.dkd_profiles dkd_other_profile_row
      on dkd_other_profile_row.user_id = case
        when dkd_friendship_row.user_low = dkd_current_user_id then dkd_friendship_row.user_high
        else dkd_friendship_row.user_low
      end
    left join public.dkd_dm_threads dkd_thread_row
      on dkd_thread_row.friendship_id = dkd_friendship_row.id
    ...
    order by coalesce(dkd_thread_row.last_message_at, dkd_friendship_row.created_at) desc,
      dkd_other_profile_row.nickname asc
  ),
  dkd_incoming_rows as (
    ...
    from public.dkd_friend_requests dkd_request_row
    join public.dkd_profiles dkd_sender_profile_row
      on dkd_sender_profile_row.user_id = dkd_request_row.requester_id
  ...
  dkd_outgoing_rows as (
    select
      ...
      dkd_request_row.created_at
    from public.dkd_friend_requests dkd_request_row
    join public.dkd_profiles dkd_receiver_profile_row
      on dkd_receiver_profile_row.user_id = dkd_request_row.addressee_id
...
begin
  ...
  select public.dkd_profiles.dbg_id
  into dkd_var_dbg_id
  from public.dkd_profiles
...
grant execute on function public.dkd_social_touch_presence() to public, authenticated, service_role;
...
do $dkd$
begin
  if to_regclass('public.dkd_push_event_queue') is not null then
    update public.dkd_push_event_queue
    set dkd_route = 'dbg'
    where dkd_route = 'ally';

    update public.dkd_push_event_queue
    set dkd_target_screen = 'dbg'
    where dkd_target_screen = 'ally';
  end if;
end
$dkd$;
...
comment on column public.dkd_profiles.dbg_id is
  'DraBornGo kullanıcı kimliği. Önceki Ally_ID alanının DBG_ID karşılığıdır.';
...
begin
  if exists (
    ...
    raise exception 'dkd_dbg_migration_failed: ally_id column still exists';
  end if;
  ...
  if not exists (
    ...
    raise exception 'dkd_dbg_migration_failed: dbg_id column missing';
  end if;

  if exists (
    select 1
    from pg_proc dkd_proc
    join pg_namespace dkd_ns on dkd_ns.oid = dkd_proc.pronamespace
    where dkd_ns.nspname = 'public'
      and (
        dkd_proc.proname ilike '%ally%'
        or dkd_proc.prosrc ilike '%ally%'
      )
  ) then
    raise exception 'dkd_dbg_migration_failed: Ally reference remains in public functions';
  end if;
end
$dkd$;
